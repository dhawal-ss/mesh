use super::admission::ADMISSION_RESPONSE_MAX_BYTES;
use super::avatar::{
    MAX_AVATAR_DOWNLOAD_BYTES, MAX_PROFILE_AVATAR_DIMENSION, MAX_PROFILE_AVATAR_UPLOAD_BYTES,
};
use super::emoji::{CUSTOM_EMOJI_EVENT_TYPE, MAX_CUSTOM_EMOJI_UPLOAD_BYTES};
use super::messages::{ProjectedMemberProfile, SendQueueUsage};
use super::personal_data::{
    PersonalDataExportBudget, PersonalExportScanReservation, MAX_PERSONAL_EXPORT_JSON_BYTES,
};
use super::search::MAX_REMEMBERED_PRE_CANCELLED_SEARCHES;
use super::*;
use matrix_sdk::ruma::room::RestrictedSummary;
use matrix_sdk::{authentication::SessionTokens, SessionMeta};
use serde_json::json;
use tokio::io::{AsyncReadExt, AsyncWriteExt};

#[test]
fn expired_saved_sign_in_returns_to_reauthentication_without_hiding_other_failures() {
    assert!(allow_saved_account_reauthentication(BackendError::NotAuthenticated).is_ok());
    assert!(matches!(
        allow_saved_account_reauthentication(BackendError::Network("offline".into())),
        Err(BackendError::Network(_))
    ));
}

#[test]
fn startup_marks_only_local_permission_failures_as_fatal() {
    assert!(matches!(
        MatrixBackend::local_startup_error(BackendError::PermissionDenied("local store".into())),
        BackendError::LocalStatePermissionDenied(_)
    ));
    assert!(matches!(
        MatrixBackend::local_startup_error(BackendError::Network("offline".into())),
...
/*
    A downloaded avatar is whatever another person's client uploaded, and every
    mainstream Matrix client uploads a JPEG. This used to be read through
    `download_custom_emoji`, which requires the payload to already be PNG, so
    the common case failed with "server emoji failed PNG validation" and the
    renderer showed a generated mark instead of the person's picture.
*/
#[test]
fn a_downloaded_avatar_is_accepted_as_jpeg_or_webp_and_returned_as_bounded_png() {
    for format in [
        ...
        let mut source = Cursor::new(Vec::new());
        image::DynamicImage::new_rgb8(600, 300)
            .write_to(&mut source, format)
            .unwrap();

        let sanitized = MatrixBackend::sanitize_downloaded_avatar(source.get_ref())
            .unwrap_or_else(|error| panic!("{format:?} avatar was refused: {error:?}"));

        // Always PNG, whatever arrived: the renderer builds its blob as
        // image/png and never inspects the payload.
        assert!(sanitized.starts_with(b"\x89PNG\r\n\x1a\n"));
        let decoded = image::load_from_memory(&sanitized).unwrap();
        assert!(
            decoded.width() <= MAX_PROFILE_AVATAR_DIMENSION
                && decoded.height() <= MAX_PROFILE_AVATAR_DIMENSION,
            "{format:?} avatar kept its source size: {}x{}",
            decoded.width(),
            decoded.height()
        );
    }
...
#[test]
fn a_downloaded_avatar_that_is_not_a_supported_image_is_refused() {
    // Sniffed from the bytes, not read from the response Content-Type: the
    // header is a remote claim about remote content.
    assert!(MatrixBackend::sanitize_downloaded_avatar(b"GIF89a not really an image").is_err());
    assert!(MatrixBackend::sanitize_downloaded_avatar(b"").is_err());
    assert!(MatrixBackend::sanitize_downloaded_avatar(b"<html>nope</html>").is_err());
}

#[test]
fn the_avatar_download_ceiling_is_not_below_what_mesh_itself_uploads() {
    // The emoji path this used to borrow capped downloads at 512 KB while
    // set_profile_avatar_image accepts 1 MB on the way up, so a large but legal
    // Mesh upload was undownloadable by Mesh.
    assert!(MAX_AVATAR_DOWNLOAD_BYTES >= MAX_PROFILE_AVATAR_UPLOAD_BYTES as u64);
}

/*
    A community icon is room state, not account data, so unlike a profile
    picture it is gated. Rust has to be the one that decides: `community.role`
    in the renderer only hides the control.
*/
#[test]
fn room_avatar_permission_uses_the_rooms_own_threshold_then_state_default() {
    ...
    // Below the room's explicit m.room.avatar threshold, even though it is far
    // below state_default, which is what a room without the entry would use.
    assert!(!MatrixBackend::user_can_update_room_avatar(
        &power_levels,
        &alice
    ));
...
#[test]
fn room_avatar_permission_falls_back_to_state_default_and_users_default() {
    let alice = UserId::parse("@alice:example.org").unwrap();
    // No m.room.avatar entry, so state_default governs; no users entry for
    // Alice, so users_default is her level. An ordinary member must not be able
    // to change a community's image just because the room named no threshold.
    let ordinary: RoomPowerLevelsEventContent = serde_json::from_value(json!({
        "events": {},
        "state_default": 50,
        "users": {},
        "users_default": 0
    ...
    let promoted: RoomPowerLevelsEventContent = serde_json::from_value(json!({
        "events": {},
        "state_default": 50,
...
#[test]
fn composer_drafts_are_structured_bounded_and_new_message_only() {
    let at_limit = "😀".repeat(MAX_COMPOSER_DRAFT_BYTES / 4);
    let formatted = "<p><a href=\"https://matrix.to/#/%40alice%3Aexample.org\">@Alice</a></p>";
    let draft = MatrixBackend::new_message_composer_draft(ComposerDraftDto {
        body: at_limit.clone(),
    ...
    assert_eq!(draft.html_text.as_deref(), Some(formatted));
    ...
    assert_eq!(
        ...
        Some(ComposerDraftDto {
            body: at_limit,
            formatted_body: Some(formatted.into()),
        })
    );

    assert!(MatrixBackend::new_message_composer_draft(ComposerDraftDto {
        body: String::new(),
        formatted_body: None,
    })
    .unwrap()
    .is_none());
    assert!(MatrixBackend::new_message_composer_draft(ComposerDraftDto {
        body: "😀".repeat((MAX_COMPOSER_DRAFT_BYTES / 4) + 1),
        formatted_body: None,
    })
    .is_err());
    assert!(MatrixBackend::new_message_composer_draft(ComposerDraftDto {
        body: "hello @Alice".into(),
        formatted_body: Some("x".repeat(MAX_COMPOSER_DRAFT_FORMATTED_BYTES + 1)),
    })
    .is_err());
...
fn matrix_rtc_test_membership(
    ...
    ActiveMatrixRtcMembership {
        member: MatrixRtcMember {
            ...
            avatar_url: None,
            participant_identity: MatrixBackend::matrix_rtc_participant_identity(
                user_id, device_id, member_id,
            )
            .expect("test participant identity"),
        },
...
fn secure_store_failures_use_the_typed_crypto_boundary() {
    let error =
        MatrixBackend::map_secure_storage_error(anyhow::anyhow!("credential store is locked"));

    assert!(matches!(error, BackendError::Crypto(_)));
    assert!(error.to_string().contains("secure store is unavailable"));
}

// The oversized-secret guard only runs on Windows, where the Credential Manager
// imposes the blob limit; other platforms accept the value and never surface
// this configuration error.
#[cfg(target_os = "windows")]
#[test]
fn oversized_secure_store_values_use_an_actionable_configuration_error() {
    let source =
        crate::crypto::keychain::store_secret("mesh-test-oversized-value", &vec![0_u8; 961])
            .expect_err("the size guard must run before opening the OS credential store");
    let error = MatrixBackend::map_secure_storage_error(source);

    assert!(matches!(error, BackendError::InvalidConfiguration(_)));
    assert!(error.to_string().contains("secure-store size limit"));
    assert!(error.to_string().contains("Remove an old account"));
}
...
fn durable_send_queue_checks_capacity_before_insert_and_uses_indexed_recovery() {
    ...
    let send_message = matrix_source
        ...
        .next()
        .unwrap();
    // rustfmt wraps long method chains across lines, so a literal like
    // `send_queue.local_echoes()` can end up split by a line break and
    // indentation. Strip whitespace before matching so a formatting-only
    // change doesn't make this test start failing on its own.
    let flat_send_message: String = send_message
        .chars()
        .filter(|c| !c.is_whitespace())
        .collect();
    let capacity_check = flat_send_message
        .find("ensure_send_queue_capacity")
        .unwrap();
    let durable_insert = flat_send_message.find(".send_raw(").unwrap();
    assert!(capacity_check < durable_insert);
    assert!(flat_send_message.contains("send_queue.local_echoes()"));
    assert!(flat_send_message.contains("queued_local_echo_from_updates"));
    assert_eq!(
        flat_send_message.matches("queue.subscribe().await").count(),
        1
    );
...
/// The room thread list uses the server's stable, standard `Participated`
/// thread filter (spec v1.4+), not the unstable MSC4306 thread-subscription
/// API this SDK build does not even compile in. Per-thread notification
/// settings stay an owner decision (P9), not a silent local substitute.
#[test]
fn thread_list_uses_the_stable_participated_filter_not_unstable_subscriptions() {
    let matrix_source = include_str!("../../matrix.rs");
    let messages_source = include_str!("../messages.rs");
    assert!(matrix_source.contains("IncludeThreads::Participated"));
    for source in [matrix_source, messages_source] {
        assert!(!source.contains("subscribe_thread"));
        assert!(!source.contains("unstable-msc4306"));
    }
}
...
fn direct_room_lookups_are_limited_to_guard_or_prejoin_paths() {
    let allowed = [
        ...
        "validated_community_invite_room",
        "knock_community",
        "join_community",
        // Returns a membership state, never a room, so nothing can be read
        // through it. See its doc comment in encryption.rs.
        "account_room_membership",
    ...
    for (file_name, source) in MATRIX_PRODUCTION_SOURCES {
        ...
        for (line_number, line) in source.lines().enumerate() {
            ...
            // Strip visibility before looking for the signature. Without this
            // the scan skipped every `pub(super) fn` outright, attributing its
            // body to whatever function was declared above it -- and
            // moderation.rs already had ten of those, so this invariant was
            // partly unenforced on a file it claims to cover. Real modules
            // make such declarations ordinary rather than rare.
            let trimmed = trimmed
...
#[test]
fn thread_participant_count_dedupes_repeat_repliers_and_counts_the_root_author() {
    let root = MessageDto {
        author_public_key: "@alice:example.org".into(),
        ..search_fixture("$root", "2026-08-16T12:00:00.000Z")
    };
    let replies = vec![
        MessageDto {
            author_public_key: "@bob:example.org".into(),
            ..search_fixture("$reply-one", "2026-08-16T12:01:00.000Z")
        },
        MessageDto {
            author_public_key: "@bob:example.org".into(),
            ..search_fixture("$reply-two", "2026-08-16T12:02:00.000Z")
        },
        MessageDto {
            author_public_key: "@alice:example.org".into(),
            ..search_fixture("$reply-three", "2026-08-16T12:03:00.000Z")
        },
    ];
    assert_eq!(MatrixBackend::thread_participant_count(&root, &replies), 2);
    assert_eq!(MatrixBackend::thread_participant_count(&root, &[]), 1);
}

#[test]
fn thread_last_activity_is_the_newest_timestamp_including_a_replyless_root() {
    let root = search_fixture("$root", "2026-08-16T12:00:00.000Z");
    let replies = vec![
        search_fixture("$reply-one", "2026-08-16T12:05:00.000Z"),
        search_fixture("$reply-two", "2026-08-16T12:02:00.000Z"),
    ];
    assert_eq!(
        MatrixBackend::thread_last_activity(&root, &replies),
        "2026-08-16T12:05:00.000Z"
    );
    assert_eq!(
        MatrixBackend::thread_last_activity(&root, &[]),
...
fn targeted_member_names_preserve_safe_fallbacks_for_missing_state() {
    ...
    MatrixBackend::apply_member_profiles(
        &mut messages,
        &HashMap::from([(
            "@alice:example.org".into(),
            ProjectedMemberProfile {
                display_name: "Alice Display".into(),
                avatar_url: Some("mxc://example.org/alice".into()),
            },
        )]),
    );
    ...
    assert_eq!(messages[1].author_avatar_url, None);
}

/// A picture someone removes has to disappear from their old messages too.
///
/// The member state in hand is the whole current answer for that person, so it
/// is assigned rather than merged. Merging would leave the old address on every
/// message that already carried one, and the only way back would be logging out.
#[test]
fn a_cleared_profile_picture_is_cleared_on_messages_that_carried_one() {
    let mut alice = search_fixture("$alice", "2026-08-06T00:00:00Z");
    alice.author_avatar_url = Some("mxc://example.org/old".into());
    let mut messages = vec![alice];
    ...
    assert_eq!(messages[0].author_avatar_url, None);
}
...
fn timeline_and_pins_use_targeted_local_member_state_without_member_sync() {
    ...
    let messages_path = matrix_source
        ...
        .next()
        .unwrap();
    assert!(messages_path.contains("resolve_projected_member_profiles"));
    ...
    let lookup_helper = messages_source
        .split("async fn resolve_projected_member_profiles(")
        .nth(1)
        .unwrap()
    ...
    assert!(pins_snapshot.contains("resolve_projected_member_profiles"));
...
fn production_syncs_lazy_load_room_members() {
    ...
    // Every sync Mesh runs, counted so a new one cannot quietly skip the lazy
    // member filter. Joining a community room added the ninth: the room's own
    // encryption state has to arrive before the join is allowed to stand.
    assert_eq!(
        matrix_source
...
fn direct_message_projection_reuses_matrix_message_and_attachment_fields() {
    let message = MessageDto {
        ...
        author_avatar_url: Some("mxc://example.org/alice".into()),
        content: "hello".into(),
    ...
    // A direct message is projected from a room message, so the author's
    // picture has to survive the conversion rather than stop at the room.
    assert_eq!(
        projected.author_avatar_url.as_deref(),
        Some("mxc://example.org/alice")
    );
}
...
fn existing_matrix_dm_surfaces_fail_closed_for_ignored_peers() {
    ...
    // The marked-unread flag is the person's own explicit action, not derived
    // from the blocked sender's activity, so the suppression branch must
    // still report the real value rather than hardcoding it away with the
    // counts it zeroes.
    assert!(unread.contains("unread_marked: room.is_marked_unread()"));
}
...
/*
    Whether a room of this community can be joined by being in this community.

    The account service answers it: a restricted room's summary carries the
    rooms whose members it admits. Mesh creates every community room admitting
    the community itself, so this is the ordinary case, and a room admitting a
    different space is the case that must not be offered.
*/
fn room_summary(join_rule: JoinRuleSummary) -> RoomSummary {
    RoomSummary::new(
...
#[test]
fn community_membership_admits_only_rooms_this_community_opens() {
    let space_id = RoomId::parse("!space:example.org").unwrap();
    let other_space = RoomId::parse("!other-space:example.org").unwrap();
    let admits = |rule| MatrixBackend::community_membership_admits(&space_id, &room_summary(rule));

    assert!(admits(JoinRuleSummary::Public));
    assert!(admits(JoinRuleSummary::Restricted(RestrictedSummary::new(
        vec![space_id.clone()]
    ))));
    assert!(admits(JoinRuleSummary::KnockRestricted(
        RestrictedSummary::new(vec![space_id.clone()])
    )));

    // Restricted to somebody else's space. Offering a join here would produce a
    // server refusal on a row that promised to work.
    assert!(!admits(JoinRuleSummary::Restricted(
        RestrictedSummary::new(vec![other_space])
    )));
    assert!(!admits(JoinRuleSummary::Restricted(
        RestrictedSummary::new(Vec::new())
    )));
    // Both need a decision from somebody else, and Mesh has no room-level
    // request-to-join surface, so neither becomes a Join button.
    assert!(!admits(JoinRuleSummary::Invite));
    assert!(!admits(JoinRuleSummary::Knock));
    assert!(!admits(JoinRuleSummary::Private));
}

#[test]
fn an_unjoined_room_reports_only_what_its_summary_carries() {
    let room_id = RoomId::parse("!room:example.org").unwrap();
    let mut summary = room_summary(JoinRuleSummary::Public);
    summary.name = Some("raid-planning".into());
    summary.topic = Some("Route notes.".into());

    let channel = MatrixBackend::unjoined_channel_dto("!space:example.org", &room_id, &summary);

    assert_eq!(channel.name, "raid-planning");
    assert_eq!(channel.topic, "Route notes.");
    assert_eq!(channel.channel_type, "text");
    assert_eq!(channel.community_id, "!space:example.org");
    // There is no membership to count unread messages against, so zero is a
    // fact here rather than a placeholder for a number Mesh could not read.
    assert_eq!(channel.unread_count, 0);
    assert!(!channel.joined);
}

#[test]
fn an_unjoined_voice_room_keeps_its_kind() {
    let room_id = RoomId::parse("!room:example.org").unwrap();
    let mut summary = room_summary(JoinRuleSummary::Public);
    summary.room_type = Some("org.mesh.voice".into());
...
#[test]
fn an_unnamed_unjoined_room_falls_back_the_same_way_a_joined_one_does() {
    let room_id = RoomId::parse("!room:example.org").unwrap();
    let channel = MatrixBackend::unjoined_channel_dto(
        "!space:example.org",
        &room_id,
        &room_summary(JoinRuleSummary::Public),
    );
...
#[test]
fn joining_a_community_room_is_gated_on_the_room_being_a_child_of_that_community() {
    let matrix_source = MATRIX_PRODUCTION_SOURCES
        .iter()
        .find(|(candidate, _)| *candidate == "matrix.rs")
        .map(|(_, source)| *source)
        .unwrap();
    let join = matrix_source
        .split("async fn join_community_channel(")
        .nth(1)
        .unwrap()
        .split("async fn update_profile_avatar(")
        .next()
        .unwrap();

    // The slice really is one function. Without this the split could silently
    // take in the rest of the file and every assertion below would pass on
    // somebody else's code.
    assert!(!join.contains("async fn update_channel("));

    // Without the child check this would be a join-any-room-by-id command
    // wearing a community's name.
    assert!(join.contains("space_has_child"));
    // The joined room is held to the same encryption rule as every other room
    // in Mesh, and a rejection un-joins rather than stranding somebody inside a
    // room Mesh will then refuse to open.
    assert!(join.contains("require_protected_room"));
    assert!(join.contains("BackendError::NotEncrypted(_)"));
    assert!(join.contains("prior_membership_known_absent"));
    assert!(join.contains("room.leave()"));
}
...
#[test]
fn search_filter_absent_matches_every_message() {
    let message = search_fixture("$m", "2026-08-06T00:00:00Z");
    let filters = ParsedSearchFilters::from_raw(&MessageSearchFilters::default());
...
#[test]
fn search_terms_and_together_regardless_of_order_or_adjacency() {
    let content = "impacts of the launch: release notes here".to_lowercase();
    // Two independent words, neither adjacent to the other in the source
    // text nor in the order they were typed: AND semantics still matches.
    assert!(message_matches_terms(
        &content,
        &["release".to_string(), "launch".to_string()]
    ));
    // A term absent from the content fails the match even when every other
    // term is present.
    assert!(!message_matches_terms(
        &content,
        &["launch".to_string(), "absent".to_string()]
    ));
}

#[test]
fn search_terms_phrase_requires_contiguous_match() {
    let content = "impacts of the launch: release notes here".to_lowercase();
    // The phrase's own two words are contiguous in the source text, so it
    // matches as a single term.
    assert!(message_matches_terms(
        &content,
        &["release notes".to_string()]
    ));
    // The same two words are not adjacent in this reordering, so the phrase
    // term must fail even though "release" and "notes" each occur.
    let reordered = "notes about the plan, then a release".to_lowercase();
    assert!(!message_matches_terms(
        &reordered,
        &["release notes".to_string()]
    ));
...
#[test]
fn search_terms_empty_matches_every_message() {
    assert!(message_matches_terms("anything at all", &[]));
}

#[test]
fn normalize_search_terms_trims_lowercases_and_drops_empties() {
    ...
    assert_eq!(
        terms,
        vec!["launch".to_string(), "release notes".to_string()]
    );
}

#[test]
fn normalize_search_terms_rejects_too_many_terms() {
    let terms = vec!["a".to_string(); MAX_SEARCH_TERMS + 1];
    assert!(matches!(
        normalize_search_terms(terms),
        Err(BackendError::InvalidConfiguration(_))
    ));
}

#[test]
fn normalize_search_terms_rejects_terms_exceeding_the_byte_budget_after_normalization() {
    let terms = vec!["x".repeat(MAX_SEARCH_QUERY_BYTES + 1)];
    assert!(matches!(
        normalize_search_terms(terms),
        Err(BackendError::InvalidConfiguration(_))
    ));
...
#[test]
fn communities_fully_scanned_counts_only_communities_the_scan_passed_completely() {
    let counts = vec![
        ("community-a".to_string(), 2usize),
        ("community-b".to_string(), 3usize),
        ("community-c".to_string(), 1usize),
    ];
    // Exactly a's rooms: a counts, b and c do not.
    assert_eq!(communities_fully_scanned(&counts, 2), 1);
    // Partway into b's rooms: still only a counts.
    assert_eq!(communities_fully_scanned(&counts, 4), 1);
    // Every room of every community.
    assert_eq!(communities_fully_scanned(&counts, 6), 3);
    // Past the last room (should not happen, but must not overcount).
    assert_eq!(communities_fully_scanned(&counts, 100), 3);
}

#[test]
fn cross_community_search_truncated_when_the_community_cap_drops_communities() {
    // 250 joined communities, capped to MAX_SEARCH_COMMUNITIES before the
    // walk starts, but every capped community's rooms were fully scanned.
    // Reporting this as complete would be exactly the silent-cap dishonesty
    // the scope report exists to avoid.
    assert!(cross_community_search_truncated(true, 10, 10, 5, 5));
}

#[test]
fn cross_community_search_truncated_when_a_communitys_rooms_could_not_be_resolved() {
    assert!(cross_community_search_truncated(false, 10, 10, 4, 5));
}

#[test]
fn cross_community_search_truncated_when_the_room_scan_stopped_early() {
    assert!(cross_community_search_truncated(false, 8, 10, 5, 5));
}

#[test]
fn cross_community_search_not_truncated_when_everything_was_fully_covered() {
    assert!(!cross_community_search_truncated(false, 10, 10, 5, 5));
}

#[test]
fn search_filter_from_matches_name_or_id_and_me() {
    // search_fixture author is "Alice" / "@alice:example.org".
    let message = search_fixture("$m", "2026-08-06T00:00:00Z");
    let by_name = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        from: Some("ALI".into()),
        ..Default::default()
    });
    assert!(message_matches_filters(
    ...
    let miss = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        from: Some("bob".into()),
        ..Default::default()
    });
    assert!(!message_matches_filters(
    ...
    let from_me = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        from: Some("me".into()),
        ..Default::default()
    });
    assert!(message_matches_filters(
...
#[test]
fn search_filter_mentions_matches_id_substring_and_me() {
    let mut message = search_fixture("$m", "2026-08-06T00:00:00Z");
    message.mentions = vec!["@bob:example.org".into(), "@carol:example.org".into()];

    let by_id = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        mentions: Some("CAROL".into()),
        ..Default::default()
    });
    assert!(message_matches_filters(
        &message,
    ...
    let mentions_me = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        mentions: Some("me".into()),
        ..Default::default()
    });
    assert!(message_matches_filters(
        &message,
...
#[test]
fn search_filter_has_attachment_image_and_link() {
    let mut with_image = search_fixture("$img", "2026-08-06T00:00:00Z");
    with_image.attachments = vec![AttachmentDto {
        file_hash: "hash".into(),
    ...
    let text_only = search_fixture("$txt", "2026-08-06T00:00:00Z");

    let has_attachment = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        has: vec!["attachment".into()],
        ..Default::default()
    });
    assert!(message_matches_filters(
        &with_image,
    ...
    // Image is detected from the filename extension when content_type is absent.
    let has_image = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        has: vec!["image".into()],
        ..Default::default()
    });
    assert!(message_matches_filters(
    ...
    // A non-image attachment does not satisfy has:image.
    let mut with_pdf = search_fixture("$pdf", "2026-08-06T00:00:00Z");
    with_pdf.attachments = vec![AttachmentDto {
        file_hash: "hash".into(),
    ...
    let mut with_link = search_fixture("$link", "2026-08-06T00:00:00Z");
    with_link.content = "look at HTTPS://example.org/x".into();
    let has_link = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        has: vec!["link".into()],
        ..Default::default()
    });
    assert!(message_matches_filters(
...
#[test]
fn search_filter_date_window_is_half_open() {
    let message = search_fixture("$m", "2026-08-06T12:00:00Z");

    let inside = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        after_ms: Some(rfc3339_millis("2026-08-06T00:00:00Z")),
        before_ms: Some(rfc3339_millis("2026-08-07T00:00:00Z")),
        ..Default::default()
    });
    assert!(message_matches_filters(
        &message,
    ...
    // after_ms is inclusive; before_ms is exclusive.
    let at_lower = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        after_ms: Some(rfc3339_millis("2026-08-06T12:00:00Z")),
        ..Default::default()
    });
    assert!(message_matches_filters(
        &message,
    ...
    let at_upper = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        before_ms: Some(rfc3339_millis("2026-08-06T12:00:00Z")),
        ..Default::default()
    });
    assert!(!message_matches_filters(
        &message,
    ...
    let after_window = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        after_ms: Some(rfc3339_millis("2026-08-07T00:00:00Z")),
        ..Default::default()
    });
    assert!(!message_matches_filters(
        &message,
...
#[test]
fn search_filter_unparseable_timestamp_excluded_only_under_a_date_filter() {
    let mut message = search_fixture("$m", "2026-08-06T00:00:00Z");
    message.timestamp = "not-a-timestamp".into();

    let with_date = ParsedSearchFilters::from_raw(&MessageSearchFilters {
        after_ms: Some(0),
        ..Default::default()
    });
    assert!(!message_matches_filters(
    ...
    // Without a date predicate the unparseable timestamp is irrelevant.
    let no_date = ParsedSearchFilters::from_raw(&MessageSearchFilters::default());
    assert!(message_matches_filters(
        &message,
        &no_date,
        "@self:example.org"
...
fn empty_parsed_search_filters() -> ParsedSearchFilters {
    ParsedSearchFilters {
        from: None,
        from_is_me: false,
        mentions: None,
        mentions_is_me: false,
        after_ms: None,
        before_ms: None,
...
async fn message_scan_returns_partial_results_when_a_room_fetch_outruns_the_deadline() {
    let filters = empty_parsed_search_filters();
    let cancellation = CancellationToken::new();
    let deadline = tokio::time::Instant::now() + Duration::from_millis(500);
    let fixture = search_fixture("$fast", "2026-08-02T00:03:00Z");

    let (entries, rooms_scanned) = MatrixBackend::run_bounded_message_scan(
        vec!["!fast:example.org".into(), "!slow:example.org".into()],
        |room_id| {
            let fixture = fixture.clone();
            async move {
                // The second room never answers before the deadline. With the
                // clock paused, tokio advances virtual time to the timeout.
                if room_id == "!slow:example.org" {
                    tokio::time::sleep(Duration::from_secs(3_600)).await;
                }
                Ok::<Vec<MessageDto>, BackendError>(vec![fixture])
            }
        },
        &["matching".to_string()],
        &filters,
    ...
    .await
    .expect("a slow room must not fail the whole search");
    ...
    assert_eq!(
        entries
            ...
            .collect::<Vec<_>>(),
        ["$fast"],
        "the fast room's match survives; the slow room is abandoned at the deadline",
    );
    assert_eq!(
        rooms_scanned, 1,
        "only the fast room was reached before the deadline abandoned the slow one",
    );
}

#[tokio::test(start_paused = true)]
async fn message_scan_reports_cancellation_when_superseded() {
    let filters = empty_parsed_search_filters();
    let cancellation = CancellationToken::new();
    cancellation.cancel();
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    let fixture = search_fixture("$x", "2026-08-02T00:03:00Z");

    let result = MatrixBackend::run_bounded_message_scan(
        vec!["!room:example.org".into()],
        |_room_id| {
            let fixture = fixture.clone();
            async move { Ok::<Vec<MessageDto>, BackendError>(vec![fixture]) }
        },
        &["matching".to_string()],
        &filters,
        "@me:example.org",
    ...
    assert!(
        matches!(result, Err(BackendError::Cancelled(_))),
        "a superseded search discards its partial results and reports cancellation",
    );
}

#[tokio::test(start_paused = true)]
async fn message_scan_returns_every_match_within_the_deadline() {
    let filters = empty_parsed_search_filters();
    let cancellation = CancellationToken::new();
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    let first = search_fixture("$a", "2026-08-02T00:03:00Z");
    let second = search_fixture("$b", "2026-08-02T00:02:00Z");

    let (entries, rooms_scanned) = MatrixBackend::run_bounded_message_scan(
        vec!["!room-a:example.org".into(), "!room-b:example.org".into()],
        |room_id| {
            let (first, second) = (first.clone(), second.clone());
            async move {
                let message = if room_id == "!room-a:example.org" {
                    first
                } else {
                    second
                };
                Ok::<Vec<MessageDto>, BackendError>(vec![message])
            }
        },
        &["matching".to_string()],
        &filters,
    ...
    .await
    .expect("neither the deadline nor cancellation should fire");
    assert_eq!(
        rooms_scanned, 2,
        "both rooms were reached before the deadline"
    );
...
#[test]
fn presence_cache_retains_only_bounded_online_entries() {
    let mut presence = PresenceCache::new();
    ...
    update_cached_presence(&mut presence, "@bob:example.org".into(), "online".into(), 1);
    assert_eq!(presence.len(), 1);
    assert!(presence.contains_key("@alice:example.org"));
...
#[test]
fn presence_cache_expires_entries_older_than_the_ttl() {
    let mut presence = PresenceCache::new();
    let start = Instant::now();
    update_cached_presence_at(
        &mut presence,
        "@alice:example.org".into(),
        "online".into(),
        8,
    ...
    assert!(cached_presence_is_online(
        &presence,
        "@alice:example.org",
        start
    ));

    let expired = start + CACHED_PRESENCE_TTL + Duration::from_secs(1);
    assert!(!cached_presence_is_online(
        &presence,
        "@alice:example.org",
        expired
    ));
    prune_expired_presence(&mut presence, expired);
    assert!(presence.is_empty());
}

#[test]
fn presence_cache_reclaims_expired_entries_before_rejecting_new_accounts() {
    let mut presence = PresenceCache::new();
    let start = Instant::now();
    update_cached_presence_at(
        &mut presence,
    ...
    // Still fresh: the bound holds and the newcomer is dropped.
    update_cached_presence_at(
        &mut presence,
    ...
    assert!(presence.contains_key("@alice:example.org"));
    assert!(!presence.contains_key("@bob:example.org"));

    // Expired: the stale entry is swept so the bound is not a permanent lockout.
    let later = start + CACHED_PRESENCE_TTL + Duration::from_secs(1);
    update_cached_presence_at(
        &mut presence,
        "@bob:example.org".into(),
        "online".into(),
...
#[test]
fn presence_cache_bound_is_sized_for_a_desktop_client() {
    const {
        assert!(
            MAX_CACHED_ONLINE_PRESENCE <= 4_096,
            "the presence cache is a desktop client cache, not a server directory"
        )
    };
    assert!(CACHED_PRESENCE_TTL <= Duration::from_secs(15 * 60));
}

#[test]
fn empty_typing_updates_release_room_cache_entries() {
    let mut typing = HashMap::new();
    update_cached_typing_users(
        &mut typing,
        "!room:example.org".into(),
    ...
    update_cached_typing_users(&mut typing, "!room:example.org".into(), Vec::new());
    assert!(typing.is_empty());
}

#[test]
fn device_verification_admission_and_identifiers_are_bounded() {
    assert!(MatrixBackend::ensure_device_verification_capacity(
        MAX_ACTIVE_DEVICE_VERIFICATIONS - 1
    )
    .is_ok());
    assert!(matches!(
        MatrixBackend::ensure_device_verification_capacity(MAX_ACTIVE_DEVICE_VERIFICATIONS),
        Err(BackendError::Other(message)) if message.contains("Finish or cancel")
    ));
    assert!(
        MatrixBackend::validate_device_verification_id("00000000-0000-4000-8000-000000000000")
            .is_ok()
    );
    assert!(matches!(
        MatrixBackend::validate_device_verification_id("not-a-session"),
        Err(BackendError::InvalidConfiguration(_))
    ));
}

#[tokio::test]
async fn security_boundary_account_shutdown_clears_room_upgrade_trust_cache() {
    let backend = MatrixBackend::new(tempfile::tempdir().unwrap().path().to_owned());
    backend.verified_room_upgrades.write().await.insert(
        RoomId::parse("!old:example.org").unwrap(),
        RoomId::parse("!new:example.org").unwrap(),
    );

    backend.shutdown_for_test().await.unwrap();

    assert!(backend.verified_room_upgrades.read().await.is_empty());
}

// A send that failed for a reason retrying cannot change must be named, so the
// renderer can withdraw the retry instead of inviting the user to press it
// forever. Announcement-only power levels, a mute, a kick and a ban all surface
// as Forbidden and are indistinguishable to the sender, so they share one
// outcome; anything Mesh cannot name honestly stays Unknown and keeps the retry.
#[test]
fn unrecoverable_send_failures_are_named_only_when_the_reason_is_certain() {
    assert_eq!(
        MatrixBackend::classify_send_failure(Some(&ErrorKind::Forbidden)),
        MatrixQueuedMessageFailure::NotAllowed
    );
    assert_eq!(
        MatrixBackend::classify_send_failure(Some(&ErrorKind::NotFound)),
        MatrixQueuedMessageFailure::RoomUnavailable
    );
    assert_eq!(
        MatrixBackend::classify_send_failure(None),
        MatrixQueuedMessageFailure::Unknown
    );
    // Any other kind stays Unknown, which keeps the retry on offer. Guessing a
    // dead end from an error Mesh does not understand would strand a message
    // that might well have sent on the next attempt.
    assert_eq!(
        MatrixBackend::classify_send_failure(Some(&ErrorKind::Unrecognized)),
        MatrixQueuedMessageFailure::Unknown
    );
}

// The reconnecting band is driven by this window, so it wants to be as small as
// possible, but a window at or below the long-poll timeout would report a
// perfectly healthy idle client as offline between polls. Nothing enforced that
// relationship before, which is how it came to sit at three sync periods.
#[test]
fn matrix_sync_freshness_outlives_a_normal_long_poll() {
    assert!(MATRIX_SYNC_STATUS_FRESHNESS >= MATRIX_SYNC_NORMAL_TIMEOUT * 2);
    assert!(MATRIX_SYNC_STATUS_FRESHNESS <= MATRIX_SYNC_NORMAL_TIMEOUT * 3);
}

// Emoji made in Element or Cinny live under that client's own state key, so
// reading only Mesh's key showed an empty picker and read as data loss.
// Reads merge every pack in the room; writes must stay on Mesh's own key, or
// adopting Mesh would silently rewrite or delete another client's state.
#[test]
fn custom_emoji_reads_every_pack_but_writes_only_the_mesh_one() {
    let emoji_source = MATRIX_PRODUCTION_SOURCES
        .iter()
        .find(|(candidate, _)| *candidate == "emoji.rs")
        .map(|(_, source)| *source)
        .unwrap();
    let reader = emoji_source
        .split("async fn merged_custom_emoji_pack(")
        .nth(1)
        .expect("merged pack reader exists")
        .split("fn sanitize_custom_emoji(")
        .next()
        .unwrap();

    // Enumerates the whole room rather than one key, and stays bounded while
    // doing it, the same way the call roster does.
    assert!(reader.contains("get_state_events_static::<RoomImagePackEventContent>"));
    assert!(reader.contains("MAX_CUSTOM_EMOJI_PACKS"));
    assert!(reader.contains("MAX_CUSTOM_EMOJI_COUNT"));
    // A foreign entry goes through the same projection checks as an upload.
    assert!(reader.contains("Self::custom_emoji_info(shortcode, image).is_ok()"));
    // Reading must never write.
    assert!(!reader.contains("send_state_event"));
    ...
    // Existence is not use: assert both readers actually call the merged pack,
    // or a later edit could point them back at the single-key read and this
    // test would still pass while the picker went empty again.
    for (reader_name, next_fn) in [
        (
        ...
        (
            ...
            "async fn matrix_rtc_join(",
        ),
    ] {
        let body = matrix_source
            .split(reader_name)
            .nth(1)
            .unwrap_or_else(|| panic!("{reader_name} exists"))
            .split(next_fn)
            .next()
            .unwrap();
        assert!(
            body.contains("Self::merged_custom_emoji_pack(&space)"),
            "{reader_name} must read every pack in the room"
        );
        assert!(
            !body.contains("Self::custom_emoji_pack(&space)"),
            "{reader_name} must not fall back to the single-key read"
        );
    }

    // Every emoji state write is keyed to Mesh's own pack, never to a key
    // discovered by reading the room.
    for write in matrix_source.match_indices(".send_state_event_for_key(") {
        let tail = &matrix_source[write.0..];
        if !tail.contains("CUSTOM_EMOJI_PACK_STATE_KEY") {
            continue;
        }
        let call = &tail[..tail.find(')').unwrap_or(tail.len())];
        assert!(
            !call.contains("state_key") || call.contains("CUSTOM_EMOJI_PACK_STATE_KEY"),
            "an emoji write must target Mesh's own pack key"
        );
    }
}

#[test]
fn only_a_definite_answer_refuses_an_address() {
    use matrix_sdk::ruma::api::error::ErrorKind;

    // The address belongs to nobody: refuse, so a typo cannot become a
    // conversation that sits in the sidebar looking delivered forever.
    assert!(MatrixBackend::profile_lookup_proves_absence(Some(
        &ErrorKind::NotFound
    )));

    // Everything else is the service declining to answer, not answering "no".
    // Forbidden is the one that matters most: a homeserver may refuse profile
    // lookups over federation as policy, and reading that as absence would make
    // every account on such a service unmessageable.
    assert!(!MatrixBackend::profile_lookup_proves_absence(Some(
        &ErrorKind::Forbidden
    )));
    assert!(!MatrixBackend::profile_lookup_proves_absence(Some(
    ...
    assert!(!MatrixBackend::profile_lookup_proves_absence(None));
}

#[test]
fn a_media_transfer_may_not_consume_more_than_it_reserved() {
    // The byte budget is only a budget if the reservation bounds the transfer.
    // It did not: the reservation came from the sender's declared size while
    // the download was capped at the global maximum, so declaring one byte
    // reserved one byte and then buffered up to MAX_ATTACHMENT_BYTES. Three
    // concurrent slots put the real ceiling near 300 MiB against a 200 MiB
    // budget.
    assert_eq!(MatrixBackend::media_reservation_bytes(4_096), 4_096);
    // No declared size cannot mean no reservation.
    assert_eq!(
        MatrixBackend::media_reservation_bytes(0),
        MAX_ATTACHMENT_BYTES,
    );
    ...
    let matrix_source = MATRIX_PRODUCTION_SOURCES
        ...
        .map(|(_, source)| *source)
        .unwrap();
    // Structural, because the property is a relationship between two calls
    // rather than a value any one function returns: every download that holds
    // a reservation must pass that same reservation as its limit.
    for call in matrix_source
        .split("Self::download_bounded_encrypted_media(")
        .skip(1)
    {
        let arguments = call.split(")\n").next().unwrap_or_default();
        assert!(
            arguments.contains("reservation"),
            "a bounded media download must be limited by the bytes it reserved, \
             not by the global maximum: {arguments}"
        );
    }
}

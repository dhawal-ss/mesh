...
/// Records that the walk stopped short, keeping whichever reason came first so a
/// discovery failure is not overwritten by the size cap that follows it.
fn mark_discovery_truncated(discovery_complete: &mut bool, reason: &mut Option<String>) {
...
pub(super) async fn read_community_permission_projection(
    ...
    let subject_user_id = matrix_sdk::ruma::UserId::parse(subject_user_id)
    ...
    while let Some(room) = pending.pop_front() {
        if rooms.len() >= MAX_PERMISSION_PROJECTION_ROOMS {
            mark_discovery_truncated(&mut discovery_complete, &mut discovery_failure_reason);
            break;
        }
            ...
            match backend.space_child_ids(&room).await {
                Ok(child_ids) => {
                    for child_id in child_ids {
                        // The check at the top of the outer loop does not bound
                        // this one. A single space names any number of children
                        // and every arm below allocates -- into `pending` for a
                        // joined child, into `rooms` for one Mesh cannot open --
                        // so without a per-child budget one iteration can grow
                        // both without limit from remote state. The budget spans
                        // both collections because a room queued here becomes a
                        // row later.
                        if rooms.len() + pending.len() >= MAX_PERMISSION_PROJECTION_ROOMS {
                            mark_discovery_truncated(
                                &mut discovery_complete,
                                &mut discovery_failure_reason,
                            );
                            break;
                        }

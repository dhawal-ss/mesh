...
pub(super) const COMMUNITY_NAME_MAX_UTF16: usize = 100;
pub(super) const COMMUNITY_DESCRIPTION_MAX_UTF16: usize = 500;
pub(super) const CHANNEL_NAME_MAX_UTF16: usize = 100;
...
pub(super) fn normalize_required_metadata(
    value: String,
    label: &str,
    max_utf16: usize,
...
pub(super) fn bounded_remote_member_display_name(value: &str, fallback_user_id: &str) -> String {
...
pub(super) fn bounded_remote_application_reason(value: Option<&str>) -> Option<String> {
    let value = value?.trim();
    let mut bounded = String::new();
    let mut used = 0;
...
mod input_bounds_tests {
    ...
    /*
        A room description is optional where a name is required, and this is the
        bound `update_channel` applies to `m.room.topic`. Now that a UI finally
        writes one, an empty submission has to mean "clear it" rather than being
        refused the way an empty name is.
    */
    #[test]
    fn a_room_description_may_be_cleared_but_not_exceed_its_limit() {
        assert_eq!(
            ...
            .expect("an emptied room description should clear rather than fail"),
            ""
        );
        assert_eq!(
            normalize_optional_metadata(
                "  Reference and sketches.  ".into(),
                "Room description",
                COMMUNITY_DESCRIPTION_MAX_UTF16,
            )
            .expect("a normal room description should pass"),
        ...
        .is_err());
        // The renderer counts UTF-16 units, so the backend has to agree or a
        // description accepted in the field would be refused on the wire.
        let astral = "😀".repeat(COMMUNITY_DESCRIPTION_MAX_UTF16 / 2);
        assert_eq!(metadata_utf16_len(&astral), COMMUNITY_DESCRIPTION_MAX_UTF16);
        assert!(normalize_optional_metadata(
            format!("{astral}😀"),
            "Room description",
            COMMUNITY_DESCRIPTION_MAX_UTF16,
    ...
    fn metadata_rejects_empty_names_controls_and_bidi_overrides() {
        assert!(
            normalize_required_metadata("   ".into(), "Room name", CHANNEL_NAME_MAX_UTF16,)
                .is_err()
        );
        assert!(normalize_required_metadata(
            "safe\nunsafe".into(),

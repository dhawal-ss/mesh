...
impl MatrixBackend {
    pub(super) fn matrix_rtc_room_name(room_id: &str) -> BackendResult<String> {
    ...
    pub(super) fn matrix_rtc_participant_identity(
        user_id: &str,
        device_id: &str,
        member_id: &str,
    ...
    pub(super) fn matrix_rtc_monotonic_now_ms() -> u64 {
    ...
    pub(super) fn decode_matrix_rtc_media_key(
        encoded: &str,
    ...
    pub(super) fn validate_matrix_rtc_media_key(
    ...
    pub(super) fn record_matrix_rtc_key_attempt(
        runtime: &mut MatrixRtcMediaKeyRuntime,
        sender: &str,
        device_id: &str,
        now_ms: u64,
    ...
    pub(super) fn matrix_rtc_key_recipients(
        memberships: &[ActiveMatrixRtcMembership],
        own_user_id: &str,
        own_device_id: &str,
    ...
    pub(super) fn matrix_rtc_membership_content(
        device_id: &str,
        member_id: &str,
    ...
    pub(super) fn validate_matrix_rtc_sfu_url(
        returned: &str,
        expected: Option<&str>,
        tauri_config: &str,
    ) -> BackendResult<String> {
    ...
    pub(super) fn classify_matrix_rtc_endpoint_failure(
    ...
    pub(super) fn parse_matrix_rtc_transports(
    ...
    pub(super) async fn discover_matrix_rtc_service_url(
        client: &Client,
    ) -> BackendResult<MatrixRtcDiscovery> {
    ...
    pub(super) fn matrix_rtc_config(
        discovered_service_url: String,
    ) -> BackendResult<VoiceServiceStatus> {
    ...
    pub(super) fn require_matrix_rtc_media_e2ee_ready() -> BackendResult<()> {
    ...
    pub(super) async fn active_matrix_rtc_memberships(
        room: &Room,
        ...
        for raw in room_state {
            ...
            let event = raw.deserialize().map_err(Self::map_display_error)?;
                ...
                .active_memberships(Some(event.origin_server_ts))
            {
                ...
                let participant_identity = Self::matrix_rtc_participant_identity(
                    event.sender.as_str(),
                    membership.device_id().as_str(),
                    &member_id,
                )?;
    ...
    pub(super) async fn matrix_rtc_members_for_room(
        room: &Room,
    ...
    pub(super) fn matrix_rtc_recipient_fingerprint(
    ...
    async fn distribute_matrix_rtc_media_key(
        ...
        for recipient in recipients {
            ...
            let user_id = matrix_sdk::ruma::UserId::parse(&recipient.user_id)
    ...
    pub(super) async fn create_initial_matrix_rtc_media_key(
        client: &Client,
        session: &MatrixRtcLocalSession,
        memberships: &[ActiveMatrixRtcMembership],
    ...
    fn spawn_matrix_rtc_activation_timeout(
        ...
        tokio::spawn(async move {
            ...
            let cleared = if let Some(session) = session {
                ...
                Self::clear_matrix_rtc_membership_or_warn(&session, &sessions, &writes).await
            } else {
                false
            };
            {
    ...
    pub(super) async fn fail_closed_matrix_rtc_room(
        room_id: &str,
        sessions: &Arc<Mutex<HashMap<(String, String), MatrixRtcLocalSession>>>,
        runtime: &Arc<Mutex<MatrixRtcMediaKeyRuntime>>,
        writes: &Arc<Mutex<()>>,
    ...
    pub(super) async fn sync_matrix_rtc_media_keys_for_room(
        room: &Room,
        sessions: &Arc<Mutex<HashMap<(String, String), MatrixRtcLocalSession>>>,
        runtime: &Arc<Mutex<MatrixRtcMediaKeyRuntime>>,
        writes: &Arc<Mutex<()>>,
    ...
    pub(super) fn matrix_rtc_pending_activation_snapshot(
    ...
    pub(super) async fn ack_matrix_rtc_media_key_pause_inner(
        &self,
        room_id: &str,
        session_id: &str,
        member_id: &str,
        ...
        let now = Self::matrix_rtc_monotonic_now_ms();
        {
            ...
            let pending = runtime
                ...
                .ok_or_else(|| {
                    BackendError::PermissionDenied(
                        "MatrixRTC media-key activation was removed before it completed".into(),
                    )
                })?;
    ...
    pub(super) async fn ack_matrix_rtc_media_key_inner(
        &self,
        room_id: &str,
        session_id: &str,
        member_id: &str,
    ...
    pub(super) fn revoke_matrix_rtc_publication(
        runtime: &mut MatrixRtcMediaKeyRuntime,
    ...
    pub(super) fn matrix_rtc_local_lease_state(
        runtime: &mut MatrixRtcMediaKeyRuntime,
        state_key: &(String, String),
        now: u64,
    ...
    pub(super) async fn renew_matrix_rtc_media_key_lease_inner(
        &self,
        room_id: &str,
        session_id: &str,
        member_id: &str,
        ...
        let key_index = match lease_state {
            ...
            MatrixRtcLocalLeaseState::Expired => {
                ...
                .await;
                let cleared = Self::clear_matrix_rtc_membership_or_warn(
                    &session,
                    &self.rtc_sessions,
    ...
    pub(super) async fn handle_matrix_rtc_media_key_event(
        ...
        let room_id = matrix_sdk::ruma::RoomId::parse(&envelope.content.room_id)
            .map_err(Self::map_display_error)?;
        let room =
    ...
    pub(super) async fn emit_matrix_rtc_membership(
        room: &Room,
        callback: &Arc<StdRwLock<Option<MatrixBackendEventCallback>>>,
    ) {
    ...
    pub(super) async fn select_matrix_rtc_service_url(
        room: &Room,
    ...
    pub(super) fn select_matrix_rtc_service_from_memberships(
        memberships: &[ActiveMatrixRtcMembership],
        local_service_url: &str,
        tauri_config: &str,
    ) -> BackendResult<String> {
        ...
        if !VoiceServiceStatus::connect_src_allows(tauri_config, &[&selected_origin]) {
            return Err(BackendError::InvalidConfiguration(
    ...
    async fn publish_matrix_rtc_membership(
        ...
        CallMemberStateKey::from_str(&session.state_key).map_err(Self::map_display_error)?;
    ...
    pub(super) async fn publish_current_matrix_rtc_membership(
        session: &MatrixRtcLocalSession,
        sessions: &Arc<Mutex<HashMap<(String, String), MatrixRtcLocalSession>>>,
        writes: &Arc<Mutex<()>>,
        active: bool,
    ...
    /// Retracts the local membership, recording why when it could not be done.
    ///
    /// A failed retraction leaves the lease blocked, which is the right answer:
    /// Mesh must not resume publishing media it may still be advertising. What
    /// was wrong was doing it silently. `unwrap_or(false)` discarded the reason,
    /// so the microphone stopped and nothing anywhere could say why, on a state
    /// the user cannot clear from the interface. The warning matches the one the
    /// publish path already emits for this same call.
    async fn clear_matrix_rtc_membership_or_warn(
        session: &MatrixRtcLocalSession,
        sessions: &Arc<Mutex<HashMap<(String, String), MatrixRtcLocalSession>>>,
        writes: &Arc<Mutex<()>>,
    ) -> bool {
        match Self::clear_current_matrix_rtc_membership(session, sessions, writes).await {
            Ok(cleared) => cleared,
            Err(error) => {
                tracing::warn!(
                    target: "mesh::matrixrtc",
                    ...
                    "Could not retract a MatrixRTC membership; the local lease stays paused: {error}"
                );
                false
            }
        }
    ...
    pub(super) async fn clear_current_matrix_rtc_membership(
        session: &MatrixRtcLocalSession,
        sessions: &Arc<Mutex<HashMap<(String, String), MatrixRtcLocalSession>>>,
        writes: &Arc<Mutex<()>>,
    ) -> BackendResult<bool> {
    ...
    pub(super) fn spawn_matrix_rtc_membership_refresh(
        session: MatrixRtcLocalSession,
        sessions: Arc<Mutex<HashMap<(String, String), MatrixRtcLocalSession>>>,
        writes: Arc<Mutex<()>>,
        callback: Arc<StdRwLock<Option<MatrixBackendEventCallback>>>,
    ...
    pub(super) async fn exchange_matrix_rtc_token(
        client: &Client,
        room_id: &str,
        member_id: &str,
        user_id: &OwnedUserId,

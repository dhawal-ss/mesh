...
pub(super) fn normalized_member_page_limit(limit: Option<u32>) -> usize {
    limit
        .unwrap_or(DEFAULT_MEMBER_PAGE_LIMIT as u32)
        .clamp(1, MAX_MEMBER_PAGE_LIMIT as u32) as usize
}

pub(super) fn normalize_member_page_cursor(
...
pub(super) fn bounded_member_page<T>(
...
mod member_page_tests {
    ...
    fn member_pages_are_stable_non_overlapping_and_bounded() {
        ...
        let (first, first_cursor) =
            bounded_member_page(values.clone(), None, 100, |value| format!("{value:05}"));
        assert_eq!(first, (0..100).collect::<Vec<_>>());
        assert_eq!(first_cursor.as_deref(), Some("00099"));

        let (second, second_cursor) =
            bounded_member_page(values, first_cursor.as_deref(), 100, |value| {
                format!("{value:05}")
            });
    ...
    fn final_member_page_has_no_cursor() {
        let (page, cursor) =
            bounded_member_page([3, 1, 2], Some("1"), 10, |value| value.to_string());

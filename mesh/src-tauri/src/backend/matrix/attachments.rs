...
impl MatrixBackend {
    pub(super) fn encrypted_file_sha256(encrypted_file: &EncryptedFile) -> Option<String> {
    ...
    pub(super) fn resolved_matrix_attachment_from_content(
        content: &serde_json::Value,
    ...
    pub(super) fn matrix_attachment_from_content(
        content: &serde_json::Value,
    ...
    #[cfg(test)]
    pub(super) fn matrix_attachment_from_event(
        event: &serde_json::Value,
        attachment_index: u32,
    ...
    pub(super) async fn resolve_protected_attachment(
        client: &Client,
        ...
        let value: serde_json::Value =
            serde_json::from_str(event.raw().json().get()).map_err(Self::map_display_error)?;
    ...
    pub(super) fn safe_media_filename(filename: &str) -> BackendResult<String> {
    ...
    pub(super) fn validate_transfer_id(transfer_id: &str) -> BackendResult<()> {
    ...
    pub(super) fn validate_transaction_id(
        transaction_id: &str,
    ) -> BackendResult<OwnedTransactionId> {
    ...
    pub(super) fn new_message_composer_draft(
        draft: ComposerDraftDto,
    ) -> BackendResult<Option<ComposerDraft>> {
        if draft.body.len() > MAX_COMPOSER_DRAFT_BYTES {
            return Err(BackendError::InvalidConfiguration(
        ...
        if draft
            .formatted_body
            .as_ref()
            .is_some_and(|formatted| formatted.len() > MAX_COMPOSER_DRAFT_FORMATTED_BYTES)
        {
            return Err(BackendError::InvalidConfiguration(
                "formatted message draft cannot exceed 64 KiB".into(),
            ));
        }
        ...
        Ok(Some(ComposerDraft {
            ...
            html_text: draft.formatted_body,
            draft_type: ComposerDraftType::NewMessage,
            attachments: Vec::new(),
        }))
    }

    pub(super) fn new_message_composer_draft_body(
        draft: ComposerDraft,
        ...
        {
            return Err(BackendError::InvalidConfiguration(
                "saved formatted message draft exceeds 64 KiB".into(),
            ));
        }
        Ok((!plain_text.is_empty()).then_some(ComposerDraftDto {
            body: plain_text,
    ...
    pub(super) fn validate_media_payload(
        data: &[u8],
    ...
    pub(super) fn validate_attachment_size(size: u64) -> BackendResult<()> {
    ...
    pub(super) fn thumbnail_image_format(content_type: &str) -> Option<image::ImageFormat> {
    ...
    pub(super) fn decode_image_with_safe_limits(
        data: &[u8],
    ...
    /// Runs an image decode off the async runtime.
    ///
    /// Decode cost is set by the dimensions an image declares, not by how many
    /// bytes it occupies, so a byte cap does not bound it -- which is exactly
    /// what a decompression bomb exploits. `decode_image_with_safe_limits`
    /// bounds pixels and allocation, but bounded is not cheap: a 25M-pixel
    /// decode still runs to completion, and on an async worker it stalls every
    /// other task sharing that thread.
    ///
    /// Every decode of remote or remote-influenced bytes goes through here so
    /// the handoff is one decision instead of one per call site. The attachment
    /// upload and lightbox paths already did this by hand; this is that pattern
    /// with a name.
    pub(super) async fn decode_off_runtime<T, F>(decode: F) -> BackendResult<T>
    where
    ...
    pub(super) fn validate_lightbox_image(data: &[u8], content_type: &str) -> BackendResult<()> {
    ...
    pub(super) fn generate_sanitized_thumbnail(
        data: &[u8],
        content_type: &str,
    ...
    pub(super) async fn upload_encrypted_media_bytes(
        client: &Client,
        data: &[u8],
    ...
    /// claims.
    pub(super) async fn collect_bounded_media(
    ...
    #[allow(deprecated)]
    pub(super) fn media_download_endpoint(
        homeserver: &str,
    ...
    pub(super) fn media_http_client() -> BackendResult<reqwest::Client> {
    ...
    pub(super) async fn download_bounded_encrypted_media(
        client: &Client,
    ...
    pub(super) async fn enforce_media_cache_quota(
        cache_root: &Path,
        protected: &Path,
    ) -> BackendResult<()> {
    ...
    pub(super) async fn enforce_media_cache_quota_with_limit(
        cache_root: &Path,
        protected: &Path,
        max_bytes: u64,
    ) -> BackendResult<()> {
        ...
        while let Some(entry) = read_dir
            ...
            .map_err(Self::map_display_error)?
        {
            let file_type = entry.file_type().await.map_err(Self::map_display_error)?;
            if !file_type.is_file() {
                continue;
            }
            let metadata = entry.metadata().await.map_err(Self::map_display_error)?;

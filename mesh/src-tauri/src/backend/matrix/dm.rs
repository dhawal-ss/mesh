...
impl MatrixBackend {
    pub(super) async fn dm_request_from_invited_room(
        room: &Room,
    ) -> BackendResult<Option<DmRequestDto>> {
        if room.state() != RoomState::Invited
            || room.is_space()
            || room.room_type().is_some()
            || !room.is_direct().await.map_err(Self::map_display_error)?
        {
            return Ok(None);
        }
        ...
        // Reported as a failure rather than collapsed into `false`, matching
        // `require_protected_room`. A read that could not complete is not the
        // same fact as a room that is not encrypted, and rendering it as one
        // greys out Accept on a legitimate invitation and blames the sender for
        // it. The caller logs this and omits the request, so it comes back on
        // the next refresh instead of standing there permanently unacceptable.
        let can_accept = room
            .latest_encryption_state()
            .await
            .map_err(|error| {
                BackendError::InvalidConfiguration(format!(
                    "Mesh could not verify end-to-end encryption on direct-message invitation {}: \
                     {error}. Resynchronize and try again",
                    room.room_id()
                ))
            })?
            .is_encrypted();
    ...
    pub(super) async fn validated_dm_request_room(
        client: &Client,
    ...
    pub(super) fn is_exact_two_party_direct_candidate(
    ...
    pub(super) async fn room_member_is_joined(
        room: &Room,
        ...
        let member_event = member_event
            .deserialize()
            .map_err(Self::map_display_error)?;
        Ok(matches!(member_event.membership(), MembershipState::Join))
    }
    ...
    pub(super) fn canonical_direct_room_id(mut room_ids: Vec<OwnedRoomId>) -> Option<OwnedRoomId> {
        room_ids.sort();
    ...
    pub(super) fn merge_direct_content_preserving_mappings(
    ...
    pub(super) fn direct_content_preserves(
    ...
    async fn reconcile_direct_duplicates(
        ...
        for _ in 0..DIRECT_ACCOUNT_DATA_MERGE_ATTEMPTS {
            ...
            let first = first_raw.deserialize().map_err(Self::map_display_error)?;
            ...
            let second = second_raw.deserialize().map_err(Self::map_display_error)?;

            let snapshots_match = serde_json::to_vec(&first).map_err(Self::map_display_error)?
                == serde_json::to_vec(&second).map_err(Self::map_display_error)?;
    ...
    pub(super) async fn is_ignored_user(
        &self,
        client: &Client,
        user_id: &matrix_sdk::ruma::UserId,
    ) -> BackendResult<bool> {
    ...
    pub(super) fn notification_sender_is_ignored(
    ...
    pub(super) fn ignored_user_change(
        ...
        let reset_all = previous.is_none() || blocked_user_ids.len() > MAX_IGNORED_USER_CHANGE_IDS;
        if reset_all {
    ...
    pub(super) fn ignored_user_list_matches(
    ...
    pub(super) fn direct_room_peer(room: &Room) -> BackendResult<Option<OwnedUserId>> {
        let targets = room.direct_targets();
        if targets.is_empty() {
            return Ok(None);
        }
    ...
    pub(super) fn room_has_ignored_direct_peer(
        room: &Room,
    ...
    pub(super) async fn reject_ignored_direct_room(
        &self,
        client: &Client,
        room: &Room,
    ) -> BackendResult<()> {
    ...
    pub(super) fn blocked_account_page_from_user_ids(
    ...
    pub(super) async fn fetch_ignored_user_list(
        client: &Client,
        ...
        client
            ...
            .await
            .map_err(Self::map_error)?
            .map(|raw| raw.deserialize().map_err(Self::map_display_error))
            .transpose()
    ...
    pub(super) async fn stored_ignored_user_list(
        client: &Client,
    ...
    pub(super) async fn ignored_user_list_with_offline_cache(
        &self,
        client: &Client,
    ) -> BackendResult<IgnoredUserListEventContent> {
    ...
    pub(super) async fn refresh_ignored_user_list(
        &self,
        client: &Client,
    ) -> BackendResult<IgnoredUserListEventContent> {
    ...
    pub(super) async fn set_ignored_user(
        &self,
        client: &Client,
    ...
    /// Refuse to open a direct room with an address no account service knows.
    ///
    /// `create_dm` will cheerfully invite an address nobody owns: the room is
    /// created, the invitation never resolves, and the conversation sits in the
    /// sidebar looking exactly like one that was delivered. That was
    /// unreachable while every recipient came from a roster the person had
    /// already joined. It is one typo away as soon as an address can be typed.
    ///
    /// Only a definite answer refuses. A homeserver may decline profile lookups
    /// over federation as a matter of policy, and a remote service may simply
    /// be unreachable for a moment; neither is evidence that a person does not
    /// exist, and treating them as such would make entire services
    /// unmessageable. So this proves absence or it stands aside.
    async fn require_known_account(
        client: &Client,
        user_id: &matrix_sdk::ruma::UserId,
    ) -> BackendResult<()> {
        match client.account().fetch_user_profile_of(user_id).await {
            Err(error) if Self::profile_lookup_proves_absence(error.client_api_error_kind()) => {
                Err(BackendError::AccountNotFound(user_id.to_string()))
            }
            _ => Ok(()),
        }
    }

    /// Whether a failed profile lookup is proof that nobody owns the address.
    ///
    /// Split out from the lookup so the rule can be tested without a homeserver,
    /// because it is the whole safety property and it is one tidying edit away
    /// from becoming "any failure refuses". `M_FORBIDDEN` in particular is
    /// routine: a homeserver may decline profile lookups over federation as
    /// policy, and reading that as absence would make every account on such a
    /// service unmessageable.
    pub(super) fn profile_lookup_proves_absence(kind: Option<&ErrorKind>) -> bool {
        matches!(kind, Some(ErrorKind::NotFound))
    }

    pub(super) async fn direct_room(
        &self,
        client: &Client,
        user_id: &matrix_sdk::ruma::UserId,
    ) -> BackendResult<Room> {
        ...
        let room = if let Some(canonical_room_id) = canonical_room_id {
            ...
            // Nothing exists yet, so this call is about to invite somebody.
            // Check they are somebody first.
            Self::require_known_account(client, user_id).await?;
    ...
    pub(super) fn direct_message_from_message(message: MessageDto) -> DirectMessageDto {
        DirectMessageDto {
            id: message.id,
    ...
    pub(super) async fn send_immediate_protected_message(
        client: &Client,
        room: &Room,
        body: String,
        ...
        Ok(MessageDto {
            ...
            undecryptable: None,
            mentions: Vec::new(),
            // m.mentions.room names a room's members; a direct room has two.
            mentions_room: false,
        })
    }
}

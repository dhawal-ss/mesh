...
pub(super) const MAX_PERSONAL_EXPORT_JSON_BYTES: u64 = 256 * 1024 * 1024;
...
pub(super) enum PersonalExportScanReservation {
    Reserved,
    RoomLimit,
    GlobalLimit,
}
...
#[derive(Default)]
pub(super) struct PersonalDataExportBudget {
    pub(super) scanned_events: usize,
    pub(super) scanned_event_bytes: u64,
    pub(super) exported_messages: usize,
    pub(super) serialized_bytes: u64,
    pub(super) media_files: usize,
    pub(super) media_bytes: u64,
}

impl MatrixBackend {
    pub(super) fn reserve_personal_export_scan_bytes(
    ...
    pub(super) fn reserve_personal_export_room_bytes(
    ...
    pub(super) fn retain_newest_personal_export_messages(
        messages: Vec<MessageDto>,
        limit: usize,
    ) -> (Vec<MessageDto>, bool) {
    ...
    pub(super) fn personal_export_event_is_relevant(
    ...
    pub(super) fn personal_export_cache_prefix(file_hash: &str) -> String {
    ...
    async fn project_personal_export_room(
        ...
        let (values, truncated, malformed) =
            Self::personal_export_room_values(room, own_user_id.as_str(), cancellation, budget)
                .await?;
    ...
    #[cfg(test)]
    pub(super) async fn copy_personal_export_media(
        cache_root: &Path,
        export_root: &Path,
    ...
    async fn copy_personal_export_media_bounded(
        ...
        while let Some(entry) = source_entries
            .next_entry()
            .await
            .map_err(Self::map_display_error)?
        {
    ...
    pub(super) async fn write_personal_export_chunk(
        ...
        tokio::select! {
            ...
            result = file.write_all(bytes) => result.map_err(Self::map_display_error)?,
        }
        budget.serialized_bytes = next_size;
        Ok(())
    }

    pub(super) async fn write_personal_data_export(
        &self,
        ...
        let export_result: BackendResult<(u32, u64, u32, Vec<String>)> = async {
            ...
            Self::write_personal_export_chunk(
                &mut file,
                &serde_json::to_vec(&exported_at.to_rfc3339()).map_err(Self::map_display_error)?,
                cancellation,
                &mut budget,
            )
            .await?;
            ...
            for (prefix, value) in [
                (b",\n\"account\":".as_slice(), serde_json::to_vec(&account).map_err(Self::map_display_error)?),
                (b",\n\"scope\":".as_slice(), serde_json::to_vec(&scope).map_err(Self::map_display_error)?),
            ] {
            ...
            for room in rooms {
                ...
                let room_json = serde_json::to_vec(&projected).map_err(Self::map_display_error)?;
                if room_count > 0 {
                    Self::write_personal_export_chunk(
                        &mut file,
                        b",",
            ...
            Self::write_personal_export_chunk(
                &mut file,
                &serde_json::to_vec(&warnings).map_err(Self::map_display_error)?,
                cancellation,
                &mut budget,
            )
            .await?;
            ...
            .await?;
            file.sync_all().await.map_err(Self::map_display_error)?;
            drop(file);
            if cancellation.is_cancelled() {
                return Err(BackendError::Cancelled(
                    "personal-data export cancelled".into(),

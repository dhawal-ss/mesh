...
impl MatrixModerationAction {
    ...
    fn permission(&self) -> CommunityPermission {
        match self {
            Self::Ban | Self::Unban | Self::Kick => CommunityPermission::Admin,
    ...
    fn role_power_level(&self) -> Option<matrix_sdk::ruma::Int> {
        match self {
            ...
            Self::Ban | Self::Unban | Self::Kick => None,
        }
    }
    ...
    async fn apply(
        ...
        match self {
            ...
            // Lifting a ban restores the ability to rejoin. It deliberately does
            // not re-add the account to any room: the person chooses to return.
            // A channel created after the ban never carried it, so unbanning
            // there would fail and report a partial outcome for a room the
            // person was never banned in. Only act where a ban actually exists.
            Self::Unban => {
                let carries_ban = room
                    .get_member_no_sync(user_id)
                    .await
                    .map_err(MatrixBackend::map_error)?
                    .is_some_and(|member| member.membership().as_str() == "ban");
                if carries_ban {
            ...
            Self::RoleAdmin | Self::RoleMember => {
                ...
                let mut power_levels = if has_power_levels_event {
                    room.power_levels()
                        .await
                        .map_err(MatrixBackend::map_display_error)?
                } else {
...
impl MatrixBackend {
    ...
    /// Why no durable moderation record was written.
    ///
    /// Not `moderation_failure_reason`: that one is worded about a channel
    /// failing to apply a change ("This channel did not allow..."), which is a
    /// different thing from the record of the change failing to be written.
    /// Reusing it would tell a moderator their action failed when it applied
    /// everywhere.
    fn audit_unrecorded_reason(error: &BackendError) -> String {
        match error {
            // Mesh's own policy sentence, not a homeserver's, so it is safe to
            // show verbatim and it is the most accurate thing available.
            BackendError::InvalidConfiguration(detail)
                if detail == MODERATION_AUDIT_UNAVAILABLE =>
            {
                detail.clone()
            }
            BackendError::Network(_) | BackendError::Cancelled(_) => {
                "The moderation record could not be reached. The change itself still applied."
                    .into()
            }
            BackendError::PermissionDenied(_) => {
                "This account may not write to the moderation record.".into()
            }
            _ => "Mesh could not write a moderation record for this change.".into(),
        }
    }
    ...
    pub(super) async fn apply_community_moderation(
        ...
        let actor_user_id = self
            ...
            .await?;
        let target_user_id =
            matrix_sdk::ruma::UserId::parse(target_user_id).map_err(Self::map_display_error)?;
        ...
        let audit_unrecorded_reason = Self::record_moderation_audit(&space, &audit)
            .await
            .err()
            .map(|error| Self::audit_unrecorded_reason(&error));
        Ok(CommunityModerationResult {
            audit,
            audit_recorded: audit_unrecorded_reason.is_none(),
            audit_unrecorded_reason,
        })
    }

...
use matrix_sdk::{
    ...
    room::{
        ...
        ListThreadsOptions, MessagesOptions, ParentSpace, Receipts, RelationsOptions,
        RoomMemberRole,
    },
    ruma::{
        api::{
        ...
        events::{
            ...
            room::{
                ...
                member::{
                    MembershipState, RoomMemberEvent, RoomMemberEventContent,
...
use crate::types::{
    ...
    message::{
        ...
        MatrixThreadListDto, MessageDto, MessageSearchFilters, ThreadListItemDto,
...
use super::{
    ...
    DirectoryPeopleResult, DirectoryPerson, EntityList, MatrixAccount, MatrixAttachmentSendRequest,
    MatrixBackendEvent, MatrixBackendEventCallback, MatrixDevice, MatrixIgnoredUsersChanged,
    ...
    MatrixQueuedMessageFailure, MatrixQueuedMessageState, MatrixQueuedMessageUpdate,
    ...
    MatrixRoomNotificationMode, MatrixRoomPins, MatrixRoomPinsUpdate, MatrixRoomUpdateKind,
    MatrixRoomUpgrade, MatrixRtcJoinResult, MatrixRtcMediaKey, MatrixRtcMediaKeyFailure,
    ...
    MatrixTransferRetryMode, MatrixTransferState, MatrixTypingChanged, MatrixUnreadUpdate,
...
mod errors;
use errors::classify_matrix_error;
mod member_page;
mod messages;
use messages::{
    MAX_SEND_QUEUE_ACCOUNT_MESSAGES, MAX_SEND_QUEUE_ACCOUNT_UTF8_BYTES,
    MAX_SEND_QUEUE_ROOM_MESSAGES, MAX_SEND_QUEUE_ROOM_UTF8_BYTES,
};
mod admission;
mod attachments;
mod dm;
mod personal_data;
mod rtc;
use admission::{AdmissionCreateResponse, AdmissionServiceResponse};
mod avatar;
mod emoji;
mod encryption;
mod rooms;
use emoji::{CUSTOM_EMOJI_PACK_NAME, CUSTOM_EMOJI_PACK_STATE_KEY, MAX_CUSTOM_EMOJI_COUNT};
mod search;
use search::{insert_bounded_search_result, NativeSearchRegistry};
mod input_bounds;
use input_bounds::{
    bounded_remote_application_reason, bounded_remote_member_display_name,
    normalize_optional_metadata, normalize_required_metadata, CHANNEL_NAME_MAX_UTF16,
    COMMUNITY_DESCRIPTION_MAX_UTF16, COMMUNITY_NAME_MAX_UTF16,
};
mod moderation;
mod oidc;
use member_page::{
    bounded_member_page, normalize_member_page_cursor, normalized_member_page_limit,
};
...
// The "My threads" list only needs enough of each thread to count replies,
// participants, and unread state, not the full reply bodies thread_context
// keeps for reading.
const MAX_THREAD_LIST_ROOTS: u32 = 30;
const MAX_THREAD_LIST_RELATION_EVENTS: usize = 200;
...
/// Hard cap on the number of free-text terms (words or quoted phrases) a
/// query can carry. Each term is an independent required substring, so
/// without a cap a query built from thousands of one-byte terms could force
/// as many `contains` scans per message. Set well above anything a person
/// would plausibly type: a full paragraph pasted into search is more likely
/// to hit `MAX_SEARCH_QUERY_BYTES` first, which the renderer does not yet
/// distinguish from any other search failure (both currently surface as the
/// same generic "Search is temporarily unavailable").
const MAX_SEARCH_TERMS: usize = 200;
const MAX_SEARCH_DEADLINE_MS: u64 = 15_000;
/// A cross-community search shares one deadline and one `MessageSearchBudget`
/// across every community, so no single community's room list can be so
/// long that resolving it alone starves the ones after it.
const MAX_SEARCH_COMMUNITIES: usize = 200;
...
const MAX_ACTIVE_DEVICE_VERIFICATIONS: usize = 4;
const MAX_DEVICE_ID_BYTES: usize = 255;
/// Online-presence cache bound. This is a desktop client, not a server: the
/// only reader is the community member page, which shows at most a page of
/// members at a time. The previous 50,000-entry bound reserved several
/// megabytes of native heap that only shrank on account transition.
const MAX_CACHED_ONLINE_PRESENCE: usize = 2_048;
/// Presence older than this is not shown anywhere, so it is not retained.
const CACHED_PRESENCE_TTL: Duration = Duration::from_secs(5 * 60);
...
const MAX_COMPOSER_DRAFT_FORMATTED_BYTES: usize = 64 * 1024;
...
const MAX_COMMUNITY_INVITES: usize = 100;
...
// Twice MATRIX_SYNC_NORMAL_TIMEOUT. A healthy idle client completes a long poll
// every 30s, so the window has to clear that or a quiet connection reports
// itself offline; at the old 90s the reconnecting band physically could not
// appear until a minute and a half into an outage, which is long past the point
// the user has noticed. Two sync periods is the smallest ratio that still
// absorbs one slow response plus its retry backoff.
const MATRIX_SYNC_STATUS_FRESHNESS: Duration = Duration::from_secs(60);
...
/// A community's child rooms, with the public summary the account service sent
/// for each one in the same response.
///
/// The summaries are what let a room this account has not joined be listed at
/// all. Name, description, kind and join rule all arrive here without joining
/// anything, so the alternative to keeping them is a second request per room, or
/// pretending the room is not there.
struct SpaceChildListing {
    children: EntityList<OwnedRoomId>,
...
fn message_search_retained_bytes(message: &MessageDto) -> usize {
    ...
    let mut bytes = std::mem::size_of::<MessageDto>()
        ...
        .saturating_add(optional_string_bytes(&message.author_avatar_url))
...
/// Precomputed, normalized form of `MessageSearchFilters` for fast per-message
/// evaluation. Text values are trimmed and lowercased once; the `*_is_me`
/// sentinels are resolved against the caller's own user id.
...
impl ParsedSearchFilters {
    fn from_raw(filters: &MessageSearchFilters) -> Self {
        ...
        let from = normalize(&filters.from);
        let mentions = normalize(&filters.mentions);
        let has = |needle: &str| {
            filters
                .has
                .iter()
        ...
        Self {
            from_is_me: from.as_deref() == Some("me"),
            mentions_is_me: mentions.as_deref() == Some("me"),
            from,
            mentions,
            after_ms: filters.after_ms,
            before_ms: filters.before_ms,
            require_attachment: has("attachment") || has("file"),
            require_image: has("image"),
            require_link: has("link"),
        }
    }

    /// True when at least one structured predicate is set. Used to decide
    /// whether a filter-only search (empty text) may proceed.
    fn is_active(&self) -> bool {
        self.from.is_some()
            || self.mentions.is_some()
            || self.after_ms.is_some()
            || self.before_ms.is_some()
            || self.require_attachment
            || self.require_image
            || self.require_link
    }
}

fn attachment_is_image(attachment: &AttachmentDto) -> bool {
    if attachment
        .content_type
        .as_deref()
...
/// Evaluate the structured filter predicates against a candidate message. The
/// caller applies the free-text and `deleted_at` checks separately; this covers
/// author, mentions, attachment/image/link, and the date window. Every active
/// predicate must match. `account_id_lc` is the lowercased own user id.
fn message_matches_filters(
    ...
    if let Some(from) = &filters.from {
        let author_matches = if filters.from_is_me {
            message.author_public_key.to_lowercase() == account_id_lc
        } else {
            message.author_display_name.to_lowercase().contains(from)
                || message.author_public_key.to_lowercase().contains(from)
        };
        if !author_matches {
            return false;
        }
    }
    if let Some(mentions) = &filters.mentions {
        let mention_matches = message.mentions.iter().any(|candidate| {
            let candidate = candidate.to_lowercase();
            if filters.mentions_is_me {
                candidate == account_id_lc
            } else {
                candidate.contains(mentions)
            }
        });
        if !mention_matches {
            return false;
    ...
    if filters.require_attachment && message.attachments.is_empty() {
        return false;
    }
    if filters.require_image && !message.attachments.iter().any(attachment_is_image) {
        return false;
    }
    if filters.require_link && !content_has_link(&message.content) {
        return false;
    }
    if filters.after_ms.is_some() || filters.before_ms.is_some() {
        // A message whose timestamp cannot be parsed cannot be confirmed inside
        // a requested window, so exclude it rather than guess.
        let Some(timestamp_ms) = chrono::DateTime::parse_from_rfc3339(&message.timestamp)
            .ok()
        ...
        if filters
            .after_ms
            .is_some_and(|after_ms| timestamp_ms < after_ms)
        {
            return false;
        }
        if filters
            .before_ms
            .is_some_and(|before_ms| timestamp_ms >= before_ms)
        {
            return false;
        }
    }
...
/// True when every free-text term is present in the message content.
///
/// Each term is matched as an independent, case-insensitive substring: an
/// unquoted word matches anywhere regardless of what surrounds it, and a
/// quoted phrase (a term containing a space) still matches anywhere but must
/// appear contiguously, which is what makes it an exact phrase match. `terms`
/// is empty when the query carries no free text, in which case every message
/// passes this check and matching is left entirely to the structured filters.
fn message_matches_terms(content_lc: &str, terms: &[String]) -> bool {
    terms.iter().all(|term| content_lc.contains(term.as_str()))
}

/// Trims, lowercases, and validates raw free-text search terms.
///
/// An empty term is dropped rather than rejected: the frontend tokenizer
/// never produces one, but this is the trust boundary, not that tokenizer.
/// Limits are enforced on the normalized terms, since padding that trims away
/// would otherwise let a caller sidestep the byte cap.
fn normalize_search_terms(terms: Vec<String>) -> BackendResult<Vec<String>> {
    if terms.len() > MAX_SEARCH_TERMS {
        return Err(BackendError::InvalidConfiguration(format!(
            "message search query has too many terms (maximum {MAX_SEARCH_TERMS})"
        )));
    }
    let normalized: Vec<String> = terms
        ...
        .collect();
    let total_bytes: usize = normalized.iter().map(|term| term.len()).sum();
    if total_bytes > MAX_SEARCH_QUERY_BYTES {
        return Err(BackendError::InvalidConfiguration(format!(
...
/// How many entries of `community_room_counts` (in the order a
/// cross-community scan walked them: every room of one community before the
/// next) were fully covered once the scan reached `rooms_scanned` rooms in
/// total. A community only counts once the scan passed every one of its
/// rooms; one reached only partway through does not count, even though some
/// of its results are present in the output.
fn communities_fully_scanned(
    community_room_counts: &[(String, usize)],
    rooms_scanned: usize,
) -> u32 {
    let mut communities_searched = 0u32;
    let mut remaining = rooms_scanned;
    for (_, room_count) in community_room_counts {
        if remaining >= *room_count {
            remaining -= room_count;
...
/// True when a cross-community search did not fully cover what it was asked
/// to. Any of three independent reasons can make this true: the
/// `MAX_SEARCH_COMMUNITIES` cap dropped some of the account's communities
/// before the walk even began; a community's room list could not be
/// resolved in time; or the shared deadline/budget stopped the room scan
/// before it reached every resolved room.
fn cross_community_search_truncated(
    capped_by_community_limit: bool,
    rooms_scanned: usize,
    rooms_total: usize,
    communities_with_resolved_rooms: usize,
    communities_attempted: usize,
) -> bool {
    capped_by_community_limit
        || rooms_scanned < rooms_total
        || communities_with_resolved_rooms < communities_attempted
}

/// Online presence, keyed by user ID. Only online accounts are stored, so the
/// value is the moment the state was last observed rather than the state
/// itself: anything older than [`CACHED_PRESENCE_TTL`] is treated as unknown.
type PresenceCache = HashMap<String, Instant>;

fn update_cached_presence(
    presence: &mut PresenceCache,
    user_id: String,
    state: String,
    max_entries: usize,
) {
    update_cached_presence_at(presence, user_id, state, max_entries, Instant::now())
}

fn update_cached_presence_at(
    presence: &mut PresenceCache,
    user_id: String,
    state: String,
    max_entries: usize,
    ...
    if !presence.contains_key(&user_id) && presence.len() >= max_entries {
        // Only pay for the sweep when the bound would otherwise reject a new
        // account, which keeps the common path a single hash insert.
        prune_expired_presence(presence, now);
    }
    let len = presence.len();
    match presence.entry(user_id) {
...
fn prune_expired_presence(presence: &mut PresenceCache, now: Instant) {
    presence.retain(|_, observed_at| now.duration_since(*observed_at) < CACHED_PRESENCE_TTL);
}

fn cached_presence_is_online(presence: &PresenceCache, user_id: &str, now: Instant) -> bool {
    presence
        .get(user_id)
        .is_some_and(|observed_at| now.duration_since(*observed_at) < CACHED_PRESENCE_TTL)
}

fn update_cached_typing_users(
    typing_users: &mut HashMap<String, Vec<String>>,
    room_id: String,
    users: Vec<String>,
) {
    if users.is_empty() {
        ...
        typing_users.insert(room_id, users);
    }
}
...
pub struct MatrixBackend {
    ...
    presence: Arc<RwLock<PresenceCache>>,
...
impl MatrixBackend {
    /// The one sanctioned lookup for a pending community invitation.
    ///
    /// `protected_joined_room` cannot apply here by definition: the room is
    /// still in the Invited state, so the joined-membership guard would reject
    /// the very invitations this path exists to answer. The validation that
    /// remains meaningful before joining, invited state and space-ness, lives
    /// here instead, mirroring `validated_dm_request_room`.
    fn validated_community_invite_room(
        client: &Client,
        room_id: &matrix_sdk::ruma::RoomId,
    ) -> BackendResult<Room> {
        let room = client.get_room(room_id).ok_or_else(|| {
            BackendError::NotFound("community invitation is no longer available".into())
        })?;
        if room.state() != RoomState::Invited || !room.is_space() {
            return Err(BackendError::NotFound(
                "community invitation is no longer available".into(),
            ));
        }
        Ok(room)
    }

    fn local_startup_error(error: BackendError) -> BackendError {
        match error {
            BackendError::PermissionDenied(detail) => {
                BackendError::LocalStatePermissionDenied(detail)
            }
            error => error,
        }
    }
    ...
    /// Scan `room_ids` for messages matching `query` and the parsed `filters`,
    /// fetching each room's recent events through `fetch`, and collect the
    /// matches into a bounded result set.
    ///
    /// The search's time budget is enforced here rather than by wrapping the
    /// whole scan in a `timeout`: when a per-room fetch outruns `deadline`, the
    /// matches already collected are returned (`Ok`) instead of being thrown
    /// away, so a search that is merely slow shows what it found rather than
    /// failing. Cancellation is different in kind: a superseded search, one the
    /// caller cancelled because a newer query replaced it, returns `Cancelled`,
    /// because its partial results are for a query the user has moved past.
    // Single private caller; each parameter is independently load-bearing
    // (search scope, filters, budget, and cancellation are distinct concerns),
    // so a params struct would add indirection without reducing complexity.
    #[allow(clippy::too_many_arguments)]
    /// Runs the shared per-room scan and returns both the matches found and
    /// how many of `room_ids` were actually fetched before the deadline,
    /// cancellation, or the budget cut the walk short. The count lets a
    /// caller that concatenated rooms from several sources (e.g. several
    /// communities) work out which of those sources were fully covered.
    async fn run_bounded_message_scan<F, Fut>(
        room_ids: Vec<String>,
        mut fetch: F,
        terms: &[String],
        ...
        cancellation: &CancellationToken,
    ) -> BackendResult<(Vec<MessageDto>, usize)>
    where
        F: FnMut(String) -> Fut,
        Fut: std::future::Future<Output = BackendResult<Vec<MessageDto>>>,
    {
        ...
        for room_id in room_ids {
            ...
            let messages = tokio::select! {
                ...
                fetched = tokio::time::timeout_at(deadline, fetch(room_id)) => {
                    match fetched {
                        // The per-room fetch outran the search's deadline. Return
                        // the matches found so far rather than failing the search.
                        Err(_elapsed) => return Ok((results.into_entries(), rooms_scanned)),
                        Ok(Ok(messages)) => messages,
                        Ok(Err(error)) if Self::blocked_entity_reason(&error).is_some() => {
                            rooms_scanned += 1;
                            continue;
                        }
            ...
            for message in messages {
                if !budget.reserve_event(message_search_retained_bytes(&message)) {
                    return Ok((results.into_entries(), rooms_scanned));
                }
                if cancellation.is_cancelled() {
                    return Err(BackendError::Cancelled(
                        "message search was superseded".into(),
                ...
                let content_lc = message.content.to_lowercase();
                if message.deleted_at.is_none()
                    && message_matches_terms(&content_lc, terms)
                    && message_matches_filters(&message, parsed_filters, account_id_lc)
                {
                    results.insert(message);
                }
            }
        }
        Ok((results.into_entries(), rooms_scanned))
    }
    ...
    fn blocked_entity_reason(error: &BackendError) -> Option<BlockedEntityReason> {
        match error {
            ...
            | BackendError::AccountNotFound(_)
            ...
            | BackendError::LocalStatePermissionDenied(_)
    ...
    /*
        Whether a room is currently a child of this Space.

        Both room mutations gate on this so a caller cannot rename or detach an
        unrelated room by passing its id: membership in the community is what
        authorises the edit, not merely being joined to the target room.
    */
    async fn space_has_child(space: &Room, room_id: &RoomId) -> BackendResult<bool> {
        // Same active-child rule validate_space_hierarchy_children applies: an
        // empty `via` means the child was detached, which is exactly how
        // remove_channel unlinks a room, so it must not count as a child.
        let candidates = vec![room_id.to_owned()];
        let state_events = space
            .get_state_events_for_keys_static::<SpaceChildEventContent, OwnedRoomId, _>(&candidates)
            .await
            .map_err(Self::map_error)?;
        for raw_event in state_events {
            match raw_event.deserialize() {
    ...
    /** The single place the voice/text distinction is read off a room. */
    fn channel_type_for_room(room: &Room) -> String {
        if room
            .room_type()
    ...
    /**
        Whether membership of this community is enough to join one of its rooms.

        The account service answers this, not Mesh: a restricted room's summary
        carries the rooms whose members it admits, so a room that admits this
        community is one this account can join by being here already. Guessing
        instead would mean offering a join that the server refuses, which is the
        same broken promise as a row that cannot open.

        Knock and invite rules are deliberately not joinable. Both need a
        decision from somebody else before this account may enter, and Mesh has
        no room-level request-to-join surface, so a room under either rule stays
        in the listing's blocked set exactly as it is today rather than growing a
        button that leads nowhere.
    */
    fn community_membership_admits(space_id: &RoomId, summary: &RoomSummary) -> bool {
        match &summary.join_rule {
            JoinRuleSummary::Public => true,
            JoinRuleSummary::Restricted(restricted)
            | JoinRuleSummary::KnockRestricted(restricted) => restricted
                .allowed_room_ids
                .iter()
                .any(|allowed| allowed == space_id),
            // Invite, knock, and any rule a later Matrix release adds. Reading
            // an unrecognised rule as joinable would offer a join the account
            // service then refuses, so the default is the closed one.
            _ => false,
        }
    }

    /**
        A room of this community that the account has not joined.

        Everything here comes from the hierarchy summary, because there is no
        local room to read: an unjoined room has no timeline, no receipts and no
        unread count, and reporting zero unread is therefore a fact rather than a
        placeholder. `name` falls back the same way the joined path does so the
        two cannot disagree about an unnamed room.
    */
    fn unjoined_channel_dto(
        community_id: &str,
        room_id: &RoomId,
        summary: &RoomSummary,
    ) -> ChannelDto {
        ChannelDto {
            ...
            name: summary.name.clone().unwrap_or_else(|| "unnamed".into()),
            topic: summary.topic.clone().unwrap_or_default(),
            channel_type: if summary
                .room_type
                .as_ref()
                .is_some_and(|room_type| room_type.as_str() == "org.mesh.voice")
            {
    ...
    async fn space_child_ids_for_listing(&self, space: &Room) -> BackendResult<SpaceChildListing> {
        ...
        let listing = async {
            ...
            for _ in 0..MAX_SPACE_HIERARCHY_PAGES {
                ...
                // Summaries are collected before the identifiers are bounded, and
                // the bound below is what stops this map growing without limit:
                // a page over the cap fails the whole listing rather than
                // returning.
                let page_summaries: Vec<RoomSummary> = response
                    .rooms
                    .into_iter()
                ...
                Self::append_bounded_space_hierarchy_room_ids(
                    space.room_id(),
                    page_summaries.iter().map(|summary| summary.room_id.clone()),
                    &mut child_ids,
                    &mut seen_room_ids,
                )?;
                for summary in page_summaries {
                    summaries.entry(summary.room_id.clone()).or_insert(summary);
                }
                let Some(next_batch) = Self::checked_space_hierarchy_next_batch(
                    ...
                    return Ok(SpaceChildListing {
                        children: Self::validate_space_hierarchy_children(space, child_ids).await?,
                        summaries,
                    });
                };
    ...
    /// How many bytes a transfer may hold in flight.
    ///
    /// This is also the transfer's hard cap, which is what makes it a
    /// reservation rather than a guess: callers pass it to
    /// `download_bounded_encrypted_media` as the limit, so nothing can consume
    /// more than it accounted for. Note that decryption briefly holds the
    /// ciphertext and the plaintext together, so peak memory for one transfer
    /// is about twice this; `MAX_INFLIGHT_MEDIA_BYTES` is tuned against that.
    ...
    fn load_stored_recovery_credential(storage: &AccountStorage) -> BackendResult<String> {
            ...
            .map_err(Self::map_secure_storage_error)?
        {
            keychain::SecretLookup::Found(mut bytes) => {
                let result = String::from_utf8(bytes.clone()).map_err(Self::map_display_error);
                bytes.zeroize();
    ...
    fn load_trusted_devices(storage: &AccountStorage) -> BackendResult<TrustedDeviceRegistry> {
        ...
        match keychain::lookup_secret(&key).map_err(Self::map_secure_storage_error)? {
            keychain::SecretLookup::Found(serialized) => {
                serde_json::from_slice(&serialized).map_err(Self::map_display_error)
            }
    ...
    fn persist_trusted_devices(
        ...
        let serialized = serde_json::to_vec(devices).map_err(Self::map_display_error)?;
    ...
    fn load_last_recovery_test(storage: &AccountStorage) -> BackendResult<Option<String>> {
        ...
        match keychain::lookup_secret(&key).map_err(Self::map_secure_storage_error)? {
            keychain::SecretLookup::Found(bytes) => String::from_utf8(bytes)
                .map(Some)
                .map_err(Self::map_display_error),
    ...
    async fn verify_recovery_credential(
        ...
        recovery_key_or_passphrase.zeroize();
        match result {
            Ok(result) => result.map_err(Self::map_display_error),
    ...
    fn persist_registry(&self, registry: &AccountRegistry) -> BackendResult<()> {
        let serialized = serde_json::to_vec(registry).map_err(Self::map_display_error)?;
    ...
    fn purge_expired_login_reservations(state: &mut LoginAttemptState) {
        ...
        while let Some(cancellation_id) = state.reservation_order.front() {
            ...
            let Some(cancellation_id) = state.reservation_order.pop_front() else {
                break;
            };
    ...
    fn qualify_user_input(input: &str, account_server: &ServerName) -> BackendResult<OwnedUserId> {
        ...
        if input.starts_with('@') {
            return UserId::parse(input).map_err(Self::map_display_error);
        }
        let username = Self::normalize_product_username(input)?;
        UserId::parse(format!("@{username}:{account_server}")).map_err(Self::map_display_error)
    }
    ...
    fn qualify_public_link_input(
        ...
        let input = input.trim();
        if input.starts_with('#') {
            return RoomAliasId::parse(input).map_err(Self::map_display_error);
        }
        let slug = Self::normalize_public_link_slug(input)?;
        RoomAliasId::parse(format!("#{slug}:{account_server}")).map_err(Self::map_display_error)
    }
    ...
    async fn build_client(
        ...
        std::fs::create_dir_all(&storage.store_root).map_err(Self::map_display_error)?;
    ...
    fn persist_session(
        ...
        let serialized = serde_json::to_vec(&value).map_err(Self::map_display_error)?;
    ...
    /// Name what moved in a room so each caller can refetch only what it needs.
    ///
    /// Read receipts and typing notifications arrive as ephemeral data and are
    /// by far the most frequent update in an active room. Treating them as
    /// "something changed" meant every one of them paid for a full timeline
    /// refetch on both sides of the IPC boundary.
    fn classify_room_update(update: &matrix_sdk::sync::RoomUpdate) -> MatrixRoomUpdateKind {
        match update {
            matrix_sdk::sync::RoomUpdate::Joined { updates, .. } => {
                if !updates.timeline.events.is_empty() {
                    MatrixRoomUpdateKind::Timeline
                } else if !matches!(
                    &updates.state,
                    ...
                    MatrixRoomUpdateKind::State
                } else {
                    MatrixRoomUpdateKind::Ephemeral
                }
            }
            // Leaving, being invited, or knocking all change who may see the
            // room, which every caller has to react to even though no timeline
            // event arrived.
            _ => MatrixRoomUpdateKind::State,
        }
    }
    ...
    async fn install_client(&self, client: Client, homeserver: String, profile_id: String) {
        ...
        // Membership transitions on this account's own user, which is how a
        // community appears or disappears. list_communities reads the local
        // store, so without this the rail only ever reflected the state at
        // bootstrap: a space joined or left on another device, or an invitation
        // arriving in a later sync, stayed invisible until the app restarted.
            ...
            move |event: SyncRoomMemberEvent, room: Room| {
                ...
                async move {
                    ...
                    MatrixBackend::dispatch_backend_event(
                        &event_callback,
                        MatrixBackendEvent::CommunitiesChanged,
                    );
                }
            }
        });
        // Invitations arrive stripped, and an invited room usually has no
        // readable create event yet, so its space-ness cannot be tested here.
        // Refetching the listing is cheap and idempotent, so an occasional
        // refresh for a non-space invitation is the right trade.
        client.add_event_handler({
            let event_callback = Arc::clone(&self.event_callback);
        ...
        client.add_event_handler({
            ...
            move |event: PresenceEvent| {
                ...
                async move {
                    let mut cached_presence = presence.write().await;
                    update_cached_presence(
                        &mut cached_presence,
                        event.sender.to_string(),
                        event.content.presence.to_string(),
                        MAX_CACHED_ONLINE_PRESENCE,
                    );
                }
            }
        });
        client.add_event_handler({
            ...
            move |event: SyncTypingEvent, room: Room| {
                ...
                async move {
                    ...
                    {
                        typing_users.write().await.remove(room_id.as_str());
                        MatrixBackend::dispatch_backend_event(
                            &event_callback,
                            MatrixBackendEvent::TypingChanged(MatrixTypingChanged { room_id }),
                        );
                        tracing::warn!(
                            target: "mesh::security",
                            room_id = %room.room_id(),
                    ...
                    let users = event
                        ...
                        .collect();
                    let mut cached_typing_users = typing_users.write().await;
                    update_cached_typing_users(&mut cached_typing_users, room_id.clone(), users);
                    drop(cached_typing_users);
                    MatrixBackend::dispatch_backend_event(
                        &event_callback,
                        MatrixBackendEvent::TypingChanged(MatrixTypingChanged { room_id }),
                    );
    ...
    async fn stop_runtime(&self) -> Option<Client> {
        ...
        *self.wire_privacy.write().await = WirePrivacyPreferences::default();
        ...
        drop(media_keys);
        self.verified_room_upgrades.write().await.clear();
        self.typing_users.write().await.clear();
        self.presence.write().await.clear();
        let verification_flows = self
            ...
            .map(|(_, flow)| flow)
            .collect::<Vec<_>>();
        for flow in verification_flows {
            let _ = Self::cancel_device_verification_flow(flow).await;
        }
    ...
    fn validate_device_verification_id(verification_id: &str) -> BackendResult<()> {
        if verification_id.len() != 36 || uuid::Uuid::parse_str(verification_id).is_err() {
            return Err(BackendError::InvalidConfiguration(
                "verification session identifier is invalid".into(),
            ));
        }
        Ok(())
    }

    fn ensure_device_verification_capacity(active: usize) -> BackendResult<()> {
        if active >= MAX_ACTIVE_DEVICE_VERIFICATIONS {
            return Err(BackendError::Other(
                "Finish or cancel an existing device verification before starting another".into(),
            ));
        }
        Ok(())
    }

    fn device_verification_is_terminal(snapshot: &MatrixVerificationSession) -> bool {
        matches!(snapshot.phase.as_str(), "done" | "cancelled")
    }

    async fn cancel_device_verification_flow(flow: DeviceVerificationFlow) -> BackendResult<()> {
        match flow {
...
fn allow_saved_account_reauthentication(error: BackendError) -> BackendResult<()> {
    match error {
        BackendError::NotAuthenticated => {
            tracing::warn!(
                target: "mesh::matrix",
                "The saved sign-in expired; opening account sign-in"
            );
            Ok(())
        }
        error => Err(error),
...
impl MeshBackend for MatrixBackend {
    ...
    async fn matrix_room_is_encrypted(&self, room_id: String) -> BackendResult<bool> {
        let client = self.client().await?;
        let room_id = matrix_sdk::ruma::RoomId::parse(room_id).map_err(Self::map_display_error)?;
    ...
    async fn start_oidc_login(
        ...
        let operation = async {
            ...
            oidc::EphemeralStore::remove_stale(&self.store_root).map_err(Self::map_display_error)?;
            let ephemeral_store =
                oidc::EphemeralStore::create(&self.store_root).map_err(Self::map_display_error)?;
            ...
            let redirect_uri =
                url::Url::parse(registration.redirect_uri()).map_err(Self::map_display_error)?;
            ...
            .await
            {
                ...
                Err(oidc::CallbackError::PortUnavailable) => {
                    oauth.abort_login(&authorization.state).await;
                    return Err(BackendError::InvalidConfiguration(
                        "Mesh lost access to its local browser sign-in port. Close any other sign-in attempt and try again."
                            .into(),
                    ));
                }
                Err(_) => {
    ...
    async fn logout(&self) -> BackendResult<()> {
        ...
        match client.auth_api() {
            ...
            Some(AuthApi::OAuth(auth)) => {
                auth.logout().await.map_err(Self::map_display_error)?;
            }
    ...
    async fn remove_local_account(&self) -> BackendResult<()> {
        ...
        if let Some(client) = client {
            let logout_result = match client.auth_api() {
                ...
                Some(AuthApi::OAuth(auth)) => auth.logout().await.map_err(Self::map_display_error),
    ...
    async fn start_device_verification(
        ...
        if device_id.is_empty() || device_id.len() > MAX_DEVICE_ID_BYTES {
            return Err(BackendError::InvalidConfiguration(
                "device identifier is invalid".into(),
            ));
        }
        Self::ensure_device_verification_capacity(self.verification_sessions.read().await.len())?;
        let client = self.client().await?;
        ...
        if let Err(error) = Self::ensure_device_verification_capacity(sessions.len()) {
            drop(sessions);
            Self::cancel_device_verification_flow(flow).await?;
            return Err(error);
        }
        sessions.insert(verification_id, flow);
        Ok(snapshot)
    }
    ...
    async fn device_verification_status(
        ...
        Self::validate_device_verification_id(&verification_id)?;
        let mut sessions = self.verification_sessions.write().await;
        let snapshot = {
            ...
            Self::verification_snapshot(verification_id.clone(), flow)?
        };
        if Self::device_verification_is_terminal(&snapshot) {
    ...
    async fn select_device_verification_method(
        ...
        let snapshot = Self::verification_snapshot(verification_id.clone(), &flow)?;
        let mut sessions = self.verification_sessions.write().await;
        match sessions.get(&verification_id) {
            Some(DeviceVerificationFlow::Request { .. }) => {
                sessions.insert(verification_id, flow);
                Ok(snapshot)
            }
            Some(_) => {
            ...
            None => {
                ...
                Err(BackendError::Cancelled(
                    "device verification was cancelled before the method was ready".into(),
                ))
            }
        }
    }
    ...
    async fn create_community(
        ...
        space_request.creation_content =
            Some(Raw::new(&space_creation).map_err(Self::map_display_error)?);
    ...
    async fn list_channels(&self, community_id: String) -> BackendResult<EntityList<ChannelDto>> {
        ...
        let listing = self.space_child_ids_for_listing(&space).await?;
        let summaries = listing.summaries;
        let mut child_listing = listing.children;
        ...
        for child_id in child_listing.entities {
            ...
            /*
                A room this account is not in, but is admitted to by being in
                this community, is listed as an unjoined room rather than
                quarantined. Membership is read before the protected-room check
                so that the two failures stay distinct: not being in a room is
                not a fault in the room, while a room that is joined and not
                encrypted still has to be kept out of the list.
            */
            let membership = Self::account_room_membership(&client, &child_id);
            if membership != RoomState::Joined {
                let admitted = summaries
                    .get(&child_id)
                    .is_some_and(|summary| Self::community_membership_admits(&space_id, summary))
                    || membership == RoomState::Invited;
                if let (true, Some(summary)) = (admitted, summaries.get(&child_id)) {
                    if summary.room_type.as_ref().map(RoomType::as_str) != Some("m.space")
                        && listed_room_ids.insert(child_id.clone())
                    {
                        channels.push(Self::unjoined_channel_dto(
                            &community_id,
                            &child_id,
                            summary,
                        ));
            ...
            let Some(room) = chain.entities.pop() else {
                return Err(BackendError::Other(
                    "Room replacement traversal returned no protected room".into(),
                ));
            };
    ...
    async fn create_channel(
        ...
        if channel_type == "voice" {
            ...
            request.creation_content = Some(Raw::new(&creation).map_err(Self::map_display_error)?);
        }
        ...
        Ok(ChannelDto {
            ...
            channel_type,
            unread_count: 0,
            // Creating a room joins it.
            joined: true,
        })
    }

    /*
        Joining a room this community already admits this account to.

        Two guards decide whether the join may stand, and they are deliberately
        the same two `join_community` applies to the rooms it opens. The room
        must be an active child of the named community, so passing an unrelated
        room ID here cannot be used to enter it; and the joined room must satisfy
        Mesh's encryption rule, or the join is rolled back rather than leaving
        somebody inside a room Mesh will then refuse to open.

        The rollback is narrow on purpose. It runs only for a deterministic
        policy rejection, and only when this call is what created the membership.
        A network failure while checking encryption leaves the join alone,
        because un-joining a room over a dropped connection would lose a
        membership the person asked for and the server already granted.
    */
    async fn join_community_channel(
        ...
        let space_id =
            matrix_sdk::ruma::RoomId::parse(&community_id).map_err(Self::map_display_error)?;
        let space =
            Self::protected_joined_room(&client, &space_id, "joining a community room").await?;
        if !space.is_space() {
            return Err(BackendError::InvalidConfiguration(
                "community ID does not identify a Matrix Space".into(),
            ));
        ...
        let room_id =
            matrix_sdk::ruma::RoomId::parse(&channel_id).map_err(Self::map_display_error)?;
        if !Self::space_has_child(&space, &room_id).await? {
            return Err(BackendError::InvalidConfiguration(
                "that room is not part of this community".into(),
            ));
        }

        let prior_membership_known_absent =
            Self::account_room_membership(&client, &room_id) != RoomState::Joined;
        let room = client
        ...
        let validation = async {
            ...
            Self::require_protected_room(&room, "joining a community room").await
        }
        .await;
        if let Err(error) = validation {
            let policy_rejection = matches!(
                error,
                BackendError::NotEncrypted(_) | BackendError::InvalidConfiguration(_)
            );
            if policy_rejection && prior_membership_known_absent {
                if let Err(leave_error) = room.leave().await {
                    tracing::warn!(
                        target: "mesh::security",
                        room_id = %room_id,
                        "Could not roll back a rejected community room join: {leave_error}"
                    );
                }
            }
            return Err(error);
        ...
        Ok(ChannelDto {
            ...
            name: room.name().unwrap_or_else(|| "unnamed".into()),
            topic: room.topic().unwrap_or_default(),
            channel_type: Self::channel_type_for_room(&room),
            unread_count: room.num_unread_messages().min(u32::MAX as u64) as u32,
            joined: true,
        })
    }
    ...
    /*
        Renaming and re-topicing a room. A typo in a room name used to be
        permanent: rooms could be created and never edited, while community
        settings told administrators to "use each room's menu in the navigation
        to manage it" and that menu carried no such action.

        Both fields are optional so a caller can change one without having to
        resend, and possibly clobber, the other.
    */
    async fn update_profile_avatar(
        ...
        MatrixBackend::set_profile_avatar_image(self, filename, content_type, bytes).await
    }

    async fn clear_profile_avatar(&self) -> BackendResult<MatrixProfile> {
        MatrixBackend::clear_profile_avatar_image(self).await
    }

    async fn load_profile_avatar(&self, avatar_url: String) -> BackendResult<Vec<u8>> {
        MatrixBackend::load_profile_avatar_image(self, avatar_url).await
    }
    ...
    async fn set_room_avatar(
        ...
        MatrixBackend::set_room_avatar_image(self, room_id, filename, content_type, bytes).await
    }

    async fn clear_room_avatar(&self, room_id: String) -> BackendResult<()> {
        MatrixBackend::clear_room_avatar_image(self, room_id).await
    }
    ...
    async fn update_channel(
        ...
        let name = name
            .map(|value| normalize_required_metadata(value, "Room name", CHANNEL_NAME_MAX_UTF16))
            .transpose()?;
        let topic = topic
            .map(|value| {
        ...
        if name.is_none() && topic.is_none() {
            return Err(BackendError::InvalidConfiguration(
                "a room update must change the name or the description".into(),
            ));
        }
        ...
        let space_id =
            matrix_sdk::ruma::RoomId::parse(&community_id).map_err(Self::map_display_error)?;
        let space = Self::protected_joined_room(&client, &space_id, "updating a room").await?;
        if !space.is_space() {
            return Err(BackendError::InvalidConfiguration(
                "community ID does not identify a Matrix Space".into(),
            ));
        ...
        let room_id =
            matrix_sdk::ruma::RoomId::parse(&channel_id).map_err(Self::map_display_error)?;
        // Refuse to edit a room that is not a child of this community, so a
        // caller cannot rename an unrelated room by passing its id here.
        if !Self::space_has_child(&space, &room_id).await? {
            return Err(BackendError::InvalidConfiguration(
                "that room is not part of this community".into(),
            ));
        }
        let room = Self::protected_joined_room(&client, &room_id, "updating a room").await?;

        if let Some(name) = name.clone() {
            room.set_name(name).await.map_err(Self::map_error)?;
        }
        if let Some(topic) = topic.clone() {
            room.set_room_topic(&topic).await.map_err(Self::map_error)?;
        }

        Ok(ChannelDto {
            id: room.room_id().to_string(),
            community_id,
            // What was just written, falling back to synced state for the field
            // this call left alone. Reading both from `room` instead would
            // report the pre-write value until the echo arrives.
            name: name.or_else(|| room.name()).unwrap_or_default(),
            topic: topic.or_else(|| room.topic()).unwrap_or_default(),
            channel_type: Self::channel_type_for_room(&room),
            unread_count: 0,
            // Editing a room requires being in it.
            joined: true,
        })
    }

    /*
        Removing a room from a community.

        Matrix has no delete: a room exists as long as somebody is in it. What
        removal can honestly mean is "this is no longer part of the community
        and I am no longer in it", so this redacts the m.space.child that binds
        the room to the Space and then leaves. Members already in the room keep
        their copy, which is why the confirmation copy says so rather than
        promising a deletion Mesh cannot perform.
    */
    async fn remove_channel(&self, community_id: String, channel_id: String) -> BackendResult<()> {
        let client = self.client().await?;
        let space_id =
            matrix_sdk::ruma::RoomId::parse(&community_id).map_err(Self::map_display_error)?;
        let space = Self::protected_joined_room(&client, &space_id, "removing a room").await?;
        if !space.is_space() {
            return Err(BackendError::InvalidConfiguration(
                "community ID does not identify a Matrix Space".into(),
            ));
        ...
        // Detach first. If leaving fails afterwards the room is already out of
        // the community, which is the state the caller asked for; doing it the
        // other way round can leave a child pointing at a room nobody can reach.
        space
            .send_state_event_for_key(&room_id, SpaceChildEventContent::new(Vec::new()))
            .await
            .map_err(Self::map_error)?;
        // Leaving goes through the same guard every other room action uses:
        // `get_room` alone would also hand back a room this account has been
        // banned from or has already left, and call leave() on it.
        if let Some(room) =
            Self::protected_joined_room_if_available(&client, &room_id, "removing a room").await?
        {
    ...
    async fn list_custom_emoji(&self, community_id: String) -> BackendResult<Vec<CustomEmoji>> {
        ...
        // Read every pack in the room, not only the one Mesh writes, so a
        // community that made its emoji in another client still sees them.
        let pack = Self::merged_custom_emoji_pack(&space).await?;
        pack.images
            .iter()
    ...
    async fn upload_custom_emoji(
        ...
        let emoji_content_type = content_type.trim().to_owned();
        let emoji_filename = filename.trim().to_owned();
        let sanitized = Self::decode_off_runtime(move || {
            Self::sanitize_custom_emoji(&bytes, &emoji_content_type, &emoji_filename)
        })
        .await?;
        let client = self.client().await?;
        let space_id =
        ...
        info.width =
            Some(UInt::try_from(u64::from(sanitized.width)).map_err(Self::map_display_error)?);
        info.height =
            Some(UInt::try_from(u64::from(sanitized.height)).map_err(Self::map_display_error)?);
        ...
        if let Some(info) = image.info.as_mut() {
            info.size = Some(UInt::try_from(size_bytes as u64).map_err(Self::map_display_error)?);
        }
    ...
    async fn matrix_rtc_join(&self, room_id: String) -> BackendResult<MatrixRtcJoinResult> {
        ...
        let parsed_room_id =
            matrix_sdk::ruma::RoomId::parse(&room_id).map_err(Self::map_display_error)?;
        let room =
    ...
    async fn send_message(
        ...
        let own_user_id = client.user_id().ok_or(BackendError::NotAuthenticated)?;
        let mut mentions =
            Self::mentions_for_user_ids(&room, &mention_user_ids, own_user_id).await?;
        if mentions_room {
            Self::require_room_notification_permission(&room, own_user_id).await?;
            mentions.room = true;
        }
        let base_content =
            RoomMessageEventContentWithoutRelation::text_plain(body.clone()).add_mentions(mentions);

        let content = match reply_to_id.as_deref() {
            Some(event_id) => {
                let event_id =
                    matrix_sdk::ruma::EventId::parse(event_id).map_err(Self::map_display_error)?;
        ...
        let mut raw_content = serde_json::to_value(&content).map_err(Self::map_display_error)?;
        raw_content
            .as_object_mut()
            .ok_or_else(|| {
                BackendError::InvalidConfiguration(
        ...
        let raw_content = serde_json::to_string(&raw_content).map_err(Self::map_display_error)?;
        ...
        let send_handle = match queue.send_raw(raw_content, "m.room.message".into()).await {
            ...
            Err(error) => {
                ...
                return Err(Self::map_display_error(error));
            }
        };
    ...
    async fn retry_queued_message(
        ...
        self.reject_ignored_direct_room(&client, &room).await?;
        let queue = room.send_queue();
        let (echoes, _) = queue.subscribe().await.map_err(Self::map_display_error)?;
        let echo = echoes
            .into_iter()
            .take(MAX_SEND_QUEUE_ROOM_MESSAGES)
    ...
    async fn cancel_queued_message(
        ...
        if !send_handle.abort().await.map_err(Self::map_display_error)? {
            return Err(BackendError::NotFound(
    ...
    async fn send_attachment(
        ...
        let metadata = match tokio::fs::metadata(&path)
            .await
            .map_err(Self::map_display_error)
        {
            Ok(metadata) => metadata,
        ...
        let result: BackendResult<MessageDto> = async {
            ...
            let data = tokio::select! {
                result = tokio::fs::read(&path) => result.map_err(Self::map_display_error)?,
                _ = cancellation.cancelled() => {
            ...
            let explicit_mentions =
                Self::mentions_for_user_ids(&room, &mentions, own_user_id).await?;
            let message_mentions = explicit_mentions
                .user_ids
                .iter()
                .map(ToString::to_string)
                .collect::<Vec<_>>();
            ...
            let base_content =
                RoomMessageEventContentWithoutRelation::new(MessageType::File(file_content))
                    .add_mentions(explicit_mentions);
            let content = match reply_to_id.as_deref() {
                Some(event_id) => {
            ...
            let response = room
                ...
                .await
                .map_err(Self::map_matrix_send_error)?;
            let (display_name, author_avatar_url) =
                Self::own_author_profile(&room, own_user_id).await?;
            ...
            Ok(MessageDto {
                ...
                // Attachment sends carry no room-wide mention: the composer
                // affordance is on the text path only.
                mentions_room: false,
    ...
    async fn download_attachment(
        ...
        let result: BackendResult<String> = async {
            ...
            let data = tokio::select! {
                // Bounded by what was reserved, not by the global maximum. The
                // reservation comes from the sender's declared size, so a
                // sender declaring one byte used to reserve one byte and then
                // be allowed to buffer the full 100 MiB; three concurrent
                // slots made the real ceiling roughly 300 MiB against a 200 MiB
                // budget. Holding the transfer to its own declaration makes the
                // reservation true by construction and cuts a lying sender off
                // at their own number.
    ...
    async fn load_attachment_image(
        ...
        .await?;
        // Same reasoning as the download path: the transfer may not exceed
        // what it reserved, or the reservation describes nothing.
    ...
    async fn dm_conversations(&self) -> BackendResult<EntityList<DmConversationDto>> {
            ...
            let projection = async {
                ...
                Ok(DmConversationDto {
                    ...
                    peers: vec![DmPeerDto {
                        ...
                        avatar_color: Self::avatar_color(user_id.as_str()),
        ...
        conversations.sort_by(|left, right| {
            // Tie-break on the first peer's name, a stable key for both a
            // one-to-one DM and a group (peer order within a room is stable).
            let peer_name = |conversation: &DmConversationDto| {
                conversation
                    .peers
                    .first()
            ...
            right
                ...
                .then_with(|| peer_name(left).cmp(&peer_name(right)))
                .then_with(|| left.id.cmp(&right.id))
        });
        Ok(EntityList {
    ...
    async fn ensure_dm(&self, recipient_user_id: String) -> BackendResult<DmConversationDto> {
        let client = self.client().await?;
        let recipient =
            matrix_sdk::ruma::UserId::parse(recipient_user_id).map_err(Self::map_display_error)?;
        ...
        Ok(DmConversationDto {
            ...
            peers: vec![DmPeerDto {
                ...
                display_name: peer_display_name,
                avatar_color: Self::avatar_color(recipient.as_str()),
                avatar_url: peer_avatar_url,
            }],
    ...
    async fn dm_messages(
        ...
        let client = self.client().await?;
        let room_id =
            matrix_sdk::ruma::RoomId::parse(&conversation_id).map_err(Self::map_display_error)?;
        let room =
    ...
    async fn messages(
        ...
        Self::resolve_projected_member_profiles(&room, &mut result, requested).await?;
        Ok(result)
    }
    ...
    async fn thread_context(
        ...
        let room_id = matrix_sdk::ruma::RoomId::parse(room_id).map_err(Self::map_display_error)?;
        let thread_root_id =
            matrix_sdk::ruma::EventId::parse(thread_root_id).map_err(Self::map_display_error)?;
        ...
        Self::resolve_projected_member_profiles(
            &room,
    ...
    /// Room-scoped "My threads": every thread this account authored or
    /// replied to, per the server's standard `Participated` thread filter.
    /// Threads where the account was only mentioned, never followed up on,
    /// are a known gap for this first slice (see P9 in the backlog spec).
    async fn thread_list(&self, room_id: String) -> BackendResult<MatrixThreadListDto> {
        let client = self.client().await?;
        let own_user_id = client.user_id().ok_or(BackendError::NotAuthenticated)?;
        let room_id = matrix_sdk::ruma::RoomId::parse(room_id).map_err(Self::map_display_error)?;
        let room = Self::protected_joined_room(&client, &room_id, "listing threads").await?;
        let ignored_users = self.refresh_ignored_user_list(&client).await?;
        ...
        let roots = room
            .list_threads(ListThreadsOptions {
                ...
                from: None,
                limit: Some(UInt::from(MAX_THREAD_LIST_ROOTS)),
            })
            .await
            .map_err(Self::map_error)?;

        let mut items = Vec::with_capacity(roots.chunk.len());
        for root_event in &roots.chunk {
            let Ok(root_value) = root_event.raw().deserialize_as::<serde_json::Value>() else {
                continue;
            };
            if Self::is_ignored_non_state_event(&root_value, &ignored_users) {
                continue;
            }
            let Some(root_id_str) = root_value
                .get("event_id")
                .and_then(serde_json::Value::as_str)
            else {
                continue;
            };
            let Ok(thread_root_id) = matrix_sdk::ruma::EventId::parse(root_id_str) else {
                continue;
            };
        ...
        Ok(MatrixThreadListDto {
            items,
            has_more: roots.prev_batch_token.is_some(),
        })
    }
    ...
    async fn edit_message(
        ...
        let mentions = Self::mentions_for_user_ids(&room, &mention_user_ids, own_user_id).await?;
    ...
    async fn toggle_reaction(
        ...
        if let Some(reaction_id) = existing {
            let reaction_id =
                matrix_sdk::ruma::EventId::parse(reaction_id).map_err(Self::map_display_error)?;
    ...
    async fn set_unread_flag(&self, room_id: String, unread: bool) -> BackendResult<()> {
        let client = self.client().await?;
        let room_id = matrix_sdk::ruma::RoomId::parse(room_id).map_err(Self::map_display_error)?;
        let room =
            Self::protected_joined_room(&client, &room_id, "updating the unread marker").await?;
        room.set_unread_flag(unread).await.map_err(Self::map_error)
    }
    ...
    async fn typing_users(&self, room_id: String) -> BackendResult<Vec<TypingUser>> {
        ...
        for user_id in user_ids {
            let parsed =
                matrix_sdk::ruma::UserId::parse(&user_id).map_err(Self::map_display_error)?;
            let display_name = room
                .get_member(&parsed)
                .await
                .map_err(Self::map_error)?
    ...
    async fn search_messages(
        ...
        let parsed_filters = ParsedSearchFilters::from_raw(&filters);
        let terms = normalize_search_terms(terms)?;
        // A search with no text and no active filter matches nothing; allow a
        // filter-only search (e.g. `has:image`) to proceed with no terms.
        if terms.is_empty() && !parsed_filters.is_active() {
            return Ok(Vec::new());
        }
        let client = self.client().await?;
        let account_id = client
            ...
            .ok_or(BackendError::NotAuthenticated)?
            .to_string();
        let account_id_lc = account_id.to_lowercase();
        ...
        // Resolving which rooms to scan is bounded by the same deadline. Setup
        // that outruns it means nothing was found in time, which is an empty
        // result, not a failure.
        let resolve_rooms = async {
            let direct_room_scope = if let Ok(room_id) =
                matrix_sdk::ruma::RoomId::parse(&community_id)
            {
                tokio::select! {
                    biased;
                    _ = cancellation.cancelled() => {
                    ...
                    room = Self::protected_joined_room(&client, &room_id, "searching messages") => {
                        // A transient failure to inspect the room must not abort the
                        // whole search: fall back to enumerating channels, the behavior
                        // used before direct conversations were scoped to a single room.
                        match room {
                            Ok(room) => !room.is_space() && room.direct_targets().len() == 1,
                            Err(_) => false,
                        }
                    }
                }
            ...
            if direct_room_scope {
                Ok(vec![community_id])
            } else {
                tokio::select! {
                    biased;
                    _ = cancellation.cancelled() => {
                        Err(BackendError::Cancelled("message search was superseded".into()))
                    }
                    result = self.list_channels(community_id) => {
                        Ok(result?
                            .entities
                            .into_iter()
        ...
        let room_ids = match tokio::time::timeout_at(deadline, resolve_rooms).await {
            Ok(result) => result?,
            Err(_elapsed) => return Ok(Vec::new()),
        };
        let (entries, _rooms_scanned) = Self::run_bounded_message_scan(
            room_ids,
            |room_id| self.messages(room_id, MAX_SEARCH_EVENTS_PER_ROOM, None, None),
            &terms,
            &parsed_filters,
            &account_id_lc,
            limit,
    ...
    async fn search_messages_everywhere(
        ...
        let parsed_filters = ParsedSearchFilters::from_raw(&filters);
        let terms = normalize_search_terms(terms)?;
        // `communities_total` is the account's real join count, reported
        // as-is even when the cap below drops some of it from the walk: a
        // truncated set that still claims to be "all N communities" would
        // be exactly the silent-cap dishonesty this scope report exists to
        // avoid.
        let communities_total = community_ids.len();
        let community_ids: Vec<String> = community_ids
            .into_iter()
            .take(MAX_SEARCH_COMMUNITIES)
            .collect();
        let capped_by_community_limit = community_ids.len() < communities_total;
        // A search with no text and no active filter matches nothing; allow a
        // filter-only search (e.g. `has:image`) to proceed with no terms.
        if terms.is_empty() && !parsed_filters.is_active() {
            return Ok(MatrixCrossCommunitySearchResultDto {
                results: Vec::new(),
                scope: MatrixSearchScopeDto {
                    communities_searched: 0,
                    communities_total: communities_total as u32,
                    rooms_searched: 0,
                    rooms_total: 0,
                    truncated: false,
                },
        ...
        // Resolve every community's rooms up front, sharing the search's own
        // deadline. A community whose list can't be resolved in time, or
        // fails outright, is simply left out of the walk: the scope report
        // below says so rather than failing the whole search over one
        // community.
        let communities_attempted = community_ids.len();
        let mut community_room_counts: Vec<(String, usize)> =
            Vec::with_capacity(communities_attempted);
        ...
        for community_id in community_ids {
            ...
            let resolved = tokio::select! {
                ...
                result = tokio::time::timeout_at(deadline, self.list_channels(community_id.clone())) => result,
            };
            let Ok(Ok(listing)) = resolved else {
                continue;
            };
            let community_room_ids: Vec<String> = listing
                .entities
                .into_iter()
                .map(|channel| channel.id)
                .collect();
            community_room_counts.push((community_id, community_room_ids.len()));
            room_ids.extend(community_room_ids);
        }
        let rooms_total = room_ids.len();
        let communities_with_resolved_rooms = community_room_counts.len();

        let (entries, rooms_scanned) = Self::run_bounded_message_scan(
            room_ids,
            |room_id| self.messages(room_id, MAX_SEARCH_EVENTS_PER_ROOM, None, None),
            &terms,
            &parsed_filters,
        ...
        // `run_bounded_message_scan` scanned rooms in the order they were
        // appended above, i.e. every room of one community before the next.
        let communities_searched = communities_fully_scanned(&community_room_counts, rooms_scanned);
        let truncated = cross_community_search_truncated(
            capped_by_community_limit,
            rooms_scanned,
            rooms_total,
            communities_with_resolved_rooms,
            communities_attempted,
        );

        Ok(MatrixCrossCommunitySearchResultDto {
            results: entries,
            scope: MatrixSearchScopeDto {
                communities_searched,
                communities_total: communities_total as u32,
                rooms_searched: rooms_scanned as u32,
                rooms_total: rooms_total as u32,
                truncated,
            },
        })
    }
    ...
    async fn wait_for_room_update(
        ...
        .await;
        Ok(match received {
            Ok(Ok(update)) => Self::classify_room_update(&update),
            // A timeout, a closed channel, or a lagged receiver are all "wait
            // again": none of them is evidence that the timeline moved, and
            // treating them as movement is what made a read receipt cost a
            // full refetch.
            _ => MatrixRoomUpdateKind::Idle,
        })
    }
    ...
    async fn list_member_page(
        ...
        let client = self.client().await?;
        let own_user_id = client.user_id().ok_or(BackendError::NotAuthenticated)?;
        let space_id = RoomId::parse(&community_id).map_err(Self::map_display_error)?;
        let space =
            ...
            .map(|member| {
                ...
                let online = joined
                    && (member.user_id() == own_user_id
                        || cached_presence_is_online(&presence, &user_id, presence_now));
                CommunityMember {
                    ...
                    // Already-synced room state, so this costs no extra
                    // request. The generated colour mark stays the fallback:
                    // avatar_color is still sent, and most members have no
                    // picture set at all.
    ...
    async fn invite_to_community(
        ...
        let user_id = if user_id.trim().starts_with('@') {
            UserId::parse(user_id.trim()).map_err(Self::map_display_error)?
        } else {
    ...
    /*
        Join rule and directory visibility are two separate choices and are kept
        that way here.

        They used to be one boolean: `discoverable` meant knock plus a public
        directory listing plus a mandatory canonical alias, and its absence meant
        invite-only. That made the product's own "Approval required" option
        inexpressible, because approval-required is a join rule and says nothing
        about whether the community is listed. Anyone asking for it without an
        alias got a hard rejection instead of a knock rule.

        The one real dependency is the other way round: a room cannot usefully be
        published to the directory without an address to publish, so
        `discoverable` still requires an alias. Knocking does not.
    */
    async fn update_community_access(
        ...
        let requested_join_rule = match join_rule.trim() {
            "invite" => JoinRule::Invite,
            "knock" => JoinRule::Knock,
            other => {
                return Err(BackendError::InvalidConfiguration(format!(
                    "unsupported community join rule: {other}"
                )))
            }
        };
        let client = self.client().await?;
        ...
        let parsed_alias = if let Some(alias) = alias {
            let alias = if alias.starts_with('#') {
                RoomAliasId::parse(alias).map_err(Self::map_display_error)?
            } else {
                let account_server = client
                    .user_id()
                    .ok_or(BackendError::NotAuthenticated)?
        ...
        /*
            Ordering is deliberate and differs by direction. Publishing sets the
            alias first, so the room is never listed in the directory without a
            resolvable address. Unpublishing drops the listing first, so the
            alias is never removed while the room is still published.
        */
        if discoverable {
            privacy
        ...
        privacy
            .update_join_rule(requested_join_rule.clone())
            .await
            .map_err(Self::map_error)?;
        ...
        Ok(CommunityAccessSettings {
            ...
            discoverable,
            join_rule: requested_join_rule.as_str().to_owned(),
        })
    }
    ...
    async fn search_person_directory(
        ...
        if term.is_empty() {
            return Ok(DirectoryPeopleResult {
                people: Vec::new(),
                truncated: false,
            });
        }
        const MAX_DIRECTORY_PEOPLE: usize = 20;
        let bounded_limit = limit.clamp(1, MAX_DIRECTORY_PEOPLE as u32);
        let mut request = search_users::v3::Request::new(term);
        request.limit = bounded_limit.into();
        ...
        let registration = self
            .search_operations
            .begin(&request_id, format!("{own_user_id}:people-directory"))
            .await?;
        let cancellation = registration.cancellation.clone();
        let _registration = registration;
        ...
        let search = async {
            let response = tokio::select! {
                biased;
                _ = cancellation.cancelled() => {
                    return Err(BackendError::Cancelled("people search was superseded".into()))
                }
                response = client.send(request) => response.map_err(Self::map_error)?,
            };
            let people = response
                .results
                .into_iter()
                // Nobody needs to be shown themselves as somebody to message.
                .filter(|user| user.user_id != own_user_id)
                .take(MAX_DIRECTORY_PEOPLE)
                .map(|user| DirectoryPerson {
                    avatar_color: Self::avatar_color(user.user_id.as_str()),
                    display_name: user
                        .display_name
                        .unwrap_or_else(|| user.user_id.localpart().to_owned()),
                    avatar_url: user.avatar_url.map(|url| url.to_string()),
                    user_id: user.user_id.to_string(),
                })
                .collect();
            Ok(DirectoryPeopleResult {
                people,
                truncated: response.limited,
            })
        };
        match tokio::time::timeout(
            Duration::from_millis(deadline_ms.clamp(250, MAX_SEARCH_DEADLINE_MS)),
        ...
        .await
        {
            Ok(result) => result,
            Err(_) => Err(BackendError::Cancelled(
                "people search reached its native deadline".into(),
            )),
        }
    }
    ...
    async fn knock_community(
        ...
        let identifier = RoomOrAliasId::parse(value).map_err(Self::map_display_error)?;
        let mut via = via
            .into_iter()
            .take(3)
            .map(ServerName::parse)
        ...
        let room_id = if value.starts_with('#') {
            let alias = RoomAliasId::parse(value).map_err(Self::map_display_error)?;
            let room_id = client
                .resolve_room_alias(&alias)
                .await
                .map_err(Self::map_error)?
            ...
            matrix_sdk::ruma::RoomId::parse(value).map_err(Self::map_display_error)?
        };
    ...
    async fn list_community_applications(
        ...
        for raw_event in response.chunk {
            let event = raw_event.deserialize().map_err(Self::map_display_error)?;
    ...
    async fn respond_community_application(
        ...
        let space_id =
            matrix_sdk::ruma::RoomId::parse(&community_id).map_err(Self::map_display_error)?;
        let user_id = matrix_sdk::ruma::UserId::parse(user_id).map_err(Self::map_display_error)?;
    ...
    async fn join_community(
        ...
        let client = self.client().await?;
        let identifier =
            RoomOrAliasId::parse(room_or_alias.trim()).map_err(Self::map_display_error)?;
        let via = via
            .into_iter()
            .take(3)
            .map(ServerName::parse)
            .collect::<Result<Vec<_>, _>>()
            .map_err(Self::map_display_error)?;
        // Prior membership decides whether a failed join may be rolled back. An
        // id-addressed target whose room is not already Joined, which is what
        // the invitation-accept path always sends, was provably joined by this
        // call and this call alone. An alias-addressed target's prior state is
        // unknowable here, so it is never rolled back.
        let prior_membership_known_absent =
            match matrix_sdk::ruma::RoomId::parse(identifier.as_str()) {
                Ok(id) => client
                    .get_room(&id)
        ...
        // Children this call newly joined, as opposed to ones that were
        // already open; only the former may be un-joined on failure.
        let mut newly_joined_child_ids = Vec::new();
        ...
        let validation = async {
            ...
            let space =
                Self::protected_joined_room(&client, space.room_id(), "joining this community")
                    .await?;
            for child_id in self.space_child_ids(&space).await? {
                ...
                if client.join_room_by_id(&child_id).await.is_ok() {
                    newly_joined_child_ids.push(child_id.clone());
                    opened_child_ids.push(child_id);
                }
            }
            client
        ...
        if let Err(error) = validation {
            // A policy rejection after the server-side join used to strand the
            // account inside a community Mesh then refused to show: the invite
            // row was gone, the rail quarantined the space, and no leave
            // affordance remained. Deterministic policy failures on rooms this
            // call created are rolled back; transient failures are not, so a
            // network blip can never un-join anything.
            let policy_rejection = matches!(
                error,
                BackendError::NotEncrypted(_) | BackendError::InvalidConfiguration(_)
            );
            if policy_rejection && prior_membership_known_absent {
                for child_id in newly_joined_child_ids.iter().rev() {
                        ...
                        if room.state() == matrix_sdk::RoomState::Joined {
                            if let Err(leave_error) = room.leave().await {
                                tracing::warn!(
                                    target: "mesh::security",
                                    room_id = %child_id,
                                    "Could not roll back a rejected community join: {leave_error}"
                                );
                            }
                        }
                    }
                }
                if let Err(leave_error) = space.leave().await {
                    tracing::warn!(
                        target: "mesh::security",
                        room_id = %space.room_id(),
                        "Could not roll back a rejected community join: {leave_error}"
        ...
        Ok(CommunityDto {
            ...
            avatar_url: space.avatar_url().map(|url| url.to_string()),
    ...
    /// Projects the invitations for this account to join a community.
    ///
    /// `list_communities` iterates joined rooms only, and the DM request
    /// surface explicitly discards spaces, so before this projection existed a
    /// space invitation from any client, including the invite Mesh itself
    /// sends when an administrator approves a join request, appeared nowhere.
    async fn community_invites(&self) -> BackendResult<Vec<CommunityInviteDto>> {
        let client = self.client().await?;
        let mut invited_spaces: Vec<Room> = client
            .invited_rooms()
            .into_iter()
            .filter(|room| room.is_space())
            .collect();
        invited_spaces.sort_by(|left, right| left.room_id().cmp(right.room_id()));
        if invited_spaces.len() > MAX_COMMUNITY_INVITES {
            tracing::warn!(
                target: "mesh::security",
                observed = invited_spaces.len(),
                retained = MAX_COMMUNITY_INVITES,
                "Bounded the incoming community invitation projection"
            );
            invited_spaces.truncate(MAX_COMMUNITY_INVITES);
        }

        // One account-data read covers every invitation. A per-invite read is a
        // full HTTP round trip each, which can stack past the listing's own
        // native deadline on a slow link.
        let ignored_users = self.refresh_ignored_user_list(&client).await?;

        let mut invites = Vec::new();
        for room in invited_spaces {
            let projection = async {
                ...
                if ignored_users.ignored_users.contains_key(&invite.inviter_id) {
                    return Ok(None);
                }
                ...
                // Mesh only joins end-to-end encrypted communities, and the
                // stripped invite state already carries the encryption event,
                // so the accept affordance can be honest before any join.
                // A failed read is not the same fact as an unencrypted room;
                // see the matching note on direct-message invitations.
                let can_accept = room
                    .latest_encryption_state()
                    .await
                    .map_err(|error| {
                        BackendError::InvalidConfiguration(format!(
                            "Mesh could not verify end-to-end encryption on community \
                             invitation {}: {error}. Resynchronize and try again",
                            room.room_id()
                        ))
                    })?
                    .is_encrypted();
                Ok(Some(CommunityInviteDto {
                    room_id: room.room_id().to_string(),
                    name: room.name().unwrap_or_else(|| "Unnamed community".into()),
            ...
            .await;
            match projection {
                Ok(Some(invite)) => invites.push(invite),
                Ok(None) => {}
                Err(error) => {
                    tracing::warn!(
                        target: "mesh::security",
                        room_id = %room.room_id(),
                        "Suppressed an invalid community invitation: {error}"
                    );
                }
            }
        }
    ...
    async fn decline_community_invite(&self, room_id: String) -> BackendResult<()> {
        let room_id = matrix_sdk::ruma::RoomId::parse(room_id).map_err(Self::map_display_error)?;
        let client = self.client().await?;
        let room = Self::validated_community_invite_room(&client, &room_id)?;
        room.leave().await.map_err(Self::map_error)
    }
    ...
    async fn enable_recovery(
        ...
        let result = match passphrase.as_deref() {
            Some(passphrase) => enable
                ...
                .await
                .map_err(Self::map_display_error),
            _ => enable.await.map_err(Self::map_display_error),
        };

...
impl Database {
    ...
    fn row_to_message(row: &rusqlite::Row) -> anyhow::Result<MessageDto> {
        ...
        Ok(MessageDto {
            ...
            // Not a stored column: the picture is whatever the room's member
            // state says now, so it is resolved when a message is projected
            // rather than frozen into the row when it was cached.

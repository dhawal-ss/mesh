            ...
            let result = conn.query_row(
                ...
                |row| {
                    Ok(CommunityDto {
                        ...
                        // The legacy store has no Matrix room state to read an
                        // icon from, and never had a column for one.
                        avatar_url: None,
                        role: row.get(3)?,
                        joined_at: row.get(4)?,
                        member_count: 0,
    ...
    pub fn get_channels(&self, community_id: &str) -> anyhow::Result<Vec<ChannelDto>> {
        ...
        let rows = stmt.query_map(params![community_id], |row| {
            Ok(ChannelDto {
                ...
                name: row.get(2)?,
                // The legacy channels table has no description column.
                topic: String::new(),
                channel_type: row.get(3)?,
                unread_count: 0,
                // The legacy channels table stores only joined rooms.
                joined: true,
            })
        })?;

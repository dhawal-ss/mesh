...
impl StartupFailure {
    ...
    pub fn from_unrecoverable_backend(error: &crate::backend::BackendError) -> Option<Self> {
        match error {
            crate::backend::BackendError::LocalStatePermissionDenied(_) => {
                Some(Self::new(StartupFailureKind::Permission))
            }
            crate::backend::BackendError::Crypto(_) => {
                Some(Self::new(StartupFailureKind::Keychain))
            }
            crate::backend::BackendError::Serialization(_) => {
                Some(Self::new(StartupFailureKind::CorruptState))
            }
            _ => None,
        }
    }
...
mod tests {
    ...
    #[test]
    fn only_local_state_failures_take_the_fatal_backend_path() {
        for error in [
            crate::backend::BackendError::LocalStatePermissionDenied("local store".into()),
            crate::backend::BackendError::Crypto("secure storage".into()),
            crate::backend::BackendError::Serialization("saved session".into()),
        ] {
            assert!(StartupFailure::from_unrecoverable_backend(&error).is_some());
        }

        for error in [
            crate::backend::BackendError::NotAuthenticated,
            crate::backend::BackendError::Network("offline".into()),
            crate::backend::BackendError::RateLimited("busy".into()),
            crate::backend::BackendError::PermissionDenied("remote service".into()),
            crate::backend::BackendError::InvalidConfiguration("service".into()),
            crate::backend::BackendError::Other("runtime".into()),
        ] {
            assert!(StartupFailure::from_unrecoverable_backend(&error).is_none());
        }
    }
}

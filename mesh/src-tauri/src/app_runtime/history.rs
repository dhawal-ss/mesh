...
async fn subscribe_dm_topics(app_handle: &AppHandle, handle: &NetworkHandle) {
    ...
    for conv in conversations {
        // Legacy P2P DMs are one-to-one, so the single peer is the topic key.
        let Some(peer) = conv.peers.first() else {
            continue;
        };
        let topic = crate::commands::dm::dm_topic(&our_key, &peer.user_id);
        let _ = handle

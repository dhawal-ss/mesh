...
use crate::backend::{
    BackendManager, BackendResult, BackendStartupPhase, BackendStartupStatus, MeshBackend,
};
#[cfg(feature = "legacy-p2p")]
...
pub struct BackendStartupCoordinator {
    gate: Mutex<()>,
    status: RwLock<BackendStartupStatus>,
}

impl BackendStartupCoordinator {
    pub fn new() -> Self {
        Self {
            gate: Mutex::new(()),
            status: RwLock::new(BackendStartupStatus::pending()),
        }
    }

    pub async fn status(&self) -> BackendStartupStatus {
    ...
    pub async fn ensure_started(
        ...
        retry: bool,
    ) -> BackendResult<BackendStartupStatus> {
        let _gate = self.gate.lock().await;
        let current = self.status().await;
        if current.phase == BackendStartupPhase::Ready
            || (current.phase == BackendStartupPhase::RecoverableFailure && !retry)
        {
            return Ok(current);
        }

        *self.status.write().await = BackendStartupStatus::starting();
        match backend.start().await {
            Ok(()) => {
                let status = BackendStartupStatus::ready();
                *self.status.write().await = status.clone();
                Ok(status)
            }
            Err(error) => {
                let Some(issue) = error.recoverable_startup_issue() else {
                    return Err(error);
                };
                let status = BackendStartupStatus::recoverable(issue);
                *self.status.write().await = status.clone();
                Ok(status)
            }
        }
...
impl Default for BackendStartupCoordinator {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct AppState {
    ...
    /// Durable startup status and retry serialization shared by the native
    /// setup task and the renderer recovery surface.
    pub backend_startup: Arc<BackendStartupCoordinator>,
    /// Sanitized startup markers are written below this already-resolved app
    /// data root. The path is never serialized to the renderer.
...
impl AppState {
    ...
    pub fn with_data_dir(app_data_dir: impl Into<std::path::PathBuf>) -> Self {
        ...
        Self {
            backend: BackendManager::from_environment(app_data_dir.clone()),
            backend_startup: Arc::new(BackendStartupCoordinator::default()),
            app_data_dir,

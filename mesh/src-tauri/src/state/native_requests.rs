...
impl NativeRequestState {
    /// Drop per-account and per-operation semaphores nobody is using.
    ///
    /// `begin` inserts one entry per `(account, operation)` pair it has ever
    /// admitted. The steady-state path in `finish` removes an entry when the
    /// last request in its scope completes, but an entry can still survive a
    /// cancelled or timed-out transition, and every entry belongs to an
    /// account that no longer exists once the account changes.
    ///
    /// A strong count of one means the map holds the only reference: no
    /// request is running, waiting to acquire, or holding a permit for that
    /// scope, so dropping the semaphore cannot change any live bound. The
    /// next request in that scope allocates a fresh one.
    fn prune_unused_limits(&mut self) {
        self.account_limits
            .retain(|_, limit| Arc::strong_count(limit) > 1);
...
impl Drop for NativeAccountTransitionGuard {
    fn drop(&mut self) {
        ...
        // The outgoing account's admission scopes are dead once the generation
        // moves. Reclaim them here so every transition path, including the
        // acknowledgement-timeout path, leaves the registry at its floor.
        state.prune_unused_limits();
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn account_transition_reclaims_idle_admission_scopes() {
        let registry = Arc::new(NativeRequestRegistry::default());
        let _ = registry
            .run(
                "transition-scope-1".into(),
                5_000,
                registry.account_generation(),
                "@alice:example.org".into(),
                "matrix_get_messages",
            ...
            .await
            .expect("the seeding read should be admitted");

        // Model a scope that outlived its request, which is what a cancelled
        // or timed-out transition can leave behind.
        {
            let mut state = registry.lock_state();
            state
                .account_limits
                .insert("@stale:example.org".into(), Arc::new(Semaphore::new(1)));
            state.operation_limits.insert(
                ("@stale:example.org".into(), "matrix_get_messages".into()),
        ...
        let guard = registry
            .close_account_admission()
            .expect("a transition should be admitted");
        let guard = registry
            .finish_account_transition(guard)
            .await
            .expect("an idle registry should settle immediately");
        drop(guard);

        let state = registry.lock_state();
        assert!(
            state.account_limits.is_empty(),
            "per-account admission scopes must not survive an account transition",
        );
        assert!(
            state.operation_limits.is_empty(),
            "per-operation admission scopes must not survive an account transition",
        );
    }

    #[test]
    fn pruning_never_drops_a_scope_that_is_still_referenced() {
        let registry = NativeRequestRegistry::default();
        let held = {
            let mut state = registry.lock_state();
            ...
            state
                .account_limits
                .insert("@alice:example.org".into(), Arc::clone(&limit));
            limit
        };

        registry.lock_state().prune_unused_limits();
        assert_eq!(
            registry.lock_state().account_limits.len(),
            1,
            "a scope with a live holder still bounds concurrency",
        );

        drop(held);
        registry.lock_state().prune_unused_limits();
        assert!(registry.lock_state().account_limits.is_empty());
    }
}

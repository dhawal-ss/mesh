...
use crate::backend::{
    ...
    MATRIX_COMMUNITIES_CHANGED_EVENT, MATRIX_IGNORED_USERS_CHANGED_EVENT,
    ...
    MATRIX_RTC_MEDIA_KEY_PAUSE_EVENT, MATRIX_RTC_MEMBERSHIP_EVENT, MATRIX_TYPING_CHANGED_EVENT,
    MATRIX_UNREAD_UPDATE_EVENT,
};
#[cfg(windows)]
use crate::backend::{MatrixNotificationActivation, MATRIX_NOTIFICATION_ACTIVATED_EVENT};
...
/// What the operating system will do with a notification Mesh tries to show.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum NotificationPermission {
    Granted,
    /// The OS will drop every notification. Mesh cannot change this from
    /// inside the app: the person has to allow Mesh in system settings.
    Denied,
    /// Never asked yet. Mesh requests it the first time it has something worth
    /// showing, so this is not a problem to report.
    NotRequested,
}
...
fn update_unread_indicators_for_count(app: &AppHandle, unread: u64) {
    ...
    if let Some(tray) = app.tray_by_id(TRAY_ID) {
        ...
        // The red unread dot is deliberately not a template image: it is an
        // attention signal, and macOS would otherwise recolor it to match
        // the idle icon and erase the one color that makes it read as new.
        let icon = if unread > 0 {
            Some(unread_indicator_image())
        } else {
            idle_tray_icon(app)
        };
        if let Err(error) = tray.set_icon_with_as_template(icon, unread == 0) {
...
const TRAY_OPEN_ITEM: &str = "mesh-tray-open";
const TRAY_QUIT_ITEM: &str = "mesh-tray-quit";

/// Brings the main window back from wherever the person left it.
///
/// Minimized, hidden behind other windows, or merely unfocused are three
/// different states and each needs its own call, which is why this is one
/// helper rather than three call sites that each remember two of the three.
pub fn focus_main_window(app: &AppHandle) {
    let Some(window) = app.get_webview_window("main") else {
        tracing::warn!(target: "mesh::notifications", "No main window to focus");
        return;
    };
    if let Err(error) = window.unminimize() {
        tracing::warn!(target: "mesh::notifications", "Could not unminimize Mesh: {error}");
    }
    if let Err(error) = window.show() {
        tracing::warn!(target: "mesh::notifications", "Could not show Mesh: {error}");
    }
    if let Err(error) = window.set_focus() {
        tracing::warn!(target: "mesh::notifications", "Could not focus Mesh: {error}");
    }
}

/*
  macOS menu-bar extras are expected to be "template" images: a black shape
  on a transparent background that the system recolors for light/dark
  appearance and for the selected/pressed state. Handing it the full-color
  app icon instead renders as a flat, non-adapting color square -- the tell
  that an app skipped this convention. This asset is that same mark with
  its alpha channel kept and every opaque pixel forced to black, so no
  shape information is lost; Windows and Linux keep the full-color icon,
  which is their own platform convention.
*/
#[cfg(target_os = "macos")]
fn macos_tray_template_icon() -> Option<Image<'static>> {
    match Image::from_bytes(include_bytes!("../../icons/tray-template-macos.png")) {
        Ok(icon) => Some(icon),
        Err(error) => {
            tracing::warn!(target: "mesh::notifications", "Could not decode the macOS tray template icon: {error}");
            None
        }
    }
}

/// The tray's idle-state (no unread) icon. See `macos_tray_template_icon`.
fn idle_tray_icon(_app: &AppHandle) -> Option<Image<'static>> {
    #[cfg(target_os = "macos")]
    {
        macos_tray_template_icon()
    }
...
/*
  The tray had an icon and a tooltip and did nothing at all: no menu, no click
  handler. An icon that does not respond to being clicked is not an affordance,
  it is a decoration that looks like one.

  Two items, because there are only two things a tray can honestly offer while
  closing the window still exits: come back, and leave. Close-to-tray is a
  different feature with its own lifecycle to test, and it is not this one.
*/
pub fn configure_tray(app: &mut App) {
    let Some(icon) = idle_tray_icon(app.handle()) else {
        tracing::warn!(
    ...
    let menu = match (
        MenuItem::with_id(app, TRAY_OPEN_ITEM, "Open Mesh", true, None::<&str>),
        MenuItem::with_id(app, TRAY_QUIT_ITEM, "Quit Mesh", true, None::<&str>),
    ) {
        (Ok(open), Ok(quit)) => match PredefinedMenuItem::separator(app)
            .and_then(|separator| Menu::with_items(app, &[&open, &separator, &quit]))
        {
            Ok(menu) => Some(menu),
            Err(error) => {
                tracing::warn!(target: "mesh::notifications", "Could not build the tray menu: {error}");
                None
            }
        },
        (open, quit) => {
            let error = open.err().or(quit.err());
            tracing::warn!(target: "mesh::notifications", "Could not build a tray menu item: {error:?}");
            None
        }
    };
    ...
    let mut builder = TrayIconBuilder::with_id(TRAY_ID)
        ...
        // the menu. Showing the menu on left click would take the cheap gesture
        // and spend it on the rarer intent.
        .show_menu_on_left_click(false)
        .on_menu_event(|app, event| match event.id().as_ref() {
            TRAY_OPEN_ITEM => focus_main_window(app),
            TRAY_QUIT_ITEM => app.exit(0),
            other => {
                tracing::warn!(target: "mesh::notifications", "Unknown tray menu item: {other}")
            }
        })
...
/*
  Tauri v2 creates no application menu by default (v1 did). On macOS, the
  Edit menu is also where Cmd+C/Cmd+V/Cmd+Z/Cmd+A key equivalents for the
  WKWebView get registered, so going without one risks silently inert
  copy/paste/undo in every text field, not just a missing menu bar.

  This is built by hand instead of `tauri::menu::Menu::default`, whose
  macOS output has a File submenu holding only "Close Window" and a Help
  submenu with no items at all -- both empty-looking stubs. Windows
  (WebView2) and Linux (WebKitGTK) do not share the key-equivalent
  requirement, so this stays macOS-only; see its call site in lib.rs.
*/
#[cfg(target_os = "macos")]
fn build_macos_menu(app: &mut App) -> tauri::Result<Menu<tauri::Wry>> {
...
/// Sets Mesh's macOS menu bar. See `build_macos_menu` for why this exists.
#[cfg(target_os = "macos")]
pub fn configure_macos_menu(app: &mut App) {
    match build_macos_menu(app) {
        Ok(menu) => {
            if let Err(error) = app.set_menu(menu) {
                tracing::warn!(target: "mesh::notifications", "Could not set the macOS application menu: {error}");
            }
        }
        Err(error) => {
            tracing::warn!(target: "mesh::notifications", "Could not build the macOS application menu: {error}");
        }
    }
}
...
pub fn handle_matrix_backend_event(app: &AppHandle, event: MatrixBackendEvent) {
    ...
    match event {
        ...
        MatrixBackendEvent::CommunitiesChanged => {
            if let Err(error) = app.emit(MATRIX_COMMUNITIES_CHANGED_EVENT, ()) {
                tracing::warn!(
                    target: "mesh::notifications",
                    "Could not emit Matrix communities changed event: {error}"
                );
            }
        }
        MatrixBackendEvent::TypingChanged(change) => {
            if let Err(error) = app.emit(MATRIX_TYPING_CHANGED_EVENT, &change) {
                tracing::warn!(
                    target: "mesh::matrix",
                    "Could not emit typing update: {error}"
                );
            }
        }
...
/*
  A notification you cannot click is a dead end.

  For a chat app running in the background, the toast is the way back in: it is
  the only surface that says which conversation wants you, and clicking it
  should land you there. Mesh's did nothing at all.

  It could not, through the plugin. `tauri_plugin_notification`'s desktop
  `show()` builds a `notify_rust::Notification`, calls `show()`, and drops the
  handle; the activation callbacks in that crate are Linux-only, and its action
  API is mobile-only. So on Windows the toast is built directly on
  `tauri-winrt-notification`, which is the same crate the plugin already pulls
  in through notify-rust and the same version already in the lock file. The
  plugin path stays for every other target and as the fallback when the WinRT
  toast cannot be shown.

  The application identifier is the one the plugin uses, so the toast is still
  attributed to Mesh by the shortcut the installer registers.
*/
fn show_native_notification(app: &AppHandle, notification: &MatrixNotification) {
    ...
    let title = private_notification_title(&notification.display_name);

    #[cfg(windows)]
    if show_activatable_toast(app, &title, notification) {
        return;
    }
...
/// Raises the window and tells the renderer which room was asked for.
#[cfg(windows)]
fn activate_notification(app: &AppHandle, activation: &MatrixNotificationActivation) {
    focus_main_window(app);
    if let Err(error) = app.emit(MATRIX_NOTIFICATION_ACTIVATED_EVENT, activation) {
        tracing::warn!(
            target: "mesh::notifications",
            room_id = activation.room_id,
            "Could not route a notification click: {error}"
        );
    }
}

/// Shows a Windows toast that responds to being clicked. False means the
/// caller should fall back to the plugin.
#[cfg(windows)]
fn show_activatable_toast(app: &AppHandle, title: &str, notification: &MatrixNotification) -> bool {
    let activation = MatrixNotificationActivation {
        room_id: notification.room_id.clone(),
        event_id: notification.event_id.clone(),
        is_dm: notification.is_dm,
    };
    let handle = app.clone();
    let result = tauri_winrt_notification::Toast::new(&app.config().identifier)
        .title(title)
        .text1(&notification.preview)
        .on_activated(move |_argument| {
            activate_notification(&handle, &activation);
            Ok(())
        })
        .show();
    match result {
        ...
        Err(error) => {
            tracing::warn!(
                target: "mesh::notifications",
                room_id = notification.room_id,
                "Could not show an activatable toast, falling back: {error}"
            );
            false
        }
    }
...
/// Whether the operating system will actually deliver a notification.
///
/// Mesh's own "Desktop notifications" switch only records what the user asked
/// for. If the OS has denied the permission, every notification is dropped in
/// silence and the switch still reads as on, so the settings panel needs the
/// real state to say so. Deliberately read-only: it never prompts, because
/// opening a system permission dialog from a settings screen the user is only
/// reading would be a nag.
#[tauri::command]
pub fn notification_permission_state(
    app: AppHandle,
) -> Result<NotificationPermission, CommandError> {
    let state = app
        .notification()
        .permission_state()
    ...
    Ok(match state {
        ...
        // Not yet asked. Mesh requests the permission the first time it has a
        // notification worth showing, so this is not something to act on here.

...
use crate::backend::{
    BackendError, BackendKind, BackendStartupStatus, BackendStatus, CommunityAccessResult,
    CommunityAccessSettings, CommunityApplication, CommunityDirectoryEntry, CommunityMemberPage,
    CommunityModerationResult, CommunityPermissionProjection, CustomEmoji, DirectoryPeopleResult,
    MatrixAccount, MatrixAttachmentSendRequest, MatrixDevice, MatrixLogin, MatrixOidcStatus,
    MatrixPersonalDataExport, MatrixProfile, MatrixRecoveryHealth, MatrixRecoverySetupResult,
    MatrixRegistration, MatrixRoomNotificationMode, MatrixRoomPins, MatrixRoomUpdateKind,
...
pub(super) fn map_error(error: BackendError) -> CommandError {
    match error {
        ...
        BackendError::LocalStatePermissionDenied(_) => {
            CommandError::PermissionDenied(error.to_string())
        }
        BackendError::NotFound(_) => CommandError::NotFound(error.to_string()),
        BackendError::AccountNotFound(_) => CommandError::AccountNotFound(error.to_string()),
...
#[tauri::command]
pub async fn ensure_backend_started(
    retry: bool,
    app: AppHandle,
    state: State<'_, AppState>,
) -> Result<BackendStartupStatus, CommandError> {
    match state
        .backend_startup
        .ensure_started(state.backend.backend(), retry)
        .await
    {
        Ok(status) => Ok(status),
        Err(error) => {
            if let Some(failure) =
                crate::startup_recovery::StartupFailure::from_unrecoverable_backend(&error)
            {
                crate::startup_recovery::report_handle_and_exit(&app, &state.app_data_dir, failure);
                return Err(CommandError::Other(
                    "Mesh is closing to protect unreadable local account state".into(),
                ));
            }
            Err(map_error(error))
        }
...
#[tauri::command]
pub async fn matrix_update_profile_avatar(
    filename: String,
    content_type: String,
    bytes: Vec<u8>,
    state: State<'_, AppState>,
    ...
    state
        .backend
        .backend()
        .update_profile_avatar(filename, content_type, bytes)
        .await
        .map_err(map_error)
}

#[tauri::command]
pub async fn matrix_clear_profile_avatar(
    state: State<'_, AppState>,
) -> Result<MatrixProfile, CommandError> {
    require_matrix(&state)?;
    let _account_guard = begin_current_account_mutation(&state)?;
...
/*
    Sets a community's icon.

    Two separate concerns, and it is easy to mistake one for the other. Who may
    change the image is decided by power levels inside `set_room_avatar_image`,
    where Rust is the authority and the renderer's `role` field is only what
    hides the control. `begin_current_account_mutation` answers a different
    question: this uploads media and writes room state under one account's
    client, so it must not straddle an account switch. Without the guard the
    upload would keep running after a transition and land as work attributed to
    an account that no longer owns the request, which is why
    `account_scoped_commands_are_transition_safe_or_explicit_cancellations`
    requires every `matrix_*` command here to hold one or to be a read with a
...
pub async fn matrix_set_room_avatar(
    ...
    state
        .backend
        .backend()
        .set_room_avatar(room_id, filename, content_type, bytes)
        .await
        .map_err(map_error)
}

#[tauri::command]
pub async fn matrix_clear_room_avatar(
    room_id: String,
    state: State<'_, AppState>,
) -> Result<(), CommandError> {
    require_matrix(&state)?;
    ...
    state
        .backend
        .backend()
        .clear_room_avatar(room_id)
        .await
        .map_err(map_error)
}

#[tauri::command]
pub async fn matrix_load_profile_avatar(
    avatar_url: String,
    request_id: String,
    deadline_ms: u64,
    state: State<'_, AppState>,
    ...
    require_matrix(&state)?;
    let backend = state.backend.backend();
    // Registered like every other media read. Without this the fetch outlives
    // an account switch, and the bytes it returns are a picture belonging to
    ...
    run_native_read(
        ...
        async move {
            backend
                .load_profile_avatar(avatar_url)
                .await
                .map_err(map_error)
        },
    )
...
pub async fn matrix_create_community(
    ...
    Ok(MatrixCreatedCommunity {
        community: CommunityDto {
            ...
            name,
            description,
            // A community is created without an icon; the owner sets one
            // afterwards from community settings.
            avatar_url: None,
            member_count: 1,
            role: "owner".into(),
        ...
        channel: ChannelDto {
            ...
            unread_count: 0,
            // Creating a community joins its first room.
            joined: true,
        },
    })
}
...
#[tauri::command]
pub async fn matrix_community_invites(
    request_id: String,
    deadline_ms: u64,
    state: State<'_, AppState>,
) -> Result<Vec<CommunityInviteDto>, CommandError> {
    require_matrix(&state)?;
    let backend = state.backend.backend();
    run_native_read(
        &state,
        ...
        async move { backend.community_invites().await.map_err(map_error) },
    )
    .await
}

#[tauri::command]
pub async fn matrix_decline_community_invite(
    room_id: String,
    state: State<'_, AppState>,
) -> Result<(), CommandError> {
    require_matrix(&state)?;
    ...
    state
        .backend
        .backend()
        .decline_community_invite(room_id)
        .await
        .map_err(map_error)
}
...
#[tauri::command]
pub async fn matrix_join_community_channel(
    community_id: String,
    channel_id: String,
    state: State<'_, AppState>,
) -> Result<ChannelDto, CommandError> {
    ...
    state
        .backend
        .backend()
        .join_community_channel(community_id, channel_id)
        .await
        .map_err(map_error)
}
...
pub async fn matrix_update_channel(
    ...
    state
        .backend
        .backend()
        .update_channel(community_id, channel_id, name, topic)
        .await
        .map_err(map_error)
}

#[tauri::command]
pub async fn matrix_remove_channel(
    community_id: String,
    channel_id: String,
    state: State<'_, AppState>,
) -> Result<(), CommandError> {
    ...
    state
        .backend
        .backend()
        .remove_channel(community_id, channel_id)
        .await
        .map_err(map_error)
}
...
pub async fn matrix_send_message(
    ...
    // Whether this message asks to notify the whole room. The backend decides
    // whether the account may: a renderer that offers the affordance to
    // somebody who has since lost the power level gets a refusal rather than
    // a message that quietly notified nobody.
...
pub async fn matrix_load_composer_draft(
    ...
    deadline_ms: u64,
    state: State<'_, AppState>,
) -> Result<Option<ComposerDraftDto>, CommandError> {
    require_matrix(&state)?;
    let backend = state.backend.backend();
    run_native_read(
        &state,
...
pub async fn matrix_thread_list(
    ...
    deadline_ms: u64,
    state: State<'_, AppState>,
) -> Result<MatrixThreadListDto, CommandError> {
    require_matrix(&state)?;
    if room_id.is_empty() || room_id.len() > 255 {
        return Err(CommandError::Validation(
            "Choose a valid room to list threads for.".into(),
        ));
    }
    let backend = state.backend.backend();
    run_native_read(
        ...
        deadline_ms,
        "matrix_thread_list",
        async move { backend.thread_list(room_id).await.map_err(map_error) },
    )
    .await
}
...
pub async fn matrix_edit_message(
    ...
    state
        .backend
        .backend()
        .edit_message(room_id, event_id, body, mentions.unwrap_or_default())
        .await
        .map_err(map_error)
}
...
#[tauri::command]
pub async fn matrix_set_unread_flag(
    room_id: String,
    unread: bool,
    state: State<'_, AppState>,
) -> Result<(), CommandError> {
    ...
    state
        .backend
        .backend()
        .set_unread_flag(room_id, unread)
        .await
        .map_err(map_error)
}
...
pub async fn matrix_search_messages(
    ...
    request_id: String,
    deadline_ms: u64,
    filters: Option<MessageSearchFilters>,
    state: State<'_, AppState>,
) -> Result<Vec<MessageDto>, CommandError> {
    require_matrix(&state)?;
    ...
    run_native_read(
        ...
        async move {
            backend
                .search_messages(community_id, terms, limit, request_id, deadline_ms, filters)
                .await
                .map_err(map_error)
        },
    )
...
#[tauri::command]
pub async fn matrix_search_messages_everywhere(
    ...
    filters: Option<MessageSearchFilters>,
    state: State<'_, AppState>,
) -> Result<MatrixCrossCommunitySearchResultDto, CommandError> {
    require_matrix(&state)?;
    let filters = filters.unwrap_or_default();
    let native_request_id = request_id.clone();
    let backend = state.backend.backend();
...
pub async fn matrix_wait_for_room_update(
    ...
    deadline_ms: u64,
    state: State<'_, AppState>,
) -> Result<MatrixRoomUpdateKind, CommandError> {
    require_matrix(&state)?;
...
pub async fn matrix_update_community_access(
    ...
    state
        .backend
        .backend()
        .update_community_access(community_id, alias, discoverable, join_rule)
        .await
        .map_err(map_error)
}
...
#[tauri::command]
pub async fn matrix_search_person_directory(
    query: String,
    limit: u32,
    request_id: String,
    deadline_ms: u64,
    state: State<'_, AppState>,
) -> Result<DirectoryPeopleResult, CommandError> {
    require_matrix(&state)?;
    let native_request_id = request_id.clone();
    let backend = state.backend.backend();
    run_native_read(
        ...
        native_request_id,
        deadline_ms,
        "matrix_search_person_directory".to_owned(),
        async move {
            backend
                .search_person_directory(query, limit, request_id, deadline_ms)
                .await
                .map_err(map_error)
        },
    )
...
#[tauri::command]
pub async fn matrix_unban_member(
    community_id: String,
    user_id: String,
    reason: Option<String>,
    state: State<'_, AppState>,
    ...
    state
        .backend
        .backend()
        .unban_member(community_id, user_id, reason)
        .await
        .map_err(map_error)
}

...
impl AttachmentGrantStore {
    ...
    pub async fn claim_to_staging(
        ...
        let grant = {
            ...
            grants.remove(token).ok_or_else(|| {
                CommandError::Cancelled(
                    "Attachment access changed while it was being checked; choose the file again"
                        .into(),
                )
            })?
        };
    ...
    pub async fn claim_custom_emoji(
        ...
        let grant = {
            ...
            grants.remove(token).ok_or_else(|| {
                CommandError::Cancelled(
                    "Image access changed while it was being checked; choose the image again"
                        .into(),
                )
            })?
        };
    ...
    if classification.disposition != AttachmentDisposition::Safe {
        return Err(CommandError::Validation(match classification.disposition {
            ...
            AttachmentDisposition::Safe => {
                "Mesh could not confirm that this attachment is safe to open directly. Save it only if you trust the sender."
                    .into()
            }
        }));
    }

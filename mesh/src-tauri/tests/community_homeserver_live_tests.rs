...
/// Real operator-selected community-service authentication check.
///
...
///   --test community_homeserver_live_tests -- --ignored
#[tokio::test]
#[ignore = "requires an operator-selected community homeserver and test credentials"]
async fn community_homeserver_accepts_password_login_and_initial_sync() {
    let server_name =
        std::env::var("MESH_TEST_SERVER_NAME").expect("MESH_TEST_SERVER_NAME must be provided");
    ...
    let client = builder
        .build()
        .await
        .expect("community homeserver discovery must succeed");

    client
        .matrix_auth()
        .login_username(&username, &password)
        .initial_device_display_name("Mesh community-service verification")
        .send()
        .await
        .expect("community homeserver password login must succeed");

    let user_id = client
        .user_id()
        .expect("successful login must install a Matrix session");
    assert_eq!(user_id.localpart(), username);
    assert_eq!(user_id.server_name().as_str(), server_name);

...
use mesh_lib::backend::{
    ...
    MatrixRoomUpdateKind, MeshBackend, ReadReceiptMode, UserPreferences,
};
use mesh_lib::types::message::MessageSearchFilters;
...
fn sanitized_backend_error(error: &BackendError) -> &'static str {
    match error {
        ...
        BackendError::LocalStatePermissionDenied(_) => "local-state-permission-denied",
        BackendError::NotFound(_) => "not-found",
        BackendError::AccountNotFound(_) => "account-not-found",
...
fn privacy_preferences_with_receipt_mode(
    ...
    UserPreferences {
        ...
        interface_sound_volume: 60,
        interface_sound_events: std::collections::BTreeMap::new(),
...
async fn run_matrix_backend_federates_and_recovers_offline_history_once() {
    ...
    let search_results = alice
        .search_messages(
            community.space_id.clone(),
            vec![edited_body.clone()],
            20,
            format!("federation-message-{nonce}"),
            10_000,
            // This case exercises free-text federation search; the operator
            // filters have their own coverage in the backend unit tests.
    ...
    // A message arriving must be reported as a timeline change specifically,
    // not merely as "something happened": the renderer refetches on that
    // distinction, so anything weaker here would let a regression through.
    assert_eq!(updated.unwrap(), MatrixRoomUpdateKind::Timeline);
    sent.unwrap();
    ...
    let second_device_dm = bob_second
        ...
        .find(|conversation| {
            conversation
                .peers
                .iter()
                .any(|peer| peer.user_id == alice_user)
        })

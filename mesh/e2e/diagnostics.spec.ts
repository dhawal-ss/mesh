...
async function openAdvancedDiagnostics(
  ...
  const signalCheck = settings.getByRole('checkbox', { name: 'Show connection check' })
  if (!(await signalCheck.isChecked())) await signalCheck.check()
  const diagnosticsButton = settings.getByRole('button', { name: 'Review connection check' })
  await expect(diagnosticsButton).toBeVisible()
  await diagnosticsButton.click()
  const surface = settings.getByRole('region', { name: 'Connection check' })
  await expect(surface).toBeVisible()
  await expect(page.getByRole('dialog', { name: 'Connection check' })).toHaveCount(0)
...
test.describe('diagnostics panel E2E', () => {
  ...
  test('Connection check opens inline from a saved contextual Advanced route', async ({ page }) => {
    await installTauriMock(page)
    await page.goto('/')
    await page.waitForLoadState('networkidle')
  ...
  test('diagnostics panel shows mocked connection count and backup warning', async ({ page }) => {
    await installTauriMock(page)
    await page.goto('/')
    await page.waitForLoadState('networkidle')
    ...
    // The missing backup path stays actionable without exposing transport vocabulary.
    await expect(
      page.getByText(
        'A backup call connection is not configured. Some calls may fail. Ask the community operator to check the call service.',
        { exact: true },
      ),
    ).toBeVisible()
  })
  ...
  test('is accessible in a narrow window and restores focus after Escape', async ({ page }) => {
    ...
    await expect(surface.getByRole('button', { name: 'Refresh connection check' })).toBeVisible()
    await expectNoWcagViolations(page, 'Narrow inline Connection check')
  ...
  test('clicking Run probe triggers the probe and renders results', async ({ page }) => {
    ...
    const probeButton = page.getByRole('button', { name: 'Run connection test' })
    await probeButton.click()

    // The mocked probe returns outcome=ok and the UI translates it to Available.
  ...
  test('auth_rejected outcome renders with error styling', async ({ page }) => {
    ...
    await expect(page.getByText('Sign-in rejected', { exact: true })).toBeVisible({ timeout: 3000 })
  ...
  test('allocation_ok outcome renders with success styling', async ({ page }) => {
    ...
    await expect(page.getByText('Backup connection available', { exact: true })).toBeVisible({
      timeout: 3000,
    })

...
test('keeps the default onboarding path within wider-beta runtime budgets', async ({ page }) => {
  ...
  for (let run = 1; run <= SAMPLE_RUNS; run += 1) {
    ...
    await page.goto('/')
    // The first control on the first screen, which is now the new-or-returning
    // question rather than a service list.
    const signIn = page.getByRole('button', { name: 'I already have an account' })
  ...
  const evidence = {
    ...
    buildType: process.env.MESH_RUNTIME_BUILD_TYPE
      ?? (process.env.CI ? 'vite-development-ci' : 'vite-development-local'),
  ...
  // The transfer ceiling is calibrated against shipped assets, and the
  // production-preview harness (playwright.performance.config.ts) is where it
  // is enforced. The default suite serves unminified dev transforms, whose
  // cold-cache transfer tracks source size rather than the product: the ceiling
  // became arithmetically unreachable there the moment the source grew, while
  // the shipped bundle stayed at roughly half the budget under its own gate
  // (scripts/check-bundle-size.mjs). Paint, readiness, long-task, DOM, and heap
  // budgets stay enforced in every mode.
  const enforceTransferBudget =

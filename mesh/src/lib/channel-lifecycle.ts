...
export function evaluateChannelLifecycle({
  ...
  if (policy.mode === 'restart-scoped') {
    return {
      ...
      action: 'reject',
      nextState: snapshot.state,
      reason: 'Restart-based rooms are not supported.',
    }
  }
  if (
    recover
    ...
    return {
      ...
      action: 'recover',
      nextState: 'active',
      reason: 'An owner can restore this archived room while its history remains available.',
    }
  }
  ...
  if (snapshot.state === 'archived' || snapshot.state === 'tombstoned') {
    return {
      ...
      action: 'none',
      nextState: snapshot.state,
      reason: 'The room is already archived or unavailable.',
    }
  }
  ...
  if (!Number.isFinite(emptyAt)) {
    return {
      ...
      reason: 'The room became empty and its waiting period has started.',
    }
  }
  ...
  return {
    ...
    reason: snapshot.federated
      ? 'Archive the room and publish its replacement. Existing history may remain available.'
...
export function evaluateTemporaryChannelCreation(
  ...
  if (!authority.canCreateChild) {
    return { allowed: false, reason: 'Your current permissions do not allow room creation.' }
  }
  ...
  if (
    ...
    return { allowed: false, reason: 'Creating this room would grant additional permissions.' }
  }

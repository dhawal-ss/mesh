...
import { describeError, errorLine } from './errors'
import {
  buildPeerFromMember,
...
export class VoiceEngine {
  ...
  async start(): Promise<VoiceSessionSnapshot | null> {
    ...
    if (!this.localPublicKey) {
      this.handlers.onError?.('This device is not ready for voice')
      this.handlers.onConnectionState?.('disconnected', 'account unavailable')
      return null
    }
    ...
    try {
      ...
      this.handlers.onConnectionWarning?.(errorLine(description))
    ...
    const snapshot = await joinVoice(this.communityId, this.channelId).catch((error) => {
      ...
      const message = errorLine(description)

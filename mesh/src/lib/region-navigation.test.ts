...
describe('nextMeshRegion', () => {
  ...
  it('leaves the innermost region when regions nest', () => {
    // `main#mesh-conversation` contains the composer, so document order is
    // container first, nested region second. Forward from the composer has to
    // continue past the container, not back into it.
    const rooms = region('Rooms')
    const conversation = region('Conversation')
    const composer = region('Message composer')
    conversation.appendChild(composer)
    const field = document.createElement('textarea')
    composer.appendChild(field)
    const regions = [rooms, conversation, composer]

    expect(nextMeshRegion(regions, field)).toBe(rooms)
    expect(nextMeshRegion(regions, composer)).toBe(rooms)
    expect(nextMeshRegion(regions, field, true)).toBe(conversation)
  })

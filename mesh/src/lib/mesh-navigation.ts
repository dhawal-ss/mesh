...
export type CommunitiesMode = 'join' | 'create'
export type DetailsTab = 'people' | 'pins' | 'files' | 'threads'
export type YouSection =
  ...
  | 'audio-video'
  | 'privacy'
  /** @deprecated Kept only so saved beta navigation opens safely after the settings split. */
  | 'privacy-voice'
  | 'safety-devices'
  | 'beta'
  | 'advanced'
...
export type MeshPane =
  ...
  | { kind: 'thread'; rootEventId: string; from?: 'threads-list' }
  | { kind: 'safety' }
...
const COMMUNITIES_MODES = new Set<CommunitiesMode>(['join', 'create'])
const DETAILS_TABS = new Set<DetailsTab>(['people', 'pins', 'files', 'threads'])
...
function isMeshPane(value: unknown): value is MeshPane {
  ...
  if (pane.kind === 'thread') {
    return isBoundedId(pane.rootEventId) && (pane.from === undefined || pane.from === 'threads-list')
  }
...
export function isMeshRoute(value: unknown): value is MeshRoute {
  ...
  if (route.kind === 'inbox') return true
  ...
  if (route.kind === 'direct-list') return true
...
export function meshRouteKey(route: MeshRoute): string {
  ...
  if (route.kind === 'direct-list') return 'direct-list'

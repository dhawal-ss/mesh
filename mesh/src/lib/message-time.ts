...
const shortDateFormatter = formatter({ month: 'short', day: 'numeric' })
const shortDateTimeFormatter = formatter({
  month: 'short',
  day: 'numeric',
  hour: 'numeric',
  minute: '2-digit',
...
/**
 * Compact calendar date for dense lists and result rows, e.g. "Jul 25". Month
 * and day order follow the reader's locale rather than a fixed pattern.
 */
export function formatShortDate(value: unknown): string {
  const parsed = parseFederatedTimestamp(value)
  if (!parsed) return INVALID_FEDERATED_TIME_LABEL
  return shortDateFormatter?.format(parsed) ?? INVALID_FEDERATED_TIME_LABEL
}

/**
 * Compact date and clock reading for search results, thread summaries and
 * room context rows, e.g. "Jul 25, 12:34 PM". The clock half respects the
 * reader's 12/24-hour preference.
 */
export function formatShortDateTime(value: unknown): string {
  const parsed = parseFederatedTimestamp(value)
  if (!parsed) return INVALID_FEDERATED_TIME_LABEL
  return shortDateTimeFormatter?.format(parsed) ?? INVALID_FEDERATED_TIME_LABEL
}

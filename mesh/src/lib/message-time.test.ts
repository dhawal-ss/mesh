...
describe('message-time', () => {
  ...
  describe('formatShortDate', () => {
    it('reads a compact month and day', () => {
      expect(formatShortDate(new Date(2026, 6, 25, 12, 34, 0))).toBe('Jul 25')
    })

    it('degrades safely on unusable input', () => {
      expect(formatShortDate('garbage')).toBe(INVALID_FEDERATED_TIME_LABEL)
      expect(formatShortDate(undefined)).toBe(INVALID_FEDERATED_TIME_LABEL)
    })
  })

  describe('formatShortDateTime', () => {
    it('reads the date and the clock together', () => {
      const reading = formatShortDateTime(new Date(2026, 6, 25, 13, 34, 0))
      expect(reading).toContain('Jul 25')
      expect(reading).toMatch(/\d{1,2}:34/)
    })

    it('degrades safely on unusable input', () => {
      expect(formatShortDateTime('garbage')).toBe(INVALID_FEDERATED_TIME_LABEL)
    })
  })
})

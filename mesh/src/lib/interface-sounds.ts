...
/**
 * Both collections are module-scoped and previously grew for the life of the
 * process: one entry per `sound:roomId` pair and one per failed send. The
 * caps below are far above any realistic repetition window, and eviction is
 * FIFO because the oldest entry is also the one whose cooldown has long since
 * elapsed.
 */
const MAX_TRACKED_MESSAGE_CUES = 256
const MAX_TRACKED_FAILURE_KEYS = 256

function rememberMessageCue(context: string, now: number): void {
  if (!recentMessageCues.has(context) && recentMessageCues.size >= MAX_TRACKED_MESSAGE_CUES) {
    const oldest = recentMessageCues.keys().next()
    if (!oldest.done) recentMessageCues.delete(oldest.value)
  }
  recentMessageCues.set(context, now)
}

function rememberFailureKey(failureKey: string): void {
  if (!playedFailureKeys.has(failureKey) && playedFailureKeys.size >= MAX_TRACKED_FAILURE_KEYS) {
    const oldest = playedFailureKeys.values().next()
    if (!oldest.done) playedFailureKeys.delete(oldest.value)
  }
  playedFailureKeys.add(failureKey)
}
...
function allowedByRepetitionPolicy(
  ...
  if (sound === 'message-mention' || sound === 'message-direct') {
    ...
    rememberMessageCue(context, now)
  }
  ...
  if (sound === 'message-failed') {
    ...
    if (options.failureKey) rememberFailureKey(options.failureKey)
  }
  return true
}

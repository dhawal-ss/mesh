...
describe('normalizeError', () => {
  ...
  it('classifies an expired saved sign-in without exposing it as an unexpected failure', () => {
...
describe('describeErrorLine', () => {
  const CODES = [
    'not_authenticated', 'network', 'timeout', 'rate_limited', 'forbidden',
    'not_found', 'conflict', 'invalid', 'unknown', 'encryption_unavailable',
  ]

  /*
    The title and the body used to be concatenated by hand at eight call sites,
    and with no operation in context both halves fell back to nearly the same
    sentence: "Mesh could not finish the request. Mesh could not finish that
    request. Try again." A person read the same failure twice before reaching
    the one clause that told them what to do.
  */
  it('never says the same thing twice', () => {
    for (const code of CODES) {
      const line = describeErrorLine(code)
      const [first, ...rest] = line.split('. ')
      expect(rest.join('. ')).not.toContain(first)
      expect(line).not.toMatch(/could not finish the request\. Mesh could not finish/)
    }
  })

  it('stays inside the copy limit the density gate enforces', () => {
    for (const code of CODES) {
      const words = describeErrorLine(code).split(/\s+/).filter(Boolean).length
      expect(words, `${code} renders ${words} words`).toBeLessThanOrEqual(16)
    }
  })

  it('keeps the operation when a caller names one', () => {
    const line = describeErrorLine('network', { operation: 'send that message' })
    expect(line).toContain('send that message')
  })

  it('reads as one line, with no double spacing or dangling separator', () => {
    for (const code of CODES) {
      const line = describeErrorLine(code)

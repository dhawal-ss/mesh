...
function fakeRoom() {
  ...
  const remoteMicrophonePublication = {
    isMuted: false,
    audioTrack: {
      mediaStreamTrack: undefined,
  ...
  const remoteParticipant = {
    ...
    getTrackPublication: vi.fn().mockImplementation((source: string) =>
      source === 'microphone' ? remoteMicrophonePublication : undefined,
    ),
  ...
  return {
    ...
    switchActiveDevice: room.switchActiveDevice,
    localParticipant,
    remoteParticipant,
    remoteMicrophonePublication,
...
describe('LiveKitVoiceEngine', () => {
  ...
  it('connects with the backend token and starts an echo-cancelled microphone', async () => {
    ...
    expect(onPeers).toHaveBeenLastCalledWith([
      ...
      expect.objectContaining({ displayName: 'Bob', speaking: true, muted: false }),
    ])
    await expect(engine.getStats()).resolves.toMatchObject({
      joinLatencyMs: expect.any(Number),
      roundTripTimeMs: 42,
      jitterMs: 7,
      packetLossPercent: 5,
      quality: 'excellent',
  ...
  it('renders its own media and keeps each peer stream stable across room events', async () => {
    const fake = fakeRoom()
    const publication = fake.remoteMicrophonePublication as unknown as {
      audioTrack: { mediaStreamTrack?: MediaStreamTrack }
    }
    publication.audioTrack.mediaStreamTrack = { id: 'bob-mic' } as MediaStreamTrack
    // This suite runs outside a DOM, where the engine's own media plumbing lives.
    vi.stubGlobal('MediaStream', class {
      constructor(readonly tracks: MediaStreamTrack[]) {}
    })
    const roomFactory = vi.fn(() => fake.room)
    const onPeers = vi.fn()
    const engine = new LiveKitVoiceEngine({ onPeers }, roomFactory, fakeEncryption().factory)
    ...
    // Mesh attaches media to its own elements, so the SDK never sees one and
    // adaptive stream would report every remote video as invisible and have the
    // server stop sending it.
    expect(roomFactory).toHaveBeenCalledWith(
      expect.objectContaining({ adaptiveStream: false }),
    )

    const bobStreamAt = (call: number) => (
      onPeers.mock.calls[call]?.[0] as Array<{ displayName: string, stream?: MediaStream }>
    ).find((peer) => peer.displayName === 'Bob')?.stream

    const initial = bobStreamAt(0)
    expect(initial).toBeDefined()

    // A talking room emits this continuously. A fresh MediaStream each time
    // would reload the audio element and clip the call.
    fake.emit(RoomEvent.ActiveSpeakersChanged, [])

    expect(bobStreamAt(onPeers.mock.calls.length - 1)).toBe(initial)

    await engine.disconnect()
    vi.unstubAllGlobals()
  })

  it('remembers the chosen camera before video starts and switches an active camera', async () => {
    const fake = fakeRoom()
    ...
    await expect(engine.switchCameraDevice('camera-2')).resolves.toBe(true)
    expect(fake.switchActiveDevice).not.toHaveBeenCalled()
    await engine.setCameraEnabled(true)
    expect(fake.setCameraEnabled).toHaveBeenCalledWith(
      true,
      expect.objectContaining({ deviceId: 'camera-2' }),
    )

    await expect(engine.switchCameraDevice('camera-3')).resolves.toBe(true)
    expect(fake.switchActiveDevice).toHaveBeenCalledWith('videoinput', 'camera-3', true)
    await engine.disconnect()
  })

  it('restarts the live microphone when audio cleanup settings change', async () => {
    const fake = fakeRoom()
    const engine = new LiveKitVoiceEngine({}, () => fake.room, fakeEncryption().factory)
    await engine.connect(
      credentials(),
      'mic-2',
      true,
    ...
    const publication = fake.localParticipant.getTrackPublication(Track.Source.Microphone)
    expect(publication?.audioTrack?.restartTrack).toHaveBeenCalledWith({
      echoCancellation: false,
      noiseSuppression: true,
      autoGainControl: false,
      deviceId: 'mic-2',

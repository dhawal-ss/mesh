...
describe('bridge IPC resilience', () => {
  ...
  it('registers room-update waits as cancellable account-scoped reads', async () => {
    invokeMock.mockResolvedValue('idle' as never)

    await expect(matrixWaitForRoomUpdate('room-3', 25_000)).resolves.toBe('idle')
  ...
  it('caps a stalled mutation instead of leaving it pending forever', async () => {
    vi.useFakeTimers()
    invokeMock.mockImplementation(() => new Promise(() => {}) as never)

    const result = matrixUpdateProfileDisplayName('New name')
    const assertion = expect(result).rejects.toMatchObject({
      code: 'network_unavailable',
      retryable: false,
    })
    ...
    await assertion
    // A write is not idempotent: the renderer stops waiting, but it must not
    // retry, and it must not claim the native work was cancelled.
    expect(invokeMock).toHaveBeenCalledTimes(1)
    expect(invokeMock).toHaveBeenCalledWith('matrix_update_profile_display_name', {
      displayName: 'New name',
    })
  })

  it('gives an account mutation its longer native budget before giving up', async () => {
    vi.useFakeTimers()
    invokeMock.mockImplementation(() => new Promise(() => {}) as never)

    const result = matrixRestoreSession()
    let settled = false
    void result.catch(() => {
      settled = true
    })
    ...
    // Session restore stacks several native deadlines, so the ordinary write
    // cap must not cut it short.
    await vi.advanceTimersByTimeAsync(60_000)
  ...
  it('lets a native picker wait for the person for as long as it takes', async () => {
    vi.useFakeTimers()
    ...
    const intake = pickAttachmentGrants()
    await vi.advanceTimersByTimeAsync(30 * 60_000)
    resolvePicker({
      files: [],
      errors: [],
      accountScope: { accountGeneration: 1, userId: '@alice:example.org' },
    })

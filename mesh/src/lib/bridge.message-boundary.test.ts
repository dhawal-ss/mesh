...
describe('bridge message mutation boundary', () => {
  ...
  it('keeps durable drafts on the protected Matrix room boundary', async () => {
    const bridge = await loadBridge('matrix')
    invokeMock
      .mockResolvedValueOnce({ body: 'restart-safe draft', formattedBody: null } as never)
      .mockResolvedValueOnce(undefined)
      .mockResolvedValueOnce(undefined)

    await expect(
      bridge.loadComposerDraft('!room:example.org'),
    ).resolves.toEqual({ body: 'restart-safe draft', formattedBody: null })
    await bridge.saveComposerDraft('!room:example.org', {
      body: 'updated draft',
      formattedBody: null,
    })
  ...
  it('keeps legacy mutations entirely on legacy commands', async () => {
    ...
    await bridge.saveComposerDraft('channel-1', {
      body: 'legacy session draft',
      formattedBody: null,
    })

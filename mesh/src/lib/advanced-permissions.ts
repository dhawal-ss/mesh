...
export function resolveAdvancedPermission({
  ...
  if (!validateAdvancedPermissionDocument(document)) {
    return unsupported('Permission data is not supported or could not be read.')
  }
  if (
  ...
  if (document.authorityRevision !== authority.revision) {
    return unsupported('Permission data is out of date.')
  }
  if (!validIntegerPower(authority.neededPower)) {
    return unsupported('The required permission level is not supported.')
  }
  ...
  for (const layer of ADVANCED_PERMISSION_LAYER_ORDER) {
    ...
    for (const rule of matching) {
      ...
      if (
        ...
        return unsupported('A rule is out of date or was created without enough permission.')
      }
      if (
        permission === 'roles'

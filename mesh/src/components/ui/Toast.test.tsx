...
describe('ToastContainer accessibility', () => {
  ...
  it('shows a bounded startup notice after the notification region mounts', async () => {
    await act(async () => root.unmount())
    showToast('Something did not finish. Try again.', 'error')
    root = createRoot(container)

    await act(async () => root.render(<ToastContainer />))

    expect(container.querySelector('[role="alert"]')?.textContent)
      .toContain('Something did not finish. Try again.')
  })
})

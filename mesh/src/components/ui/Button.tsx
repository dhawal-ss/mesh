...
const toneClasses: Record<UiTone, Record<'solid' | 'soft' | 'outline' | 'ghost', string>> = {
  ...
  accent: {
    solid: 'border-accent/80 bg-accent text-accent-content hover:bg-accent-hover',
    ...
    return (
      <button
        ...
        className={clsx(
          ...
          'transition-[background-color,border-color,color,box-shadow,transform] duration-fast focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent',
          'disabled:cursor-not-allowed disabled:opacity-40',
          toneClasses[resolvedTone][resolvedVariant],
          {
            'min-h-8 gap-1.5 px-3 py-1 text-xs': size === 'sm',
            'min-h-10 gap-2 px-4 py-2 text-sm': size === 'md',
            'min-h-11 gap-2.5 px-5 py-2.5 text-base': size === 'lg',
          },
          className,
        )}
        {...props}

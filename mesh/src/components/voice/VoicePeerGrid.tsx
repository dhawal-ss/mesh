...
export function VoicePeerGrid({
  ...
  const visiblePeers = useMemo<Peer[]>(() => {
    const sessionPeers = peers.length > 0
      ...
      : sessionSnapshot?.members.map((member) => ({
          ...
          avatarColor: member.avatarColor ?? 'var(--mark-sky)',
  ...
  return (
    <div className="relative flex min-h-0 flex-1 overflow-hidden bg-surface">
      ...
      >
        {reconnecting ? (
          <div className="mb-3 flex flex-none items-center gap-2 border border-marker-container-line bg-marker-container px-3 py-2 text-body-sm text-on-marker-container" role="status">
      ...
      <header className="flex h-14 flex-none items-center justify-between border-b border-outline-variant px-4">
        ...
        {onClose ? (
          <button
            type="button"
            onClick={onClose}
            className="flex h-9 w-9 items-center justify-center text-on-surface-variant hover:bg-state-hover hover:text-on-surface"
            aria-label="Close people list"
          >
            <Icon name="x" size="sm" />
          </button>

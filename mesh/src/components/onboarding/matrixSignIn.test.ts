...
describe('consumer Matrix sign-in helpers', () => {
  ...
  it('fails closed when the recommended build service is absent or unsafe', () => {
    ...
    expect(serviceAddressConfigError('http://remote.example')).toContain('secure service address')
    expect(serviceAddressConfigError(credentialedService)).toContain('sign-in details')
    expect(serviceAddressConfigError('https://mesh.example/?access_token=secret')).toContain('extra information')
    expect(serviceAddressConfigError('https://mesh.example/#account')).toContain('extra information')
  ...
  it('turns classified service failures into next-step guidance', () => {
    ...
    expect(privateConnection).toContain('secure connection')

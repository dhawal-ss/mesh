...
describe('InvitationSurface', () => {
  ...
  it('joins only after explicit action and opens the first text room', async () => {
    ...
    vi.mocked(bridge.getChannels).mockResolvedValue([
      {
        id: '!lounge:community.example',
    ...
    // The decision state: joining, deferring and discarding are all offered
    // before anything is joined.
    expect(queryButton('Save for later')).toBeTruthy()
    expect(queryButton('Discard invitation')).toBeTruthy()
    ...
    expect(useChannelStore.getState().activeChannelId).toBe('!welcome:community.example')
    expect(useMeshNavigationStore.getState().entries.slice(-1)[0]).toEqual({
      kind: 'room',
      communityId: '!lantern:community.example',
      roomId: '!welcome:community.example',
    })
  })

  it('opens the community safely when its rooms are not available yet', async () => {
    const pending = pendingInvitation({ communityName: 'Lantern Guild' })
    useShellStore.getState().setPendingInvitation(pending)
    vi.mocked(bridge.joinPendingInvitation).mockResolvedValue({
      id: '!lantern:community.example',
    ...
    vi.mocked(bridge.getChannels).mockRejectedValue(new Error('rooms still loading'))

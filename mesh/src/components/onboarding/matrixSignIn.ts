...
export function serviceAddressConfigError(value: string): string | null {
  ...
  if (/[\u0000-\u0020\u007f]/.test(rawAddress)) {
    return 'That service address is invalid. Enter a domain like matrix.org.'
  }

  const normalized = normalizeServiceAddress(value)
  if (!normalized) return 'No account service is configured for this version of Mesh.'
  ...
  try {
    ...
    if (url.username || url.password) {
      return 'The service address must not include sign-in details.'
    }
    if (url.search || url.hash) {
      return 'The service address must not include extra information after the domain.'
    }
    if (url.protocol !== 'https:' && !(url.protocol === 'http:' && isLoopback)) {
      return 'Enter a secure service address beginning with https://.'
    }
...
export function friendlyServiceError(cause: unknown, operation: string): string {
  ...
  switch (classifyServiceFailure(cause)) {
    ...
    case 'tls':
      return `${prefix} The account service could not make a secure connection. Check the address or choose another service.`
    case 'http_status':
      return `${prefix} The account service could not complete the request. Try again later or choose another service.`
    case 'malformed_well_known':
      // `.well-known` belongs in the provider details block, not in the lead
      // sentence of a default-path error.
      return `${prefix} Mesh could not read this service's setup information. Choose another service, or ask whoever runs it to check the setup.`
    case 'timeout':
...
export function friendlySignInError(cause: unknown): string {
  ...
  if (error.code === 'network_unavailable') {
    return 'We could not reach your account service. Check your connection and try again.'
  }

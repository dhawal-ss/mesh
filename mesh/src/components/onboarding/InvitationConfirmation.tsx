...
export function InvitationSurface({
  ...
  const join = async () => {
    ...
    try {
      ...
      let firstTextRoomId: string | null = null
      try {
        ...
        firstTextRoomId = channels.find((channel) => channel.channelType === 'text')?.id ?? null
      } catch {
        // The normal room loader retries after the community opens.
      }
      ...
      if (firstTextRoomId) {
        setActiveChannel(firstTextRoomId)
        navigate({
          kind: 'room',
          communityId: community.id,
          roomId: firstTextRoomId,
        }, { replace: true })
      } else {

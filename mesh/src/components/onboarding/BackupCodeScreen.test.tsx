...
describe('BackupCodeScreen', () => {
  ...
  it('shows the supplied code and delegates copy and print actions', async () => {
    ...
    expect(container.textContent).toContain('does not download an unprotected backup file')
  ...
  it('reports protected-device storage and SDK-backed verification honestly', () => {
    ...
    expect(container.textContent).toContain('could not save another copy')
    expect(container.textContent).toContain('could not complete the backup check')
    // The user must still be told to secure the code before leaving.
    expect(container.textContent).toContain('Copy or print the code before leaving')
  })

  it('names the Windows storage boundary and its local-account implication', () => {
    act(() => {
      root.render(
        <BackupCodeScreen
          backupCode={BACKUP_CODE}
    ...
    // The copy that replaced "does not sync to another device": the saved copy
    // is still stated to stay on this device.
    expect(container.textContent).toContain('on this device')
    expect(container.textContent).toContain('someone who can use this Windows account')
  })

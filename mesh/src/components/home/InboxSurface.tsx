...
export function InboxSurface() {
  ...
  return (
    <section className="mesh-inbox-surface flex min-h-0 flex-1 flex-col overflow-hidden" aria-labelledby="mesh-inbox-heading">
      <header className="mesh-route-header mesh-inbox-header flex flex-shrink-0 items-center gap-3 border-b border-border-subtle px-shell-gutter py-2">
        <div className="min-w-0 flex-1">
          <p
            className="mesh-surface-kicker font-mono text-caption font-semibold lowercase tracking-eyebrow text-content-secondary"
          ...
          <h1
            ...
            tabIndex={-1}
            className="text-title font-semibold text-primary outline-none"
          >
            Inbox
          </h1>
        </div>
      ...
      <div className="mesh-inbox-body min-h-0 flex-1 overflow-y-auto bg-surface-base">
        {rows.length === 0 ? (
          /*
          ...
          */
          <div className="border-b border-border-subtle px-shell-gutter py-4">
            <p className="font-semibold text-secondary">You are caught up</p>
          </div>
        ) : rows.map((row) => (
          <div
            key={row.key}
            className="mesh-inbox-row flex w-full items-center gap-3 border-b border-border-subtle hover:bg-surface-hover"
          >
            <button
              type="button"
            ...
            >
              ...
              */}
              {row.unreadMentions > 0 && (
                <span className="font-mono text-meta font-semibold text-status-danger">
              ...
              {row.unreadCount === 0 && row.unreadMentions === 0 && row.unreadMarked && (
                <span className="font-mono text-meta text-accent">Marked unread</span>
              )}
            </button>
            <button
              type="button"
              className="mr-shell-gutter flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-control text-muted hover:bg-surface-active hover:text-primary"

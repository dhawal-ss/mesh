...
describe('saved-message synchronization notice', () => {
  it('explains restore failure without implying saved messages were lost', async () => {
    ...
    // The reassurance sentence is gone, but the invariant it guarded is not:
    // a restore that failed must never be reported as messages being lost.
    expect(container.textContent).not.toMatch(/lost|deleted|gone|discarded/i)

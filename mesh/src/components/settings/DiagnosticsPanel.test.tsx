...
import type { BackendStatus, SystemDiagnostics } from '../../lib/bridge'
...
const mockMatrixStatus = (
  voiceService: Partial<BackendStatus['voiceService']> = {},
...
describe('DiagnosticsPanel', () => {
  ...
  it('does not render anything when open=false', async () => {
    ...
    expect(document.body.textContent).not.toContain('Connection check')
  })
  ...
  it('renders the panel title when open', async () => {
    ...
    expect(document.body.textContent).toContain('Connection check')
  ...
  it('renders the routed Signal Check inline without creating another dialog', async () => {
    ...
    const close = Array.from(document.body.querySelectorAll('button')).find((button) =>
      button.textContent?.includes('Close connection check'),
    )
  ...
  it('does not request diagnostic data until Signal Check is explicitly enabled', async () => {
    ...
    expect(document.body.textContent).toContain('Connection check is off')
    expect(getDiagnostics).not.toHaveBeenCalled()
    const enable = Array.from(document.body.querySelectorAll('button')).find((button) =>
      button.textContent?.includes('Turn on connection check'),
    )
    await act(async () => {
      enable?.click()
      await vi.waitFor(() => {
  ...
  it('shows account health without exposing implementation names in product copy', async () => {
    vi.mocked(getBackendStatus).mockResolvedValue(mockMatrixStatus())
    ...
    expect(document.body.textContent).toContain('Your Mesh account is connected and up to date.')
    ...
    // The calling readout moved behind the same gate as every other voice
    // surface, so a build that cannot open a call shows no calling panel at
    // all. The no-jargon guarantee below is what this test exists to protect.
    expect(document.body.textContent).not.toContain('Private calling')
  ...
  it('shows the calling readout only when the backend can open a call', async () => {
      ...
      mockMatrixStatus(
        {
          ...
          cspReady: true,
          mediaE2eeReady: true,
          livekitServiceUrl: 'https://rtc.example.org',
          tokenEndpoint: 'https://rtc.example.org/get_token',
          reason: null,
        },
        {
          capabilities: {
    ...
    await act(async () => {
      root.render(<DiagnosticsPanel open onClose={() => {}} backendKind="matrix" />)
    })
    await act(async () => {
      await new Promise((resolve) => setTimeout(resolve, 10))
    })
    ...
    expect(document.body.textContent).toContain('Media protection support')
    expect(document.body.textContent).toContain('Available')
    expect(document.body.textContent).toContain('Call access')
    expect(document.body.textContent).toContain('Allowed')
    expect(document.body.textContent).not.toContain('MatrixRTC')
    expect(document.body.textContent).not.toContain('LiveKit')
  })

  it('reports an unreachable calling check as temporary, never as a missing feature', async () => {
    vi.mocked(getBackendStatus).mockResolvedValue(
      mockMatrixStatus(
        {
          availability: 'client-unavailable',
          reason: 'Calling service discovery is temporarily unavailable.',
        },
        { syncRunning: false },
      ),
    )
    ...
    expect(document.body.textContent).toContain('Mesh could not check private calling')
    // The old copy told the user their build has no calling, which stopped
    // them retrying a recoverable network state. The reassurance that followed
    // is gone; the retry button is what tells them the state is recoverable.
    expect(document.body.textContent).not.toContain('unavailable in this version of Mesh')

    const retry = Array.from(document.body.querySelectorAll('button')).find((button) =>
      button.textContent?.includes('Check private calling again'),
    )
    expect(retry).toBeDefined()
    const before = vi.mocked(getBackendStatus).mock.calls.length
    await act(async () => {
      retry?.click()
      await new Promise((resolve) => setTimeout(resolve, 10))
    })
    expect(vi.mocked(getBackendStatus).mock.calls.length).toBeGreaterThan(before)
  })
  ...
  it('displays network connected status when connected', async () => {
    ...
    expect(document.body.textContent).toContain('You and 3 other connections')
  })

  it('displays an offline device state when no other connections are available', async () => {
    vi.mocked(getDiagnostics).mockResolvedValue(
    ...
    expect(document.body.textContent).toContain('Only this device')
    expect(document.body.textContent).toContain('This device is offline from other Mesh users')
  })
  ...
  it('shows TURN missing warning when turn is not configured', async () => {
    ...
    expect(document.body.textContent).toContain('A backup call connection is not configured')
  })
  ...
  it('shows download progress when downloads are active', async () => {
    ...
    expect(document.body.textContent).toContain('100 parts')
  ...
  it('calls onClose when close button is clicked', async () => {
    ...
    const closeButton = document.body.querySelector('button[aria-label="Close connection check"]')
  ...
  it('refreshes when refresh button is clicked', async () => {
    ...
    const refreshButton = document.body.querySelector('button[aria-label="Refresh connection check"]')
  ...
  it('shows reachability probe button and instructions initially', async () => {
    ...
    expect(document.body.textContent).toContain('Connection test')
    expect(document.body.textContent).toContain('Run connection test')
    const probeButton = document.body.querySelector('button[aria-label="Run connection test"]')
  ...
  it('renders probe results when run probe is clicked', async () => {
    ...
    expect(document.body.textContent).toContain('Address not found')
  })
  ...
  it('renders unreachable probe outcome with warning color', async () => {
    ...
    expect(document.body.textContent).toContain('Could not connect')
    expect(document.body.textContent).toContain('No response time is available')
  })
  ...
  it('renders TURN allocation_ok outcome with success color', async () => {
    ...
    expect(document.body.textContent).toContain('Backup connection available')
    expect(document.body.textContent).not.toContain('accepted credentials')
  ...
  it('renders TURN auth_rejected outcome with error color', async () => {
    ...
    expect(document.body.textContent).toContain('Sign-in rejected')
  ...
  it('renders stun_reachable outcome for TURN servers that fail auth but answer STUN', async () => {
    ...
    expect(document.body.textContent).toContain('Direct connection only')
    expect(document.body.textContent).not.toContain('Allocate failed')
  })
})

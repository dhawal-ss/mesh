...
describe('SecurityDevicesPanel', () => {
  ...
  it('renders registered devices and the real second-device path in plain language', async () => {
    await act(async () => {
    ...
    const addDeviceGuide = [...document.body.querySelectorAll('details')].find(
      (details) => details.querySelector('summary')?.textContent?.includes('Add another device'),
    )
    expect(addDeviceGuide?.open).toBe(false)
    expect(addDeviceGuide?.textContent).toContain('Install and open Mesh on the other device.')
    expect(addDeviceGuide?.textContent).toContain(
      'Sign in there with the same account through example.org.',
    )
    expect(addDeviceGuide?.textContent).toContain('choose Check device')
    expect(addDeviceGuide?.textContent).toContain('choose Scan with other device')
  ...
  it('uses the compact empty state when no registered devices are returned', async () => {
    ...
    expect(document.body.textContent).toContain('Add another device')
    expect(document.body.textContent).toContain('Open Mesh on another device and sign in to this account.')
    const emptyTitle = [...document.body.querySelectorAll('h3')].find(
      (heading) => heading.textContent === 'Add another device',
    )
  ...
  it('does not describe an offline device list as an empty account', async () => {
    ...
    expect([...document.body.querySelectorAll('h3')].some(
      (heading) => heading.textContent === 'Add another device',
    )).toBe(false)
  ...
  it('guides lost-device response with an explicit accessible device choice', async () => {
    ...
    expect(document.body.textContent).toContain('Mesh does not save the password')
  ...
  it('shows whether a newly created backup code was protected and verified', async () => {
    ...
    expect(document.body.textContent).toContain('saved another copy in Windows Credential Manager')
    expect(document.body.textContent).toContain('checked the code against your message backup')
  ...
  /*
    Every group label here used to be a `<p>` at 14px, so a 1,500-line safety
    surface exposed two headings in total. The panel is a dialog on one route
    and an inline sub-panel on the other, so the level has to follow the frame
    rather than be hardcoded.
  */
  it('nests group headings under the dialog title', async () => {
    await act(async () => {
      root.render(<SecurityDevicesPanel open onClose={() => {}} />)
      await new Promise((resolve) => setTimeout(resolve, 0))
    })
    ...
    for (const label of ['This device', 'Message backup', 'Your devices', 'Your personal data']) {
  ...
  it('steps group headings down one level when it renders inline in You', async () => {
    await act(async () => {
      root.render(<SecurityDevicesPanel embedded open onClose={() => {}} />)
      await new Promise((resolve) => setTimeout(resolve, 0))
    })
    await act(async () => {
      await new Promise((resolve) => setTimeout(resolve, 0))

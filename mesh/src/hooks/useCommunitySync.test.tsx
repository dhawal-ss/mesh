...
describe('Matrix community roster synchronization', () => {
  ...
  it('carries each member profile picture through to the roster', async () => {
    /*
      This mapping dropped avatarUrl while the store, the DTO, and the Avatar
      element all carried it, so a member with a picture set rendered a
      generated mark and nothing anywhere reported a problem. Losing a field in
      a hand-written projection is invisible by construction: the object still
      type-checks, because the field is optional.
    */
    vi.mocked(bridge.getMemberPage).mockResolvedValue({
      members: [{
        publicKey: '@alice:mesh.test',
        displayName: 'Alice',
        avatarColor: '#607080',
        avatarUrl: 'mxc://mesh.test/alice-picture',
        role: 'member',
        joinStatus: 'joined',
        banStatus: 'none',
        lastSeen: null,
    ...
    const roster = useMembershipStore.getState().members['!community:mesh.test']
    expect(roster?.[0]?.avatarUrl).toBe('mxc://mesh.test/alice-picture')
  })

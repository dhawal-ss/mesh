...
export function useDurableDraft(
  ...
  applyLoadedDraft: (draft: ComposerDraftDto) => void,
): DurableDraftState {
  const matrixMode = bridge.isMatrixBackend()
  const { body, formattedBody } = draft
  ...
  useEffect(() => {
    latestDraftRef.current = { body, formattedBody }
  }, [body, formattedBody])
  ...
  useEffect(() => {
    ...
    void bridge.loadComposerDraft(roomId).then((loadedDraft) => {
      ...
      const remoteDraft = loadedDraft ?? { body: '', formattedBody: null }
      const remoteValue = remoteDraft.body
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const operation = body
        ? () => bridge.saveComposerDraft(roomId, { body, formattedBody })
  ...
  }, [body, enqueueWrite, formattedBody, matrixMode, roomId, status])
  ...
  useEffect(
    () => () => {
      ...
      const operation = pendingDraft.body
        ? () => bridge.saveComposerDraft(roomId, pendingDraft)
        : () => bridge.clearComposerDraft(roomId)

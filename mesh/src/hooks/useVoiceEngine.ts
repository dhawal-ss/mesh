...
export function useVoiceEngine() {
  ...
  const matrixVoiceFrontendEnabled = isMatrixVoiceFrontendEnabled()
  const matrixVoiceReady = matrixVoiceFrontendEnabled && canStartMatrixVoice(backendStatus)
  ...
  const setAudioProcessingState = useVoiceStore((state) => state.setAudioProcessing)
  ...
  const refreshDevices = useCallback(async (requestPermissions = false) => {
    ...
    try {
      ...
      setConnectionWarning(errorLine(description))
    }
  }, [])
  ...
  useEffect(() => {
    ...
    /**
     * Release every legacy voice listener registered so far. Draining the
     * array keeps a second call safe, and settling each task on its own means
     * one rejected registration cannot swallow the other four cleanups.
     */
    const releaseLegacyListeners = () => {
      const tasks = unlistenTasks.splice(0, unlistenTasks.length)
      if (tasks.length === 0) return
      void Promise.allSettled(tasks).then((results) => {
        for (const result of results) {
          if (result.status === 'rejected') {
            console.warn('A voice listener never registered:', result.reason)
            continue
          }
          try {
            result.value()
          } catch (error) {
            console.warn('Failed to stop a voice listener:', error)
          }
        }
      })
    }
    ...
    if (backendStatus?.kind === 'matrix') {
      ...
      if (!matrixVoiceFrontendEnabled) {
        setConnectionState('disconnected', voiceServiceUserMessage('client-unavailable'))
        return
      }

      if (!matrixVoiceReady) {
        setConnectionState('disconnected', voiceServiceUserMessage(voiceService.availability))
        return
      }
      ...
      const startMatrixEngine = async () => {
        ...
        try {
          ...
          await engine.setAudioProcessing(audioProcessingRef.current)
          if (
          ...
          /*
           * Requesting devices here, before connect() ever touches the
           * microphone, is what lets a denied permission reach the UI while
           * the lifecycle is still "requesting" instead of only surfacing
           * after connect() has already tried (and failed) to publish audio.
           */
          ...
          if (cameraDeviceIdRef.current) {
            await engine.switchCameraDevice(cameraDeviceIdRef.current)
          }
        } catch (error) {
          ...
          setConnectionState('disconnected', errorLine(description))
        }
      }
    ...
    const startLegacyEngine = async () => {
      ...
      const engine = new VoiceEngine(currentCommunityId, currentChannelId, {
        ...
        onConnectionState: (state, reason) => {
          ...
          setConnectionState(state, errorLine(description))
        },
        onError: (message) => {
      ...
      // Each registration is tracked the moment it starts. Collecting them
      // into a local array first meant a teardown that ran before this point
      // saw an empty task list and leaked all five listeners permanently.
      unlistenTasks.push(
      ...
      // Teardown may have run during the await above, and it releases only the
      // listeners it can see at that moment.
      if (disposed) releaseLegacyListeners()
    }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Release only what push-to-talk actually engaged. Running the full
      // release here asserted mute on teardown, so switching to voice activity
      // dropped the person into the new mode silently muted, undoing the
      // unmute the mode change itself had just applied, with nothing on screen
      // to explain it and no unmute on the new mode's mount.
      if (!pttKeyboardActiveRef.current) return
      pttKeyboardActiveRef.current = false
  ...
  const switchCameraDevice = useCallback(async (deviceId: string) => {
    const switched = await liveKitEngineRef.current?.switchCameraDevice(deviceId)
    if (!switched) throw new Error('The camera could not be selected')
  }, [])

  const updateAudioProcessing = useCallback(async (settings: AudioProcessingSettings) => {
    const engine = liveKitEngineRef.current
    if (!engine) throw new Error('Voice must be connected before audio processing can change')
    await engine.setAudioProcessing(settings)
    audioProcessingRef.current = settings
    setAudioProcessingState(settings)
  ...
  const toggleCamera = useCallback(async (enabled: boolean) => {
    const engine = liveKitEngineRef.current
    if (!engine) throw new Error('Voice must be connected before the camera can change')
    await engine.setCameraEnabled(enabled)
    setCameraEnabled(enabled)
  }, [setCameraEnabled])

  const toggleScreenSharing = useCallback(async (enabled: boolean) => {
    const engine = liveKitEngineRef.current
    if (!engine) throw new Error('Voice must be connected before screen sharing can change')
    await engine.setScreenShareEnabled(enabled)
  ...
  const matrixUnavailableReason = useMemo(() => {
    ...
    if (!voiceService.mediaE2eeReady) {
      return 'Mesh has not confirmed that this call is protected, so the microphone will stay off.'
    }
    return voiceServiceUserMessage(voiceService.availability)
  }, [backendStatus?.kind, matrixVoiceReady, voiceService.availability, voiceService.mediaE2eeReady])

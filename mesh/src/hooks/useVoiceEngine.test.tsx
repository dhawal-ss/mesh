...
vi.mock('@mesh/matrix-voice-runtime', () => ({
  LiveKitVoiceEngine: class {
    ...
    setAudioProcessing = mocks.setAudioProcessing
    setMuted = vi.fn().mockResolvedValue(undefined)
    setDeafened = vi.fn()
    switchCameraDevice = vi.fn().mockResolvedValue(true)
  },
}))
...
describe('useVoiceEngine MatrixRTC lifecycle', () => {
  ...
  beforeEach(() => {
    ...
    mocks.setAudioProcessing.mockReset().mockResolvedValue(undefined)
  ...
  it('requests device permissions before connect() ever touches the microphone', async () => {
    await act(async () => {
      root.render(<Harness />)
    })

    await vi.waitFor(() => expect(mocks.connect).toHaveBeenCalledOnce())
    expect(mocks.getDevices).toHaveBeenCalledWith(true)
    const getDevicesOrder = mocks.getDevices.mock.invocationCallOrder[0]
    const connectOrder = mocks.connect.mock.invocationCallOrder[0]
    expect(getDevicesOrder).toBeLessThan(connectOrder)
  })
  ...
  it('leaves the microphone alone when the person leaves push-to-talk mode', async () => {
    act(() => {
      useVoiceStore.getState().setInputMode('push-to-talk')
    })
    ...
    // Choosing voice activity unmutes. The push-to-talk teardown used to
    // assert mute right after that, so the person landed in the new mode
    // silently muted with nothing on screen to explain it.
    expect(useVoiceStore.getState().isMuted).toBe(false)
    expect(useVoiceStore.getState().isPushToTalking).toBe(false)
  })
})

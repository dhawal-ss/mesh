...
export interface PublicService {
  ...
  /**
   * When Mesh's review of this operator goes stale. Soft: it withdraws the
   * recommendation and the account-creation offer, and nothing else. Someone who
   * already has an account here must still be able to sign in, so this date
   * never gates {@link publicServiceReviewExpired} consumers into blocking sign-in.
   */
  reviewAfter: string
  /**
   * When the entry stops being offered at all. Hard, and deliberately far past
   * `reviewAfter`. A build ships with no update channel, so an installed copy
   * has to stay usable for its supported life: tying the whole option to the
   * review date would have every install lose its account choices on one day.
   */
  hardExpiryAfter: string
  prominent: boolean
}
...
export function validatePublicServiceCatalog(value: unknown): PublicServiceCatalogValidation {
  ...
  value.forEach((candidate, index) => {
    ...
    if (!validDate(candidate.hardExpiryAfter)) {
      errors.push(`${path}.hardExpiryAfter must be an ISO date`)
    } else if (
      validDate(candidate.reviewAfter)
      && candidate.hardExpiryAfter <= candidate.reviewAfter
    ) {
      // Equal dates would collapse the two concerns back into one and restore
      // the single-day cliff this split exists to remove.
      errors.push(`${path}.hardExpiryAfter must follow reviewAfter`)
    }
...
/**
 * Mesh's review of this operator has gone stale.
 *
 * Withdraw the recommendation and the account-creation offer. Do NOT use this
 * to disable sign-in: a stale review says Mesh has not re-checked the operator,
 * not that the operator has gone away, and locking an existing account holder
 * out of their own account over Mesh's paperwork is the worse failure.
 */
...
/**
 * The entry has aged out entirely and must stop being offered, sign-in included.
 */
export function publicServiceUnavailable(service: PublicService, now = new Date()): boolean {
  return new Date(`${service.hardExpiryAfter}T23:59:59.999Z`).valueOf() < now.valueOf()
}

...
function mergePeerRecord(
  ...
  return {
    ...
    muted: next.muted ?? peer.muted ?? false,
  }
}
...
/**
 * Call participants, each carrying the picture of the person behind them.
 *
 * The join happens here rather than in the voice adapter because neither side
 * has both halves: LiveKit knows who is in the call but identifies them by an
 * opaque digest, deliberately, so the SFU never learns a Matrix user ID; the
 * room's MatrixRTC membership knows the person and their picture but not who is
 * currently audible. `participantIdentity` is the same digest on both sides,
 * and it is the one end-to-end encrypted media already depends on matching.
 *
 * Every consumer of the peer list reads through this, so a call tile, the dock
 * and the roster cannot disagree about whether someone has a picture.
 */
export function useCallPeers(): Peer[] {
  const peers = useVoiceStore((state) => state.peers)
  const roomId = useVoiceStore((state) => state.currentChannelId)
  const members = useVoiceStore((state) =>
    roomId ? state.matrixRtcMembersByRoom[roomId] : undefined,
  )

  return useMemo(() => {
    if (!members?.length) return peers
    // Keyed only by call identity, never by account: every peer in a Matrix
    // call is named by its digest, including this account's own, because
    // `localPublicKey` is set from the join credentials before the call
    // connects and no peer exists before that.
    const pictures = new Map<string, string>()
    for (const member of members) {
      if (member.avatarUrl) pictures.set(member.participantIdentity, member.avatarUrl)
    }
    if (pictures.size === 0) return peers
    return peers.map((peer) => {
      const avatarUrl = pictures.get(peer.publicKey)
      return avatarUrl ? { ...peer, avatarUrl } : peer
    })
  }, [members, peers])
}

registerAccountReset('voice', () => {

...
interface TypingStore {
  ...
  /** Replace a channel snapshot after the native account reports a change. */
  setTypingUsers: (
    channelId: string,
    users: Array<{ author: string; displayName: string }>,
  ) => void
...
export const useTypingStore = create<TypingStore>((set, get) => ({
  ...
  setTypingUsers: (channelId, users) => {
    ...
    set((state) => ({
      typingByChannel: {
        ...state.typingByChannel,
        [channelId]: users.map((user) => ({ ...user, expiresAt })),
      },
    }))
  },

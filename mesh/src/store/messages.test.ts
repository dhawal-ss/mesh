...
describe('durable Matrix queue reconciliation', () => {
  ...
  it('carries a named send failure, and drops it the moment the send is retried', () => {
    const store = useMessageStore.getState()
    store.acceptQueuedMessage(queued())
    store.applyQueuedMessageUpdate({
      roomId: 'ch-1',
    ...
    expect(useMessageStore.getState().messages['ch-1'][0].sendFailure).toBe('not-allowed')

    // A retry puts the message back in flight, so a reason describing why the
    // last attempt was hopeless must not survive onto the new one.
    store.applyQueuedMessageUpdate({
      roomId: 'ch-1',
      transactionId: 'txn-1',
      state: 'pending',
    })
    expect(useMessageStore.getState().messages['ch-1'][0].sendFailure).toBeNull()
  })

  it('leaves the failure unset when the backend does not name one', () => {
    const store = useMessageStore.getState()
    store.acceptQueuedMessage(queued())
    store.applyQueuedMessageUpdate({
      roomId: 'ch-1',
    ...
    // Not knowing why is not the same as knowing it is hopeless: the renderer
    // reads an absent reason as "a retry is still fair to offer".
    expect(useMessageStore.getState().messages['ch-1'][0].sendFailure).toBeNull()
  })

...
const [
  ...
  read('infra/homeserver/backup.sh'),
  read('infra/homeserver/backup-lib.sh'),
...
assert.match(homeserverBackup, /docker compose exec -T --user 991:991 synapse[\s\\]*\n\s*tar -czf - -C \/data/)
assert.match(homeserverBackup, /test -f "\/data\/\$required_file"/)
assert.match(homeserverBackup, /test ! -L "\/data\/\$required_file"/)
assert.match(homeserverBackup, /printf present; else printf absent/)
assert.match(homeserverBackup, /refusing an incomplete backup/)
assert.doesNotMatch(homeserverBackup, /tar -czf "\$staging\/synapse-critical\.tar\.gz"/)
assert.match(homeserverBackupLib, /stream_recovery_artifact\(\) \(/)
assert.match(homeserverBackupLib, /Refusing to replace an existing recovery artifact/)
assert.match(homeserverRestoreTest, /cp[\s\\]+\n\s*"\$homeserver_dir\/backup\.sh"/)
assert.match(homeserverRestoreTest, /MESH_SYNAPSE_CONTAINER="\$synapse_container"/)
assert.match(homeserverRestoreTest, /assert_host_owned_readable_file "\$backup_dir\/synapse-critical\.tar\.gz"/)
assert.equal(
...
assert.match(securityPolicy, /\*\*Confidential route status: enabled\.\*\*/)
assert.match(publicSecurityPage, /Report a vulnerability confidentially/)
assert.match(publicSecurityPage, /github\.com\/dhawal-ss\/mesh\/security\/advisories\/new/)
...
console.log('Operations contract passed: disclosure, UID-bound host-owned backup streaming, two-cycle recovery evidence, link-safe cleanup, transactional full-secret rotation, pinned Synapse, and isolated admission runtime.')

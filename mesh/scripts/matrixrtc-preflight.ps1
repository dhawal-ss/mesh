...
if (Test-Path -LiteralPath $composePath) {
    ...
    $authImage = "image: ghcr.io/dhawal-ss/mesh-lk-jwt-service:0.5.0-mesh.2@sha256:78bf2f1e8535928037abc35a9886614b3f722885b54f1f707a570e6991252710"
    ...
    if ($compose -notmatch [regex]::Escape($authImage)) {
        Add-Failure "MatrixRTC Authorization Service must remain pinned to the reviewed Mesh 0.5.0-mesh.2 OCI manifest digest."
    }
...
if ((Test-Path -LiteralPath $packagePath) -and
    ...
    (Test-Path -LiteralPath $disabledVoicePath)) {
    ...
    if ($voiceHook -notmatch 'isMatrixVoiceFrontendEnabled\(\)' -or
        $voiceRuntime -notmatch 'function\s+shouldExposeVoiceRoutes' -or
        $voiceRuntime -notmatch 'matrixFrontendEnabled\s*&&\s*canStartMatrixVoice\(status\)' -or
        $disabledVoice -notmatch 'throw new Error' -or
        $disabledVoice -notmatch 'Calling is not included in this version of Mesh') {
    ...
    if ($failures.Count -eq $buildFailureCount) {
        Add-Pass "Non-voice builds fail closed and the matrix-voice build stays separately named."
    }
}
...
if ($docker -and (Test-Path -LiteralPath $composePath) -and
    ...
    if ($RequireCompose -or $Production -or $RequireLiveAcceptance) {
        Add-Failure "Docker CLI, the MatrixRTC Compose file, and the environment file are required so Compose rendering cannot be skipped."
    } else {

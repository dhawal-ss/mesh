...
describe('external acceptance evidence validator', () => {
  it('accepts the fail-closed tracked template', () => {
    assert.equal(REQUIRED_EXTERNAL_ACCEPTANCE_IDS.length, 63)
    assert.equal(requiredExternalAcceptanceIdsForMilestone('R2').length, 55)
    assert.equal(requiredExternalAcceptanceIdsForMilestone('R4').length, 63)
  ...
  it('rejects live evidence that is explicitly marked unsanitized', () => {
    const document = copy(template)
    document.status = 'passed'
    document.acceptanceMilestone = 'R2'
    document.sourceSha = 'a'.repeat(40)
    document.releaseTag = 'v0.2.0'
    document.testedAt = '2026-08-01T00:00:00Z'
    document.expiresAt = '2026-09-01T00:00:00Z'
    document.operator = 'release acceptance role'
    document.artifacts.push({
      id: 'download-log',
    ...
    document.results[0].evidenceIds = ['download-log']
    ...
    assert.ok(errors.some((error) => error.includes('must be explicitly sanitized')))
  })
  ...
  it('allows benign operational prose and requires human review for opaque visual evidence', () => {
    ...
    assert.ok(inspectEvidenceContent(Buffer.from('%PDF-1.7'), 'application/pdf').some((error) => error.includes('privacy reviewer')))
    assert.ok(inspectEvidenceContent(new Uint8Array([0, 0, 0, 24]), 'video/mp4').some((error) => error.includes('privacy reviewer')))
  })
})

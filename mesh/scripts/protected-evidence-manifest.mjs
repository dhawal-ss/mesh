...
const IMMUTABLE_RUN_URI = /^https:\/\/github\.com\/[^/]+\/[^/]+\/actions\/runs\/[1-9][0-9]*$/
...
export function validateProtectedEvidenceManifest(manifest, { now = new Date() } = {}) {
  ...
  if (!manifest.runner?.os || !manifest.runner?.arch || Object.keys(manifest.runner?.toolVersions ?? {}).length === 0) fail('runner OS, architecture, and tool versions are required')
  ...
  for (const [index, command] of (manifest.commands ?? []).entries()) {
    if (!command.command || !['pass', 'fail'].includes(command.status)) fail(`commands[${index}] is invalid`)
  ...
  if (!manifest.build?.mode || !Array.isArray(manifest.build?.features)) fail('build mode and feature set are required')
  if (!Number.isFinite(Date.parse(manifest.collectedAt ?? ''))) fail('collectedAt is invalid')
  if (!Number.isFinite(Date.parse(manifest.expiresAt ?? ''))) fail('expiresAt is invalid')
  else if (Date.parse(manifest.expiresAt) <= now.getTime()) fail('evidence manifest is expired')

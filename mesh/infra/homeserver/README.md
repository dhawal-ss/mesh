...
> **Upstream image disclosure:** The referenced Synapse and PostgreSQL images
> are published upstream. Mesh does not operate them and cannot patch them for
> a community owner. The pinned Synapse image currently has fixable High or
> Critical vulnerability findings. Mesh keeps those findings visible in its
> container scan evidence, but a community owner must review the current
> upstream security advisories, release notes, image digest, and a fresh
> fixable High and Critical scan before deploying or upgrading this optional
> stack. Do not expose the service if those findings are outside the owner's
...
Run `backup.sh` as the host operator while the Compose services are healthy.
It reads Synapse recovery files through the running container as UID 991, while
the invoking host shell creates the private backup artifacts. Do not loosen or
change ownership of `runtime/synapse` to make backups work, and do not run the
whole backup process inside the Synapse container.

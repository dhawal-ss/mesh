...
do
  if ! docker compose exec -T --user 991:991 synapse \
       test -f "/data/$required_file" ||
     ! docker compose exec -T --user 991:991 synapse \
       test ! -L "/data/$required_file"
  then
    echo "Required homeserver file is missing, unreadable, or unsafe: /data/$required_file" >&2
    exit 1
  fi
done
...
stream_recovery_artifact \
  "$staging/synapse-critical.tar.gz" \
  docker compose exec -T --user 991:991 synapse \
  tar -czf - -C /data \
  homeserver.yaml \
  "$MESH_SERVER_NAME.signing.key" \
...
if ! media_state="$(
  docker compose exec -T --user 991:991 synapse \
    sh -c 'if [ -d /data/media_store ]; then printf present; else printf absent; fi'
)"
then
  echo "Could not determine whether the Synapse media store exists; refusing an incomplete backup." >&2
  exit 1
fi
case "$media_state" in
  present)
    stream_recovery_artifact \
      "$staging/media-store.tar.gz" \
      docker compose exec -T --user 991:991 synapse \
      tar -czf - -C /data \
      media_store
    ;;
  absent) ;;
  *)
    echo "Synapse returned an invalid media-store state; refusing an incomplete backup." >&2
    exit 1
    ;;
esac

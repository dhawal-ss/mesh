...
case "$*" in
  ...
  *" pg_restore --list")
    cat >/dev/null
    ;;
  "compose exec -T --user 991:991 synapse test -f /data/"*)
    exit 0
    ;;
  "compose exec -T --user 991:991 synapse test ! -L /data/"*)
    exit 0
    ;;
  "compose exec -T --user 991:991 synapse sh -c "*"/data/media_store"*)
    if [ "${FAKE_DOCKER_FAIL_MEDIA_STATE:-0}" = "1" ]; then
      exit 1
    fi
    if [ -d runtime/synapse/media_store ]; then
      printf present
    else
      printf absent
    fi
    ;;
  "compose exec -T --user 991:991 synapse tar -czf - -C /data media_store")
    tar -czf - -C runtime/synapse media_store
    ;;
  "compose exec -T --user 991:991 synapse tar -czf - -C /data homeserver.yaml mesh.example.test.signing.key mesh.example.test.log.config")
    tar -czf - -C runtime/synapse \
      homeserver.yaml \
      mesh.example.test.signing.key \
      mesh.example.test.log.config
    ;;
...
if (
  cd "$backup_tool"
  PATH="$backup_tool/fake-bin:$PATH" FAKE_DOCKER_FAIL_MEDIA_STATE=1 \
    sh ./backup.sh >/dev/null 2>&1
); then
  echo "backup.sh accepted a failed media-state query." >&2
  exit 1
fi
if find "$backup_tool/runtime/backups" -mindepth 1 -maxdepth 1 \
  \( -name '.*.partial.*' -o -name '.backup.lock' \) |
  grep -q .
then
  echo "backup.sh left partial state after a failed media-state query." >&2
  exit 1
fi

backup_log="$test_root/backup-tool.log"
if ! backup_path="$(
  cd "$backup_tool"
  PATH="$backup_tool/fake-bin:$PATH" sh ./backup.sh 2> "$backup_log"
)"
then
  cat "$backup_log" >&2
  echo "backup.sh failed its successful-backup fixture." >&2
  exit 1
fi
test -d "$backup_path"
...
stream_fixture="$test_root/streamed-recovery.bin"
stream_recovery_artifact "$stream_fixture" printf 'recovery payload\n'
assert_host_owned_readable_file "$stream_fixture"
if stream_recovery_artifact "$stream_fixture" printf 'replacement\n' >/dev/null 2>&1; then
  echo "recovery streaming replaced an existing artifact." >&2
  exit 1
fi
failed_stream="$test_root/failed-stream.bin"
if stream_recovery_artifact \
     "$failed_stream" \
     sh -c 'printf partial; exit 1' \
     >/dev/null 2>&1
then
  echo "recovery streaming accepted a failed producer." >&2
  exit 1
fi
if [ -e "$failed_stream" ] || [ -L "$failed_stream" ]; then
  echo "recovery streaming retained a partial artifact." >&2
  exit 1
fi
prune_local_backups \
  "$retention_root" \

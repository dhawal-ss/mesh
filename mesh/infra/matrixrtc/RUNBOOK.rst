...
6. Complete acceptance-matrix.example.json on two physical devices and the
   required networks, including microphone, camera, screen video, optional
   display audio, native sharing-stop, reconnect, and late-join behavior. Copy
   the template to an operator-owned location and

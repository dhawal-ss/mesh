...
export default defineConfig(async ({ command, mode }) => {
  ...
  resolve: {
    ...
    alias: [
      {
        find: "@mesh/workspace-preview-runtime",
        replacement: fileURLToPath(new URL(
          command === "serve" || mode === "performance-fixture"
            ? "./src/dev/installWorkspacePreview.ts"
            : "./src/dev/installWorkspacePreview.disabled.ts",
          import.meta.url,
        )),
      },
      {
        find: "@mesh/workspace-preview-state",
        replacement: fileURLToPath(new URL(
          command === "serve" || mode === "performance-fixture"
            ? "./src/dev/WorkspacePreviewState.tsx"
            : "./src/dev/WorkspacePreviewState.disabled.tsx",
          import.meta.url,
        )),
      },
      {
  ...
  define: {
    ...
    // The optimized large-history harness uses a separate output directory.
    // This constant is false in every shipping Matrix and LAN build so design
    // preview fixtures cannot enter a release candidate.
    __MESH_PERFORMANCE_FIXTURE__: JSON.stringify(mode === "performance-fixture"),
  },
  ...
  build: {
    // Keep pixel-art masks as cacheable files. Vite's default base64 inlining
    // inflated the shipping stylesheet and duplicated encoding overhead.
    assetsInlineLimit: 0,
    // Lightning CSS safely merges equivalent rules and shorthands that the
    // default esbuild CSS pass leaves duplicated in the design-system output.

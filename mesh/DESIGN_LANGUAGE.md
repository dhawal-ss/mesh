...
The shape scale is the system. `--shape-xs` 4 px, `--shape-sm` 8 px, `--shape-md` 12 px, `--shape-lg` 16 px, `--shape-lg-inc` 20 px, `--shape-xl` 28 px, `--shape-xl-inc` 32 px, and `--shape-full` for a pill or a circle. `--shape-none` is 0 and is legal only on full-bleed media clipped by an ancestor that has its own radius. Every pane is a 28 px rounded surface inset by 12 px from the window and from its neighbours, so there are no shared edges left for a rule to draw. The rail is the exception, and it is not a pane: it carries the window ground with no radius and no inset, so the list and the conversation are the only rounded surfaces and the rail reads as the frame they sit in. The panes step up from that ground -- `--surface` for the window and the rail, `--surface-container-low` for the list and the conversation, `--surface-container` for an app bar, a user panel or a docked side sheet, and `--surface-container-high` for the composer.
